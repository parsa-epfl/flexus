...
namespace Flexus {
//Simulator Name
std::string theSimulatorName = "TSEFlex v1.0";
}
...
#include <components/Nic/Nic4.hpp>
...
CREATE_CONFIGURATION( Nic4, "nic", theNicCfg );
CREATE_CONFIGURATION( NetShim, "network", theNetworkCfg );

CREATE_CONFIGURATION( PrefetchBuffer, "pb", thePrefetchBufferCfg );
CREATE_CONFIGURATION( PrefetchListener, "pl", thePrefetchListenerCfg );
CREATE_CONFIGURATION( Consort, "consort", theConsortCfg );
CREATE_CONFIGURATION( MRCReflector, "reflector", theReflectorCfg );
...
bool initializeParameters() {
  ...
  thePrefetchBufferCfg.NumEntries.initialize(0);
  thePrefetchBufferCfg.QueueSizes.initialize(8);
  thePrefetchBufferCfg.ProcessingDelay.initialize(0);
  thePrefetchBufferCfg.NumWatches.initialize(16);
  thePrefetchBufferCfg.MAFSize.initialize(48);

  thePrefetchListenerCfg.VChannels.initialize(3);
  thePrefetchListenerCfg.ConcurrentPrefetches.initialize(16);
  thePrefetchListenerCfg.NumQueues.initialize(16);
  thePrefetchListenerCfg.TargetBlocks.initialize(8);
  thePrefetchListenerCfg.MoreAddressThreshold.initialize(15);

  theReflectorCfg.VChannels.initialize(3);
  theReflectorCfg.Enable.initialize(false);
  theReflectorCfg.TwoEnable.initialize(true);
  theReflectorCfg.AllMisses.initialize(false);

  theConsortCfg.VChannels.initialize(3);
  theConsortCfg.Enable.initialize(false);
  theConsortCfg.OrderSize.initialize(250000);
  theConsortCfg.AddressList.initialize(16);
  theConsortCfg.CoherenceBlockSize.initialize(64);
  theConsortCfg.OrderReadLatency.initialize(240);
  theConsortCfg.OrderCacheSize.initialize(32);
  theConsortCfg.AddressList.initialize(16);
  theConsortCfg.Trace.initialize(false);
  theConsortCfg.AllMisses.initialize(false);
...
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( PrefetchBuffer, thePrefetchBufferCfg, theSVB, SCALE_WITH_SYSTEM_WIDTH, MULTIPLY, 1 );
...
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( Nic4, theNicCfg, theNic, SCALE_WITH_SYSTEM_WIDTH, MULTIPLY, 1 );
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( PrefetchListener, thePrefetchListenerCfg, thePrefetchListener, SCALE_WITH_SYSTEM_WIDTH, MULTIPLY, 1 );
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( Consort, theConsortCfg, theConsort, SCALE_WITH_SYSTEM_WIDTH, MULTIPLY, 1 );
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( MRCReflector, theReflectorCfg, theReflector, SCALE_WITH_SYSTEM_WIDTH, MULTIPLY, 1 );
...
WIRE( theL1d, BackSideOut_Snoop,        theSVB, FrontSideIn_Snoop         )
WIRE( theIDMux, TopOutD,                theL1d, BackSideIn                )

//IDMux to SVB
WIRE( theIDMux, BottomOut,              theSVB, FrontSideIn_Request       )
WIRE( theSVB, FrontSideOut,             theIDMux, BottomIn                )

//SVB to L2
WIRE( theSVB, BackSideOut_Request,      theL2, FrontSideIn_Request        )
WIRE( theSVB, BackSideOut_Snoop,        theL2, FrontSideIn_Snoop          )
WIRE( theL2, FrontSideOut,              theSVB, BackSideIn                )
...
//PrefetchListener to SVB
WIRE( thePrefetchListener, ToPrefetchBuffer, theSVB, MasterIn             )
WIRE( theSVB, MasterOut,                thePrefetchListener, FromPrefetchBuffer)

//Execute to Consort
WIRE( theExecute, ToConsort,            theConsort, FromCPU               )

//Engines to Reflector
WIRE( theProtocolEngine, ToPredictor,   theReflector, FromEngine          )
WIRE( theLocalEngine, ToPredictor,      theReflector, FromLocal           )

//ProtocolEngine to Nic
WIRE( theProtocolEngine, ToNic,         theNic, FromNode0                 )
WIRE( theNic, ToNode0,                  theProtocolEngine, FromNic        )

//PrefetchListener to Nic
WIRE( thePrefetchListener, ToNic,       theNic, FromNode1                 )
WIRE( theNic, ToNode1,                  thePrefetchListener, FromNic      )

//Consort to Nic
WIRE( theConsort, ToNic,                theNic, FromNode2                 )
WIRE( theNic, ToNode2,                  theConsort, FromNic               )

//Reflector to Nic
WIRE( theReflector, ToNic,              theNic, FromNode3                 )
WIRE( theNic, ToNode3,                  theReflector, FromNic             )
...
, DRIVE( theReflector, ReflectorDrive )
, DRIVE( theConsort, ConsortDrive )
...
, DRIVE( thePrefetchListener, PrefetchListenerDrive )
