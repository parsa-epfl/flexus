...
namespace Flexus {
//Simulator Name
std::string theSimulatorName = "TraceNUCAFlex 1.0";
}
...
CREATE_CONFIGURATION( FastCMPNetworkController, "network-control", theCMPNetControllerCfg );
CREATE_CONFIGURATION( WhiteBox, "white-box", theWhiteBoxCfg );
CREATE_CONFIGURATION( FastVictimCache, "victim", theVictimCacheCfg );
...
#   define PAGE_SIZE   8192
  ...
  theFeederCfg.DecoupleInstrDataSpaces.initialize(true); // Must be true for R-NUCA /* CMU-ONLY */
  theFeederCfg.WhiteBoxDebug.initialize(false);
  theFeederCfg.WhiteBoxPeriod.initialize(10000);
  ...
  theVictimCacheCfg.Enable.initialize(true);
  theVictimCacheCfg.NumEntries.initialize(16);
  theVictimCacheCfg.CacheLineSize.initialize(BLOCK_SIZE);
  ...
  theL2Cfg.CleanEvictions.initialize(false);      // Must be true only for private: the implementation depends on this
  ...
  // ASR parameters
  theL2Cfg.DirtyRingWritebacks.initialize(false);   // Enable dirty ring writebacks
  /* CMU-ONLY-BLOCK-END */
  ...
  theCMPNetControllerCfg.Placement.initialize("shared");
  ...
  /* CMU-ONLY-BLOCK-BEGIN */
  theCMPNetControllerCfg.PrivateWithASR.initialize(false);
  theCMPNetControllerCfg.SizeOfInstrCluster.initialize( SIZE_INSTR_CLUSTER );
  theCMPNetControllerCfg.SizeOfPrivCluster.initialize( SIZE_PRIV_CLUSTER );
  theCMPNetControllerCfg.DecoupleInstrDataSpaces.initialize(true); // Must be true for R-NUCA
  // Dir size study
  theCMPNetControllerCfg.DirEntries.initialize(0);
  theCMPNetControllerCfg.DirWays.initialize(16);
  // ASR parameters
  theCMPNetControllerCfg.FramesPerL2Slice.initialize(512 * K / BLOCK_SIZE);
  theCMPNetControllerCfg.ASRLocalHitLatency.initialize(1);
  theCMPNetControllerCfg.ASRRemoteHitLatency.initialize(4);
  theCMPNetControllerCfg.ASROffChipLatency.initialize(18);
  ...
  theCMPNetControllerCfg.CleanSingletonRingWritebacks.initialize(false); // Enable ring writebacks of clean singleton lines
  /* CMU-ONLY-BLOCK-END */
...
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( FastVictimCache, theVictimCacheCfg, theVictimCache, SCALE_WITH_SYSTEM_WIDTH, MULTIPLY, 1);
...
// victim cache: intercept requests
WIRE(theL1D, RequestOut,                 theCMPNetController, RequestInD)
// WIRE(theL1D, RequestOut,                 theVictimCache, RequestIn)
// WIRE(theVictimCache, RequestOut,         theCMPNetController, RequestInD)
...
// victim cache: intercept snoops
WIRE(theCMPNetController, SnoopOutD,     theL1D, SnoopIn)
// WIRE(theCMPNetController, SnoopOutD,     theVictimCache, SnoopIn)
// WIRE(theVictimCache, SnoopOut,           theL1D, SnoopIn)
