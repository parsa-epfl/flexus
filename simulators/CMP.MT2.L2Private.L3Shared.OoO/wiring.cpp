...
namespace Flexus {
//Simulator Name
std::string theSimulatorName = "CMP.MT2.PrivateL2.SharedL3.OoO v1.0";
}
...
CREATE_CONFIGURATION( CMPCache, "L3", theL3Cfg );
...
bool initializeParameters() {
  ...
  theL2Cfg.EvictOnSnoop.initialize(true);
  theL2Cfg.UseReplyChannel.initialize(true);

  theL3Cfg.Cores.initialize(64);
  theL3Cfg.BlockSize.initialize(64);
  theL3Cfg.Banks.initialize(64);
  theL3Cfg.BankInterleaving.initialize(64);
  theL3Cfg.Groups.initialize(1);
  theL3Cfg.GroupInterleaving.initialize(4096);
  theL3Cfg.DirLatency.initialize(3);
  theL3Cfg.DirIssueLatency.initialize(1);
  theL3Cfg.TagLatency.initialize(3);
  theL3Cfg.TagIssueLatency.initialize(1);
  theL3Cfg.DataLatency.initialize(7);
  theL3Cfg.DataIssueLatency.initialize(1);
  theL3Cfg.QueueSize.initialize(8);
  theL3Cfg.MAFSize.initialize(32);
  theL3Cfg.DirEvictBufferSize.initialize(16);
  theL3Cfg.CacheEvictBufferSize.initialize(16);
  theL3Cfg.Policy.initialize("NonInclusiveMESI");
  theL3Cfg.DirectoryType.initialize("infinite");
  theL3Cfg.DirectoryConfig.initialize("");
  theL3Cfg.CacheLevel.initialize(eL3);
  theL3Cfg.EvictClean.initialize(false);
  theL3Cfg.ArrayConfiguration.initialize("STD:size=1048576:assoc=16:repl=LRU");
  ...
  theMTManagerCfg.FrontEndPolicy.initialize( kFE_StrictRoundRobin ) ;
...
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( IDCacheMux, theIDMuxCfg, theIDMux, SCALE_WITH_SYSTEM_WIDTH, DIVIDE, THREADS_PER_CORE);
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( Cache, theL2Cfg, theL2, SCALE_WITH_SYSTEM_WIDTH, DIVIDE, THREADS_PER_CORE);
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( CMPCache, theL3Cfg, theL3, SCALE_WITH_SYSTEM_WIDTH, DIVIDE, THREADS_PER_CORE);
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( MemoryLoopback, theMemoryCfg, theMemory, SCALE_WITH_SYSTEM_WIDTH, DIVIDE, 8);
FLEXUS_INSTANTIATE_COMPONENT_ARRAY( MultiNic1, theNicCfg, theNic, FIXED, FIXED, 12 );
...
WIRE( theL2, BackSideOut_Snoop,         theNetMapper, CacheSnoopIn        )
WIRE( theL2, BackSideOut_Reply,         theNetMapper, CacheReplyIn        )
...
WIRE( theNetMapper, DirReplyOut,        theL3, Reply_In            )
WIRE( theNetMapper, DirSnoopOut,        theL3, Snoop_In            )
WIRE( theNetMapper, DirRequestOut,      theL3, Request_In          )
WIRE( theL3, Reply_Out,          theNetMapper, DirReplyIn          )
WIRE( theL3, Snoop_Out,          theNetMapper, DirSnoopIn          )
WIRE( theL3, Request_Out,        theNetMapper, DirRequestIn        )
