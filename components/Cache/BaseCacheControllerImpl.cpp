...
BaseCacheControllerImpl *BaseCacheControllerImpl::construct(CacheController *aController,
...
BaseCacheControllerImpl::BaseCacheControllerImpl(CacheController *aController,
    ...
    : theController(aController), theSnoopBuffer(anInit->theSBSize), theInit(anInit),
      theName(anInit->theName), theBlockSize(anInit->theBlockSize), theNodeId(anInit->theNodeId),
      theProbeOnIfetchMiss(anInit->theProbeOnIfetchMiss), theReadTracker(anInit->theName),
      theCacheLevel(anInit->theCacheLevel), theDoCleanEvictions(anInit->theDoCleanEvictions),
      ...
      requests(anInit->theName + "-Requests"), hits(anInit->theName + "-Hits"),
      hits_user_I(anInit->theName + "-Hits:User:I"), hits_user_D(anInit->theName + "-Hits:User:D"),
      ...
      hitsEvict(anInit->theName + "-HitsEvictBuf"), misses(anInit->theName + "-Misses"),
      ...
      upgrades(anInit->theName + "-Upgrades"), fills(anInit->theName + "-Fills"),
      ...
      evicts_protocol(anInit->theName + "-EvictsProtocol"), snoops(anInit->theName + "-Snoops"),
      probes(anInit->theName + "-Probes"), iprobes(anInit->theName + "-Iprobes"),
...
void BaseCacheControllerImpl::loadState(std::string const &aDirName, bool aTextFlexpoint,
...
Action BaseCacheControllerImpl::handleRequestTransport(MemoryTransport transport,
                                                       bool has_maf_entry) {
  ...
  if ((msg->type() == MemoryMessage::PrefetchReadNoAllocReq ||
       msg->type() == MemoryMessage::PrefetchReadAllocReq) &&
      has_maf_entry) {
...
Action BaseCacheControllerImpl::wakeMaf(MemoryTransport transport,
...
Action BaseCacheControllerImpl::examineRequest(MemoryTransport transport, bool has_maf_entry,
                                               TransactionTracker_p aWakingTracker) {
  MemoryMessage_p msg = transport[MemoryMessageTag];
  TransactionTracker_p tracker = transport[TransactionTrackerTag];
  ...
  if (was_hit) {
    ...
    if (tracker->originatorLevel() && (*tracker->originatorLevel() == eL2Prefetcher)) {
      ...
      theTraceTracker.access(theNodeId, theCacheLevel, msg->address(), msg->pc(), was_prefetched,
                             was_write, false, msg->isPriv(),
    ...
    if (tracker && aWakingTracker) {
      ...
      if (aWakingTracker->fillLevel()) {
        if (aWakingTracker->originatorLevel() &&
            (*aWakingTracker->originatorLevel() == eL2Prefetcher)) {
