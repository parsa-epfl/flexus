...
public:
  ...
  AbstractEvictBuffer(uint32_t aSize) : theSize(aSize), theReserve(0), theCurSize(0) {
  }
...
template <typename _State> class EvictBuffer : public AbstractEvictBuffer {
  ...
  struct UpdateEvictAddr {
    UpdateEvictAddr(MemoryAddress a) : addr(a) {
    }
    void operator()(EvictEntry<_State> &e) {
      e.theBlockAddress = addr;
    }
...
public:
  ...
  EvictBuffer(uint32_t aSize) : AbstractEvictBuffer(aSize) {
  }
  ...
  virtual boost::intrusive_ptr<MemoryMessage> pop() {
    ...
    theCurSize--;
    DBG_(Trace, (<< "Evict buffer popping entry for " << retval->address()
                 << ", CurSize = " << theCurSize));
    return retval;
  }
  ...
  virtual uint32_t realSize() const {
    return theEvictions.size();
  }
...
private:
  typedef multi_index_container<
      RequestTrackerEntry,
      indexed_by<hashed_unique<member<RequestTrackerEntry, uint64_t, &RequestTrackerEntry::theSet>,
                               ULLHash>>>
      request_hash_t;
