...
public:
  ...
  virtual std::function<bool (MemoryAddress a, MemoryAddress b)> setCompareFn() const {
    //return [this](auto a, auto b){ return this->sameSet(a, b); };//std::bind(&RTArray<_BState, _DefaultBState>::sameSet, *this, _1, _2);
  ...
  bool loadState( std::istream & s, int32_t theIndex, bool aTextFlexpoint ) {
    ...
    for (int32_t set = 0; set < theNumRTSets; set++) {
      for (int32_t way = 0; way < theRTAssociativity; way++) {
        ...
        (theRVA[set].template get<by_order>()).push_back(RTEntry(theBlocksPerRegion, serial.tag, way, rstate, serial.owner));
      }
    }
    ...
    for (int32_t way = 0; way < erb_size; way++) {
      ...
      (theERB.template get<by_order>()).push_back(RTEntry(theBlocksPerRegion, serial.tag, way, rstate, serial.owner));
    }
