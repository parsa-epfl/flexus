...
// but I use these three a few times and it's getting annoying putting std all the time.
using std::list;
using std::string;
using std::pair;
  ...
  std::pair<_BState, MemoryAddress> getPreemptiveEviction() {
    ...
    for (i = 0; (i < theBlocksPerRegion) && (!found_victim || !found_valid_blocks); i++, addr += theBlockSize) {
      ...
      if (region.state[i].isValid()) {
        ...
        tag_set->get<by_way>().modify(block, ChangeBlockState(_DefaultBState));
        theERB.get<by_order>().modify(theERB.get<by_order>().begin(), InvalidateRTState(i));
...
// What if the only block is protected?
  ...
  void updateOwner( RTLookupResult & result, int32_t owner, uint64_t tagset) {
    ...
    if (!result.region_allocated || result.region->tag != rt_tag) {
      ...
      result.rt_set->get<by_tag>().modify(result.region, ChangeRegionOwner(owner));
    }
  }
  ...
  way_iterator replace_lru_block(block_set_t * set, uint64_t tag, const _BState & state, LookupResult_p v_lookup) {
    ...
    while (block->state.isValid() && block->state.isProtected()) {
      block++;
      DBG_Assert(block != set->get<by_order>().end(), ( << "No Un-protected blocks to evict"));
    }
...
// It looks like the codes working, so it's turned off to speed things up
  ...
  bool canAllocate(AbstractLookup_p abs_lookup, const MemoryAddress & anAddress) {
    ...
    if ( way < theAssociativity) {
      ...
      order_iterator block = block_set->get<by_order>().begin();
      DBG_Assert(block != block_set->get<by_order>().end());
      while (block->state.isValid() && block->state.isProtected()) {
        block++;
        if (block == block_set->get<by_order>().end()) {
          return false;
        }
      }
  ...
  AbstractLookup_p allocate(AbstractLookup_p abs_lookup, const MemoryAddress & anAddress) {
    ...
    if (entry == end) {
      ...
      DBG_Assert((int)rt_set->get<by_order>().size() <= theRTAssociativity);
    ...
    if (entry->state[offset].isValid()) {
      ...
      if (t_block != t_end) {
        ...
        way = block->way;
        block_set->get<by_way>().modify(block, ChangeBlockState(_DefaultBState));
        update_lru(block_set, block);
        ...
        if (way < theAssociativity) {
          std::pair<order_iterator, bool> result = block_set->get<by_order>().push_back(BlockEntry(block_tag, _DefaultBState, way));
          if (!result.second) {
            ...
            DBG_(Dev, ( << theName << " size: " << block_set->get<by_way>().size() ));
        ...
        } else if (!block_set->get<by_order>().front().state.isValid()) {
  ...
  void setPrefetched(RTLookupResult & result, bool val) {
    result.rt_set->get<by_tag>().modify(result.region, ChangeRTPrefetched(result.offset, val));
    result.block_set->get<by_way>().modify(result.block, ChangeBlockPrefetched(val));
  }

  void setProtected(RTLookupResult & result, bool val) {
    result.rt_set->get<by_tag>().modify(result.region, ChangeRTProtected(result.offset, val));
    result.block_set->get<by_way>().modify(result.block, ChangeBlockProtected(val));
  }
  ...
  bool loadState( std::istream & s, int32_t theIndex, bool aTextFlexpoint ) {
    ...
    DBG_Assert( set_count == theNumRTSets, ( << "Error loading flexpoints. Flexpoint containts " << set_count << " RT sets, but simulator configured for " << theNumRTSets << " RT sets." ));
    DBG_Assert( set_count == theRTAssociativity, ( << "Error loading flexpoints. Flexpoint containts " << assoc << "-way RT sets, but simulator configured for " << theRTAssociativity << "-way RT sets." ));
    ...
    DBG_Assert( set_count == theNumDataSets, ( << "Error loading flexpoints. Flexpoint containts " << set_count << " Data sets, but simulator configured for " << theNumDataSets << " Data sets." ));
    DBG_Assert( set_count == theAssociativity, ( << "Error loading flexpoints. Flexpoint containts " << assoc << "-way Data sets, but simulator configured for " << theAssociativity << "-way Data sets." ));
    ...
    for (int32_t set = 0; set < theNumDataSets; set++) {
      for (int32_t way = 0; way < theAssociativity; way++) {
        ...
        if (bstate != _DefaultBState) {
          ...
          DBG_Assert(isConsistent(bs.tag), ( << "After loading block: " << std::hex << bs.tag << " in set " << set << ", and way " << way << " RTArray is inconsistent." ));
        }
      }
    }
    return false;
