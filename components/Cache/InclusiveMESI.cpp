...
InclusiveMESI::createInstance(std::list<std::pair<std::string, std::string>>& args, const ControllerParams& params)
{
    ...
    while (!args.empty()) {
        ...
        if (!(anAlwaysNAck | aSnoopLRU | anEvictAcksRequired | a2LevelPrivate))
            DBG_Assert(
              false,
              NoDefaultOps()(<< "Unknown argument to InclusiveMESI controller: '" << args.front().first << "'"));

        args.pop_front();
    }
    DBG_Assert(args.empty(),
               NoDefaultOps()(<< "Tried to create InclusiveMESI controller with "
                                 "invalid arguments"));
...
InclusiveMESI::doRequest(MemoryTransport transport, bool has_maf_entry, TransactionTracker_p waking_tracker)
{
    bool is_write         = false;
    ...
    switch (msg->type()) {
        ...
        case MemoryMessage::NonAllocatingStoreReq:
            ...
            if (!(lookup->state() == State::Modified) && !(lookup->state() == State::Exclusive)) {
                ...
                if (!(lookup->state() == State::Modified)) { lookup->setState(State::Modified); }
        ...
        case MemoryMessage::StorePrefetchReq: {
            ...
            switch (msg->type()) {
                case MemoryMessage::RMWReq: reply_type = MemoryMessage::RMWReply; break;
                case MemoryMessage::CmpxReq: reply_type = MemoryMessage::CmpxReply; break;
                case MemoryMessage::StoreReq: reply_type = MemoryMessage::StoreReply; break;
                case MemoryMessage::StorePrefetchReq:
                    reply_type       = MemoryMessage::StorePrefetchReply;
                    is_prefetchwrite = true;
                    break;
                default: DBG_Assert(false, (<< "?!?!? What happened?")); break;
            }
...
bool
InclusiveMESI::evictBlock(LookupResult_p victim)
{
...
bool
InclusiveMESI::allocateBlock(LookupResult_p result, MemoryAddress aBlockAddress)
{
...
Action
InclusiveMESI::handleBackMessage(MemoryTransport transport)
{
    MemoryMessage_p msg          = transport[MemoryMessageTag];
    TransactionTracker_p tracker = transport[TransactionTrackerTag];
    ...
    switch (msg->type()) {
        ...
        case MemoryMessage::Invalidate:
            if (state == State::Invalid) {
                ...
                DBG_Assert(
                  !waiting_maf || original_miss->isWrite(),
                  (<< "Received invalidate for block in " << state << " state while waiting for: " << *original_miss));

                // L1-I can reply without sending the Invalidate up
                if (theCacheLevel == eL1I) {
                    if (!tracker->fillType()) { tracker->setFillType(eCoherence); }
                    if (!from_eb) {
                        result->setState(State::Invalid);
                        theArray->invalidateBlock(result);
                    } else {
        ...
        case MemoryMessage::MissNotify: {
            ...
            DBG_Assert(original_miss->isWrite(),
            ...
            if (original_miss->type() != MemoryMessage::AtomicPreloadReq) {
                ...
                if (maf_entry->outstanding_msgs == 0) {
                    ...
                    if (!maf_entry->data_received && result->state() != State::Invalid) {
                        tracker->setNetworkTrafficRequired(true);
                        tracker->setResponder(theNodeId);
                        if (!tracker->fillLevel()) { tracker->setFillLevel(theCacheLevel); }
    ...
    switch (original_miss->type()) {
        case MemoryMessage::AtomicPreloadReq: {
            ...
            switch (msg->type()) {
                ...
                case MemoryMessage::InvalidateAck:
                case MemoryMessage::InvalidateNAck:
                case MemoryMessage::MissNotify: break;
                case MemoryMessage::MissNotifyData:
                case MemoryMessage::FwdReplyOwned:
                case MemoryMessage::FwdReplyDirty:
                case MemoryMessage::InvUpdateAck:
            ...
            DBG_Assert(
              !((result->state() == State::Shared) && is_dirty),
              (<< "Shared should not recieve dirty reply " << *msg << " for original_miss " << *original_miss));
            ...
            if (!msg->ackRequired() &&
                ((msg->type() == MemoryMessage::MissReplyWritable) || (msg->type() == MemoryMessage::MissReplyDirty))) {
                maf_entry->outstanding_msgs = 0;
            }
            if ((msg->outstandingMsgs() == -1) &&
                ((msg->type() == MemoryMessage::MissReplyWritable) || (msg->type() == MemoryMessage::MissReplyDirty))) {
                maf_entry->outstanding_msgs = 0;
            }
            ...
            if (maf_entry->outstanding_msgs == 0) {
                ...
                if (result->state() == State::Shared) result->setState(State::Exclusive);
                ...
                DBG_Assert(maf_entry->data_received,
                           (<< " # of outstanding msgs after receiveing miss "
        ...
        case MemoryMessage::FetchReq: {
            ...
            switch (msg->type()) {
                ...
                case MemoryMessage::FwdReply:
                case MemoryMessage::MissReply:
                case MemoryMessage::FwdReplyOwned: result->setState(State::Shared); break;
            ...
            if (msg->ackRequired()) {
                ...
                if (msg->type() == MemoryMessage::FwdReplyOwned) { reply->type() = MemoryMessage::FetchAckDirty; }
        ...
        case MemoryMessage::LoadReq: {
            ...
            switch (msg->type()) {
                case MemoryMessage::FwdReplyOwned: dirty_data = true;
                case MemoryMessage::MissReply:
                case MemoryMessage::FwdReply: result->setState(State::Shared); break;
                case MemoryMessage::MissReplyWritable: result->setState(State::Exclusive); break;
                case MemoryMessage::MissReplyDirty: result->setState(State::Modified); break;
            ...
            if (msg->ackRequired()) {
                ...
                if (dirty_data) { reply->type() = MemoryMessage::ReadAckDirty; }
        ...
        case MemoryMessage::WriteReq: {
            ...
            if (maf_entry->outstanding_msgs == 0) {
                ...
                if (original_miss->type() != MemoryMessage::WriteReq) {
                    switch (original_miss->type()) {
                        case MemoryMessage::StorePrefetchReq: msg->type() = MemoryMessage::StorePrefetchReply; break;
                        case MemoryMessage::StoreReq: msg->type() = MemoryMessage::StoreReply; break;
                        case MemoryMessage::CmpxReq: msg->type() = MemoryMessage::CmpxReply; break;
                        case MemoryMessage::RMWReq: msg->type() = MemoryMessage::RMWReply; break;
                        default: DBG_Assert(false, (<< "What??")); break;
                    }
                    DBG_Assert(theCacheLevel == eL1);
                    result->setState(State::Modified);
                    DBG_(
                      Trace,
                      (<< theName << " received reply to Store/Atomic setting size to " << original_miss->reqSize()));
                    msg->reqSize() = original_miss->reqSize();
                    msg->address() = original_miss->address();
                    msg->pc()      = original_miss->pc();
                    msg->coreIdx() = original_miss->coreIdx();
...
Action
InclusiveMESI::finalizeSnoop(MemoryTransport transport, LookupResult_p result)
{
    ...
    switch (orig_msg->type()) {
        ...
        case MemoryMessage::FetchFwd:
            if ((block_state == State::Modified)) {
                ...
                if (theSnoopLRU && result->state() != State::Invalid) { theArray->invalidateBlock(result); }
            } else {
    ...
    if (wake_evicts) { action.theWakeEvicts = true; }
...
// TransactionTracker_p tracker) {
Action
InclusiveMESI::handleSnoopMessage(MemoryTransport transport)
{
...
// TransactionTracker_p tracker) {
Action
InclusiveMESI::handleIprobe(bool aHit, MemoryTransport transport)
{
...
bool
InclusiveMESI::idleWorkAvailable()
{
...
MemoryMessage_p
InclusiveMESI::getIdleWorkMessage(ProcessEntry_p process)
{
...
void
InclusiveMESI::removeIdleWorkReservations(ProcessEntry_p process, Action& action)
{
...
// TransactionTracker_p tracker) {
Action
InclusiveMESI::handleIdleWork(MemoryTransport transport)
{
...
//                TransactionTracker_p tracker ) {
Action
InclusiveMESI::handleWakeSnoop(MemoryTransport transport)
{
...
uint32_t
InclusiveMESI::freeEvictBuffer() const
{
...
bool
InclusiveMESI::evictableBlockExists(int32_t anIndex) const
{
...
void
InclusiveMESI::dumpEvictBuffer() const
{
