...
COMPONENT_PARAMETERS(
  ...
  //ALEX
  PARAMETER( isRMCCache, bool, "Specifies whether it's a normal or an RMC cache", "isRMCCache", false )
  PARAMETER( hasStreamBuffers, bool, "Stream buffers for RRPPs", "hasStreamBuffers", false )
);
...
COMPONENT_INTERFACE(
  ...
  //ALEX - for magic connection of caches with RMCs
  PORT( PushInput, MemoryTransport, MagicRMCAccess )
  PORT( PushOutput, bool, MagicRMCAccessReply )

  //ALEX - ports for RRPP Stream Buffers
  PORT( PushInput, SABReEntry, AllocateStrBuf )          //For RMC to check for StreamBuffer availability
  PORT( PushInput, uint32_t, FreeStrBuf )
  PORT( PushInput, uint32_t, AllocateStrBufEntry ) //For RMC to allocate an entry in a particular StreamBuffer (using an ATT index)
  PORT( PushOutput, bool, StrBufEntryAvail )        //For replies to the requests that arrive through the previous port
  PORT( PushOutput, uint32_t, SABReInval )          //Notify the RMC about an invalidation msg received for a SB entry (message contains corresponding ATT index)
