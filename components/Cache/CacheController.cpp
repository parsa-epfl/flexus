...
{
    s << process.type();
    if (process.transport()[MemoryMessageTag]) { s << ": OrigMessage = " << *(process.transport()[MemoryMessageTag]); }
    s << " Reservations: ";
    if (process.getReservations() & kResFrontSideOut) { s << "FrontSideOut |"; }
    if (process.getReservations() & kResBackSideOut_Request) { s << "BackSideOut_Request |"; }
    if (process.getReservations() & kResBackSideOut_Snoop) { s << "BackSideOut_Snoop |"; }
    if (process.getReservations() & kResBackSideOut_Reply) { s << "BackSideOut_Reply |"; }
    if (process.getReservations() & kResBackSideOut_Prefetch) { s << "BackSideOut_Prefetch |"; }
    if (process.getReservations() & kResEvictBuffer) { s << "EvictBuffer |"; }
    if (process.getReservations() & kResSnoopBuffer) { s << "SnoopBuffer |"; }
    if (process.getReservations() & kResScheduledEvict) { s << "ScheduledEvict |"; }
    if (process.getReservations() & kResMaf) { s << "MAF |"; }
    if (process.tagOutstanding()) { s << " TagOutstanding"; }
    if (process.requiresData()) { s << " requiresData=" << process.requiresData(); }
    if (process.removeMafEntry()) { s << " removeMafEntry"; }
    if (process.hasFrontMessage()) { s << " hasFrontMessage"; }
    if (process.hasBackMessage()) { s << " hasBackMessage"; }
    return s;
}
...
const bool
CacheController::isFrontSideOutEmpty_D(void) const
{
    for (int32_t i = 0; i < theCores; i++)
        if (!FrontSideOut_D[i].empty()) return false;
    return true;
}

const bool
CacheController::isFrontSideOutEmpty_I(void) const
{
    for (int32_t i = 0; i < theCores; i++)
        if (!FrontSideOut_I[i].empty()) return false;
    return true;
}

const bool
CacheController::isFrontSideOutFull(void) const
{
    for (int32_t i = 0; i < theCores; i++)
...
const bool
CacheController::isPipelineSetEmpty(const std::vector<Pipeline>& pipelines) const
{
    for (int32_t i = 0; i < theBanks; i++) {
        if (!pipelines[i].empty()) return false;
    }
    return true;
}
...
const bool
CacheController::isWakeMAFListEmpty() const
{
    for (int32_t i = 0; i < theBanks; i++) {
        if (!theWakeMAFList[i].empty()) return false;
    }
    return true;
}

const bool
CacheController::isIProbeListEmpty() const
{
    for (int32_t i = 0; i < theBanks; i++) {
        if (!theIProbeList[i].empty()) return false;
    }
    return true;
}

const uint32_t
CacheController::getBank(const ProcessEntry_p entry) const
{
    return getBank(addressOf(entry));
}

const uint32_t
CacheController::getBank(const MemoryTransport trans) const
{
    return getBank(trans[MemoryMessageTag]->address());
}

const uint32_t
CacheController::getBank(const boost::intrusive_ptr<MemoryMessage> msg) const
{
    return getBank(msg->address());
}

const uint32_t
CacheController::getBank(const MemoryAddress address) const
{
...
uint32_t
CacheController::totalPipelineSize(std::vector<Pipeline>& pipe) const
{
    uint32_t size = 0;
...
bool
CacheController::isQuiesced() const
{
    return theMaf.empty() && isPipelineSetEmpty(theMAFPipeline) && isPipelineSetEmpty(theTagPipeline) &&
           isPipelineSetEmpty(theDataPipeline) && isWakeMAFListEmpty() && isIProbeListEmpty() &&
           isQueueSetEmpty(FrontSideIn_Snoop) && isQueueSetEmpty(FrontSideIn_Request) &&
           isQueueSetEmpty(FrontSideIn_Prefetch) && BackSideIn_Reply[0].empty() && BackSideIn_Request[0].empty() &&
           isFrontSideOutEmpty_I() && isFrontSideOutEmpty_D() && BackSideOut_Reply.empty() &&
           BackSideOut_Snoop.empty() && BackSideOut_Request.empty() && BackSideOut_Prefetch.empty() &&
  ...
  , BackSideOut_Prefetch(aQueueSize)
{
    ...
    boost::intrusive_ptr<Stat::StatLog2Histogram> mafHist  = new Stat::StatLog2Histogram(aName + "-MafServer" +
                                                                                        "-InterArrivalTimes"),
                                                  tagHist  = new Stat::StatLog2Histogram(aName + "-TagServer" +
                                                                                        "-InterArrivalTimes"),
                                                  dataHist = new Stat::StatLog2Histogram(aName + "-DataServer" +
                                                                                         "-InterArrivalTimes");
    ...
    for (int32_t i = 0; i < theBanks; i++) {
        ...
        theDataPipeline.push_back(Pipeline(aName + "-DataServer", aPorts, aDataIssueLatency, aDataLatency, dataHist));
...
bool
CacheController::hasMAF(const MemoryAddress& aBlockAddress) const
{
    return theMaf.contains(aBlockAddress);
}

void
CacheController::reserveFrontSideOut(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveBackSideOut_Request(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveBackSideOut_Evict(ProcessEntry_p aProcess)
{
    if (theEvictOnSnoop) {
...
void
CacheController::reserveBackSideOut_Snoop(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveBackSideOut_Prefetch(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveEvictBuffer(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveSnoopBuffer(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveScheduledEvict(ProcessEntry_p aProcess)
{
...
void
CacheController::reserveMAF(ProcessEntry_p aProcess)
{
...
void
CacheController::unreserveFrontSideOut(ProcessEntry_p aProcess)
{
...
void
CacheController::unreserveBackSideOut_Request(ProcessEntry_p aProcess)
{
...
void
CacheController::unreserveBackSideOut_Evict(ProcessEntry_p aProcess)
{
    if (theEvictOnSnoop) {
...
void
CacheController::unreserveBackSideOut_Snoop(ProcessEntry_p aProcess)
{
    if (theUseReplyChannel) {
...
void
CacheController::unreserveBackSideOut_Prefetch(ProcessEntry_p aProcess)
{
...
void
CacheController::unreserveEvictBuffer(ProcessEntry_p aProcess)
{
...
void
CacheController::clearEvictBufferReservation(ProcessEntry_p aProcess)
{
    aProcess->unreserve(kResEvictBuffer);
}

void
CacheController::unreserveSnoopBuffer(ProcessEntry_p aProcess)
{
...
void
CacheController::unreserveScheduledEvict(ProcessEntry_p aProcess)
{
...
void
CacheController::unreserveMAF(ProcessEntry_p aProcess)
{
...
CacheController::unreserveBSO(ProcessEntry_p aProcess)
{
    switch (aProcess->type()) {
        ...
        case eProcEviction: unreserveBackSideOut_Evict(aProcess); break;
        case eProcRequest:
        case eProcIProbe:
        case eProcMAFWakeup: unreserveBackSideOut_Request(aProcess); break;
        default: DBG_Assert(false, (<< " Cannot unreserve resources for processes of type: " << aProcess->type()));
    }
}

// Enqueue requests in the appropriate tag pipelines
void
CacheController::enqueueTagPipeline(Action action, ProcessEntry_p aProcess)
{
    DBG_Assert(action.theRequiresTag > 0);

    if (action.theRequiresTag) { theTagPipeline[getBank(aProcess)].enqueue(aProcess, action.theRequiresTag); }
}
...
// hits
void
CacheController::processMessages()
{
    FLEXUS_PROFILE();
    ...
    for (int32_t i = 0; i < theBanks; i++) {
        while (theMAFPipeline[i].ready() && theTagPipeline[i].serverAvail()) {
            ...
            switch (process->type()) {
                case eProcRequest:
                case eProcPrefetch: runRequestProcess(process); break;
                case eProcSnoop: runSnoopProcess(process); break;
                case eProcBackRequest:
                case eProcBackReply: runBackProcess(process); break;
                case eProcMAFWakeup: runWakeMafProcess(process); break;
                case eProcIProbe: runIProbeProcess(process); break;
                case eProcEviction: runEvictProcess(process); break;
                case eProcIdleWork: runIdleWorkProcess(process); break;
                case eProcWakeSnoop: runWakeSnoopProcess(process); break;
...
void
CacheController::advancePipelines(void)
{
    FLEXUS_PROFILE();
    int32_t i, bankCount;
        ...
        while (theTagPipeline[i].ready()) {
            ...
            if (process->requiresData()) {
                if (theDataPipeline[i].serverAvail()) {
                    if (process->transmitAfterTag()) { doTransmitProcess(process); }
...
void
CacheController::scheduleNewProcesses()
{
    FLEXUS_PROFILE();

    int32_t i, bankCount;

    for (i = theLastScheduledBank, bankCount = 0; bankCount < theBanks; i = (i + 1) % theBanks, bankCount++) {
        ...
        while (theMAFPipeline[i].serverAvail() && !theWakeMAFList[i].empty() && !BackSideOut_Request.full() &&
               !isFrontSideOutFull() && !theCacheControllerImpl->fullEvictBuffer() &&
        ...
        // BackSideOut_Request.
        while (theMAFPipeline[i].serverAvail() && !theIProbeList[i].empty() && !BackSideOut_Request.full() &&
               !isFrontSideOutFull()) {
        ...
        while (theMAFPipeline[i].serverAvail() && theCacheControllerImpl->hasWakingSnoops() && !isFrontSideOutFull() &&
        ...
        // We don't allocate an evict buffer entry, but instead relying on having
        // one from an earlier request
        while (theMAFPipeline[i].serverAvail() && !BankBackSideIn_Request[i].empty() && !isFrontSideOutFull() &&
               !(theUseReplyChannel ? BackSideOut_Reply.full() : BackSideOut_Snoop.full()) &&
        ...
        while (
          theMAFPipeline[i].serverAvail() && !BankFrontSideIn_Request[i].empty() && !BackSideOut_Request.full() &&
          !isFrontSideOutFull() && !theMaf.full() && !theCacheControllerImpl->fullEvictBuffer() &&
        ...
        if (!scheduled && !BankFrontSideIn_Request[i].empty()) {
            DBG_(VVerb,
                 (<< "Failed to schedule Request: BSO_Req full " << std::boolalpha << BackSideOut_Request.full()
                  ...
                  << !(BankFrontSideIn_Snoop[i].empty() ||
                       BankFrontSideIn_Snoop[i].headTimestamp() > BankFrontSideIn_Request[i].headTimestamp())
                  << " canStartRequest(" << std::hex << BankFrontSideIn_Request[i].peek()[MemoryMessageTag]->address()
                  << ") = "
        ...
        if (!scheduled && !BankFrontSideIn_Snoop[i].empty()) {
            DBG_(
              Trace,
              (<< "Failed to schedule FS Snoop: BSO_Snoop full " << std::boolalpha << BackSideOut_Snoop.full()
               << ", Snoop Resources Avail: "
               << theCacheControllerImpl->snoopResourcesAvailable(BankFrontSideIn_Snoop[i].peek()[MemoryMessageTag])));
        }
...
void
CacheController::doNewRequests(std::vector<MessageQueue<MemoryTransport>>& aMessageQueue,
                               ...
                               int32_t& lastQueue)
{
    ...
    while (sentMessages) {
        ...
        for (i = lastQueue, queueCount = 0; queueCount < numMsgQueues; queueCount++, i = (i + 1) % numMsgQueues) {
...
void
CacheController::runIdleWorkProcess(ProcessEntry_p aProcess)
{
    ...
    switch (action.theAction) {
        case kNoAction:
            if (aProcess->requiresTag()) { enqueueTagPipeline(action, aProcess); }
            break;

        case kSend:
            if (action.theFrontMessage) { aProcess->enqueueFrontTransport(action); }
            if (action.theBackMessage) { aProcess->enqueueBackTransport(action); }
        ...
        default: DBG_Assert(false, (<< "Unsupported Action type for IdleWorkProcess")); break;
    };
}
...
CacheController::runEvictProcess(ProcessEntry_p aProcess)
{
    ...
    if (action.theAction == kNoAction) { return; }
...
void
CacheController::runRequestProcess(ProcessEntry_p aProcess)
{
    FLEXUS_PROFILE();
...
void
CacheController::runWakeMafProcess(ProcessEntry_p aProcess)
{
    FLEXUS_PROFILE();
    DBG_Assert(aProcess->type() == eProcMAFWakeup);
    DBG_(
      Trace,
      (<< " runWakeMafProcess " << aProcess->serial() << ": " << *aProcess->mafEntry()->transport[MemoryMessageTag]));
...
CacheController::runSnoopProcess(ProcessEntry_p aProcess)
{
    ...
    if (temp != theMaf.end() && aProcess->transport()[MemoryMessageTag]->isProbeType()) {
        ...
        switch (aProcess->transport()[MemoryMessageTag]->type()) {
            case MemoryMessage::ProbedNotPresent: theMaf.modifyState(temp, kProbeMiss); break;
            case MemoryMessage::ProbedClean:
            case MemoryMessage::ProbedWritable:
            case MemoryMessage::ProbedDirty: theMaf.modifyState(temp, kProbeHit); break;
            default: DBG_Assert(false);
        }
...
CacheController::runIProbeProcess(ProcessEntry_p aProcess)
{
    ...
    Action action =
      theCacheControllerImpl->handleIprobe(aProcess->mafEntry()->state == kProbeHit, aProcess->mafEntry()->transport);
...
CacheController::runBackProcess(ProcessEntry_p aProcess)
{
    ...
    if (aProcess->type() == eProcBackRequest) { unreserveSnoopBuffer(aProcess); }
...
MemoryAddress const
CacheController::addressOf(ProcessEntry_p aProcess) const
{
...
CacheController::trackerOf(ProcessEntry_p aProcess) const
{
...
CacheController::doTransmitProcess(ProcessEntry_p aProcess)
{
    ...
    if (aProcess->hasBackMessage()) {
        ...
        switch (aProcess->type()) {
            ...
            default: DBG_Assert(false, (<< " For transmit to back, invalid process type: " << aProcess->type()));
        }
    }
...
CacheController::sendFront(MemoryTransport& transport, bool to_D, bool to_I)
{
    ...
    DBG_Assert(transport[MemoryMessageTag]->coreIdx() >= 0 && transport[MemoryMessageTag]->coreIdx() < theCores);
    ...
    if (to_D) { FrontSideOut_D[transport[MemoryMessageTag]->coreIdx()].enqueue(transport); }
    if (to_I) { FrontSideOut_I[transport[MemoryMessageTag]->coreIdx()].enqueue(i_transport); }
}
