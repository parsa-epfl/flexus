...
struct Action {
  ...
  Action(enumAction anAction = kNoAction, int32_t aRequiresData = 0)
      : theAction(anAction), theFrontMessage(false), theBackMessage(false), theFrontToDCache(true),
        theFrontToICache(false), theRequiresData(aRequiresData), theRequiresTag(1),
        theWakeSnoops(false), theWakeEvicts(false), theWakeRegion(false),
        theRememberSnoopTransport(true) {
  }

  Action(enumAction anAction, TransactionTracker_p aTracker, int32_t aRequiresData = 0)
      : theAction(anAction), theOutputTracker(aTracker), theFrontMessage(false),
        theBackMessage(false), theFrontToDCache(true), theFrontToICache(false),
        theRequiresData(aRequiresData), theRequiresTag(1), theWakeSnoops(false),
        theWakeEvicts(false), theWakeRegion(false), theRememberSnoopTransport(true) {
  }
};
...
class CacheInitInfo {
public:
  CacheInitInfo(std::string aName, int32_t aCores, std::string const &anArrayConfiguration,
                int32_t aBlockSize, int32_t aNodeId, tFillLevel aCacheLevel, int32_t anEBSize,
                int32_t anSBSize, bool aProbeOnIfetchMiss, bool aDoCleanEvictions,
                bool aWritableEvictsHaveData, bool anAllowOffChipStreamFetch)
      : theName(aName), theCores(aCores), theArrayConfiguration(anArrayConfiguration),
        theBlockSize(aBlockSize), theNodeId(aNodeId), theCacheLevel(aCacheLevel),
        theEBSize(anEBSize), theSBSize(anSBSize), theProbeOnIfetchMiss(aProbeOnIfetchMiss),
        theDoCleanEvictions(aDoCleanEvictions), theWritableEvictsHaveData(aWritableEvictsHaveData),
...
public:
  static BaseCacheControllerImpl *construct(CacheController *aController, CacheInitInfo *anInfo,
  ...
  virtual void setProtectedPublic(MemoryAddress addr, bool flag) {
    setProtectedBlock(addr, flag);
  }
  ...
  virtual void dumpEvictBuffer() const {
    ...
    uint32_t er_free = arrayEvictResourcesFree();
    DBG_(VVerb, (<< theName << " EB Free/Reserved/Used " << eb_free << "/" << eb_reserved << "/"
                 << eb_used << ", ERB free " << er_free));
  }
  ...
  virtual void dumpSnoopBuffer() const {
    ...
    DBG_(VVerb, (<< theName << " SnoopBuffer Size/Reserve/Free = " << sb_size << "/" << sb_reserve
                 << "/" << sb_free));
  }
...
protected:
  ...
  virtual Action examineRequest(MemoryTransport transport, bool has_maf_entry,
...
public:
  ...
  virtual void wakeWaitingSnoops(MemoryAddress addr) {
    theSnoopBuffer.wakeWaitingEntries(addr);
  }
