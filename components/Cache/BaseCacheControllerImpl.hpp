...
//ALEX
public:
  virtual bool myStupidFunction(MemoryTransport       transport);
...
public:
  ...
  //ALEX
  virtual bool myInternalStupidFunction(MemoryTransport       transport) = 0;
  virtual bool streamBufferAvailable() = 0;
  virtual void allocateStreamBuffer(uint32_t anATTidx, uint64_t anAddress, uint32_t aLength) = 0;
  virtual void freeStreamBuffer(uint32_t anATTidx) = 0;
  virtual bool allocateStreamBufferSlot(uint32_t anATTidx) = 0;
  virtual std::list<uint32_t> SABReAtomicityCheck() = 0;
