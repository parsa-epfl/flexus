...
InclusiveMOESI::createInstance(std::list<std::pair<std::string, std::string>> &args,
  ...
  DBG_Assert(args.empty(), NoDefaultOps()(<< "Tried to create InclusiveMOESI controller with "
                                             "invalid arguments"));
  return new InclusiveMOESI(params.first, params.second, anAlwaysNAck, aSnoopLRU,
                            aMissNotifyRequired, aEvictAcksRequired);
}

InclusiveMOESI::InclusiveMOESI(CacheController *aController, CacheInitInfo *anInit,
                               bool aNAckAlways, bool aSnoopLRU, bool aMissNotifyRequired,
    ...
    : BaseCacheControllerImpl(aController, anInit), theEvictBuffer(anInit->theEBSize),
      ...
      theMissNotifyRequired(aMissNotifyRequired), theEvictAcksRequired(aEvictAcksRequired),
...
Action InclusiveMOESI::handleBackMessage(MemoryTransport transport) {
  ...
  case MemoryMessage::FetchReq: {
    ...
    if (msg->ackRequired()) {
      intrusive_ptr<MemoryMessage> reply(
          new MemoryMessage(MemoryMessage::FetchAck, msg->address()));
      action.theBackMessage = true;
      action.theBackTransport = transport;
      action.theBackTransport.set(MemoryMessageTag, reply);
    }
