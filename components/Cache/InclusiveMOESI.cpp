...
InclusiveMOESI::createInstance(std::list<std::pair<std::string, std::string>>& args, const ControllerParams& params)
{
    ...
    DBG_Assert(args.empty(),
               NoDefaultOps()(<< "Tried to create InclusiveMOESI controller with "
                                 "invalid arguments"));
    return new InclusiveMOESI(params.first,
                              params.second,
                              anAlwaysNAck,
                              aSnoopLRU,
                              aMissNotifyRequired,
...
InclusiveMOESI::doRequest(MemoryTransport transport, bool has_maf_entry, TransactionTracker_p waking_tracker)
{
    bool is_write       = false;
    ...
    switch (msg->type()) {
        ...
        case MemoryMessage::NonAllocatingStoreReq:
            ...
            if (!(lookup->state() == State::Modified) && !(lookup->state() == State::Exclusive)) {
                ...
                // Need to track this in the MAF so we don't send multiple requests to mem
                // at the same time
...
bool
InclusiveMOESI::evictBlock(LookupResult_p victim)
{
...
void
InclusiveMOESI::allocateBlock(LookupResult_p result, MemoryAddress aBlockAddress)
{
...
// Action InclusiveMOESI::handleBackMessage(MemoryMessage_p msg,
// TransactionTracker_p tracker) {
Action
InclusiveMOESI::handleBackMessage(MemoryTransport transport)
{
    MemoryMessage_p msg          = transport[MemoryMessageTag];
    TransactionTracker_p tracker = transport[TransactionTrackerTag];
    ...
    switch (msg->type()) {
        ...
        case MemoryMessage::MissNotify: {
            DBG_Assert(theMissNotifyRequired,
                       (<< "Received MissNotify but protocol configured not to require "
                           "them, this will probably cause errors."));
...
// Action InclusiveMOESI::finalizeSnoop(MemoryMessage_p msg,
// TransactionTracker_p tracker, LookupResult_p result) {
Action
InclusiveMOESI::finalizeSnoop(MemoryTransport transport, LookupResult_p result)
{
...
// Action InclusiveMOESI::handleSnoopMessage(MemoryMessage_p msg,
// TransactionTracker_p tracker) {
Action
InclusiveMOESI::handleSnoopMessage(MemoryTransport transport)
{
...
// TransactionTracker_p tracker) {
Action
InclusiveMOESI::handleIprobe(bool aHit, MemoryTransport transport)
{
...
bool
InclusiveMOESI::idleWorkAvailable()
{
...
MemoryMessage_p
InclusiveMOESI::getIdleWorkMessage(ProcessEntry_p process)
{
...
void
InclusiveMOESI::removeIdleWorkReservations(ProcessEntry_p process, Action& action)
{
...
// Action InclusiveMOESI::handleIdleWork(MemoryMessage_p msg,
// TransactionTracker_p tracker) {
Action
InclusiveMOESI::handleIdleWork(MemoryTransport transport)
{
...
//                TransactionTracker_p tracker ) {
Action
InclusiveMOESI::handleWakeSnoop(MemoryTransport transport)
{
...
uint32_t
InclusiveMOESI::freeEvictBuffer() const
{
...
bool
InclusiveMOESI::evictableBlockExists(int32_t anIndex) const
{
