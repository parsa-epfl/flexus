...
// and the init info.
BaseCacheControllerImpl *TaglessInclusiveMOESIControllerImpl::createInstance(
    std::list<std::pair<std::string, std::string>> &args, const ControllerParams &params) {
  ...
  while (!args.empty()) {
    if (strcasecmp(args.front().first.c_str(), "AlwaysNAck") == 0) {
      ...
      DBG_Assert(false, NoDefaultOps()(<< "Unknown argument to InclusiveMOESI controller: '"
                                       << args.front().first << "'"));
    }
    args.pop_front();
  }
  DBG_Assert(args.empty(), NoDefaultOps()(<< "Tried to create InclusiveMOESI controller with "
                                             "invalid arguments"));
  return new TaglessInclusiveMOESIControllerImpl(params.first, params.second, anAlwaysNAck);
}

TaglessInclusiveMOESIControllerImpl::TaglessInclusiveMOESIControllerImpl(
    CacheController *aController, CacheInitInfo *anInit, bool aNAckAlways)
    : BaseCacheControllerImpl(aController, anInit), theEvictBuffer(anInit->theEBSize),
      theNAckAlways(aNAckAlways), thePendingEvicts(0), theEvictThreshold(4) {
  DBG_Assert(anInit->theCores == 1, (<< "InclusiveMOESI cache does not support cfg.Cores == "
                                     << anInit->theCores << " (only supports 1 right now!)"));
...
void TaglessInclusiveMOESIControllerImpl::allocateBlock(LookupResult_p result,
...
Action TaglessInclusiveMOESIControllerImpl::handleBackMessage(MemoryTransport transport) {
    ...
    if (state == State::Invalid) {
      ...
      if (!theSnoopBuffer.hasEntry(msg->address())) {
        ...
        intrusive_ptr<MemoryMessage> request(
            new MemoryMessage(MemoryMessage::Downgrade, msg->address()));
  ...
  case MemoryMessage::Invalidate:
    if (state == State::Invalid) {
      ...
      // exclusive or not
      DBG_Assert(!waiting_maf, (<< "Received Invalidate while in Exlcusive state and waiting for: "
                                << (*original_miss)));
      ...
      DBG_Assert(!waiting_maf || original_miss->type() == MemoryMessage::WriteReq,
  ...
  case MemoryMessage::MissNotify: {
    ...
    if (maf_entry->outstanding_msgs == 0) {
      ...
      if (!maf_entry->data_received) {
        Action act(kSend, tracker, false);
        intrusive_ptr<MemoryMessage> retry(
            new MemoryMessage(MemoryMessage::WriteRetry, msg->address()));
        act.theBackMessage = true;
        act.theBackTransport = transport;
      ...
      intrusive_ptr<MemoryMessage> reply(
          new MemoryMessage(MemoryMessage::WriteAck, msg->address()));
      if (sent_upgrade) {
  ...
  case MemoryMessage::UpgradeReq: {
    ...
    if (maf_entry->outstanding_msgs == 0) {
      ...
      is_final = true;
      intrusive_ptr<MemoryMessage> reply(
          new MemoryMessage(MemoryMessage::UpgradeAck, msg->address()));
      action.theBackMessage = true;
      action.theBackTransport = transport;
      action.theBackTransport.set(MemoryMessageTag, reply);
      action.theBackTransport.set(TaglessDirMsgTag, maf_entry->transport[TaglessDirMsgTag]);
...
Action TaglessInclusiveMOESIControllerImpl::finalizeSnoop(MemoryTransport transport,
                                                          LookupResult_p result) {
  ...
  if (requires_tagless_msg) {
    ...
    for (; eb_iter != theEvictBuffer.end(); eb_iter++) {
      if (theArray->sameSet(msg->address(), eb_iter->address())) {
        std::set<int> my_buckets =
            nGlobalHasher::GlobalHasher::theHasher().hashAddr(eb_iter->address());
        conflicting_buckets.insert(my_buckets.begin(), my_buckets.end());
      }
    }
    ...
    std::vector<int>::iterator last_it =
        std::set_difference(buckets.begin(), buckets.end(), conflicting_buckets.begin(),
                            conflicting_buckets.end(), conflict_free.begin());
...
void TaglessInclusiveMOESIControllerImpl::removeIdleWorkReservations(ProcessEntry_p process,
                                                                     Action &action) {
