...
BaseCacheControllerImpl * TaglessInclusiveMOESIControllerImpl::createInstance(std::list<std::pair<std::string, std::string> > &args, const ControllerParams & params) {
...
std::tuple<bool, bool, Action> TaglessInclusiveMOESIControllerImpl::doRequest( MemoryTransport      transport,
  ...
  void TaglessInclusiveMOESIControllerImpl::allocateBlock(LookupResult_p result, MemoryAddress aBlockAddress) {
  ...
  // Action TaglessInclusiveMOESIControllerImpl::handleBackMessage(MemoryMessage_p msg, TransactionTracker_p tracker) {
  Action TaglessInclusiveMOESIControllerImpl::handleBackMessage(MemoryTransport transport) {
    MemoryMessage_p msg = transport[MemoryMessageTag];
    TransactionTracker_p tracker = transport[TransactionTrackerTag];
    ...
    switch (msg->type()) {
      ...
      case MemoryMessage::MissNotify: {
        ...
        if (maf_entry->outstanding_msgs == 0) {
          ...
          // If we didn't receive any data, then we need to send a retry to the directory
          // The directory will then forward that to memory
  ...
  //Action TaglessInclusiveMOESIControllerImpl::finalizeSnoop(MemoryMessage_p msg, TransactionTracker_p tracker, LookupResult_p result) {
  Action TaglessInclusiveMOESIControllerImpl::finalizeSnoop(MemoryTransport transport, LookupResult_p result) {
  ...
  //Action TaglessInclusiveMOESIControllerImpl::handleSnoopMessage(MemoryMessage_p msg, TransactionTracker_p tracker) {
  Action TaglessInclusiveMOESIControllerImpl::handleSnoopMessage(MemoryTransport transport) {
  ...
  // at this cache level.
  //Action TaglessInclusiveMOESIControllerImpl::handleIprobe(bool aHit, MemoryMessage_p fetchReq, TransactionTracker_p tracker) {
  ...
  uint32_t TaglessInclusiveMOESIControllerImpl::freeEvictBuffer() const {
  ...
  bool TaglessInclusiveMOESIControllerImpl::evictableBlockExists(int32_t anIndex) const {
  ...
  Action TaglessInclusiveMOESIControllerImpl::doEviction() {
    Action action(kSend);
  ...
  bool TaglessInclusiveMOESIControllerImpl::idleWorkAvailable() {
  ...
  MemoryMessage_p TaglessInclusiveMOESIControllerImpl::getIdleWorkMessage(ProcessEntry_p process) {
  ...
  void TaglessInclusiveMOESIControllerImpl::removeIdleWorkReservations(ProcessEntry_p process, Action & action) {
  ...
  //Action TaglessInclusiveMOESIControllerImpl::handleIdleWork(MemoryMessage_p msg, TransactionTracker_p tracker) {
  Action TaglessInclusiveMOESIControllerImpl::handleIdleWork(MemoryTransport transport) {
  ...
  //Action TaglessInclusiveMOESIControllerImpl::handleWakeSnoop ( MemoryMessage_p      msg,
  //                TransactionTracker_p tracker ) {
  Action TaglessInclusiveMOESIControllerImpl::handleWakeSnoop ( MemoryTransport transport ) {
  ...
  void TaglessInclusiveMOESIControllerImpl::dumpEvictBuffer() const {
  ...
  //ALEX
  bool TaglessInclusiveMOESIControllerImpl::myInternalStupidFunction(MemoryTransport       transport) {
    MemoryMessage_p msg = transport[MemoryMessageTag];
  ...
  bool TaglessInclusiveMOESIControllerImpl::streamBufferAvailable() {
    DBG_Assert(false);
    return false;
  }

  void TaglessInclusiveMOESIControllerImpl::allocateStreamBuffer(uint32_t anATTidx, uint64_t anAddress, uint32_t aLength) {
    DBG_Assert(false);
  }

  void TaglessInclusiveMOESIControllerImpl::freeStreamBuffer(uint32_t anATTidx) {
    DBG_Assert(false);
  }

  bool TaglessInclusiveMOESIControllerImpl::allocateStreamBufferSlot(uint32_t anATTidx) {
    DBG_Assert(false);
    return false;
  }

  std::list<uint32_t> TaglessInclusiveMOESIControllerImpl::SABReAtomicityCheck() {
    DBG_Assert(false);
