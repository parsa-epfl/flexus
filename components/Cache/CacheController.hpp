...
struct MafEntry {
  ...
  MafEntry(MemoryAddress aBlockAddress, Transport &aTransport, MafStates aState, eEntryType aType)
      : theBlockAddress(aBlockAddress), state(aState), transport(aTransport), type(aType),
        outstanding_msgs(0), data_received(false) {
  }
};
...
class MissAddressFile {
  ...
  typedef multi_index_container<
      MafEntry, indexed_by<ordered_non_unique<composite_key<
                    MafEntry, member<MafEntry, MemoryAddress, &MafEntry::theBlockAddress>,
...
public:
  ...
  MissAddressFile(std::string const &aStatName, uint32_t aSize, uint32_t aMaxTargetsPerRequest)
      : theSize(aSize), theMaxTargetsPerRequest(aMaxTargetsPerRequest), theWaitResponseEntries(0),
        theLastAccounting(0), theMaxMAFTargets(aStatName + "-MaxMAFTargets"),
        theMaxMAFMisses(aStatName + "-MaxMAFMisses"), theName(aStatName), theReserve(0) {
    ...
    theCyclesWith[kAtomic] =
        new Stat::StatInstanceCounter<int64_t>(aStatName + "-MAFEntries:Atomic");
    ...
    theCyclesWith[kReadPrefetch] =
        new Stat::StatInstanceCounter<int64_t>(aStatName + "-MAFEntries:ReadPrefetch");
    theAverage[kReadPrefetch] = new Stat::StatAverage(aStatName + "-MAFAvg:ReadPrefetch");
    theCyclesWith[kWritePrefetch] =
        new Stat::StatInstanceCounter<int64_t>(aStatName + "-MAFEntries:WritePrefetch");
  ...
  void allocEntry(MemoryAddress aBlockAddress, Transport &aTransport, MafStates aState) {
    DBG_(Trace, (<< theName << " - Adding MAF entry for block " << std::hex << aBlockAddress
                 << " in state " << mafState2String(aState)));
    DBG_Assert(!full());
  ...
  bool full() const {
    if (theMaxTargetsPerRequest == 0) {
      ...
      return theWaitResponseEntries >= theSize ||
             theMshrs.size() > theSize * theMaxTargetsPerRequest;
    }
  }
  ...
  getWaitingMAFEntry(const MemoryAddress &aBlockAddress) {
    ...
    if (iter == theMshrs.end()) {
      ...
      return std::make_pair(boost::intrusive_ptr<MemoryMessage>(0),
                            boost::intrusive_ptr<TransactionTracker>(0));
    }

    return std::make_pair(iter->transport[MemoryMessageTag],
                          iter->transport[TransactionTrackerTag]);
  }
  ...
  getAllMessages(const MemoryAddress &aBlockAddress) {
  ...
  std::list<boost::intrusive_ptr<MemoryMessage>>
  getAllUncompletedMessages(const MemoryAddress &aBlockAddress) {
    std::list<boost::intrusive_ptr<MemoryMessage>> ret_val;
    maf_t::iterator iter, end;
    std::tie(iter, end) = theMshrs.equal_range(std::make_tuple(aBlockAddress));
    while (iter != end) {
  ...
  void dump(void) {
    ...
    DBG_(Trace, Set((CompName) << theName)(<< theName << " MAF content dump (" << theMshrs.size()
                                           << "/" << theSize));
...
public:
  ProcessEntry(ProcessType aType)
      : theType(aType), theRequiresData(0), theTransmitAfterTag(false), theTagOutstanding(false),
        theReservations(0), theRemoveMafEntry(false), theSerial(theProcessSerial++),
        theSendToD(false), theSendToI(false), theSendToBack(false), theWakeSnoops(false),
        theWakeEvicts(false), theWakeRegion(false), theWakeMaf(false) {
  }

  ProcessEntry(const Transport &request, ProcessType aType)
      : theOrig(request), theFrontTransport(request), theBackTransport(request), theType(aType),
        theRequiresData(0), theTransmitAfterTag(false), theTagOutstanding(false),
        theReservations(0), theRemoveMafEntry(false), theSerial(theProcessSerial++),
        theSendToD(false), theSendToI(false), theSendToBack(false), theWakeSnoops(false),
        theWakeEvicts(false), theWakeRegion(false), theWakeMaf(false) {
  }

  ProcessEntry(MissAddressFile::maf_iter iter, boost::intrusive_ptr<TransactionTracker> wake,
               ProcessType aType)
      : theOrig(iter->transport), theFrontTransport(iter->transport),
        theBackTransport(iter->transport), theMafEntry(iter), theType(aType),
        theWakeTransaction(wake), theRequiresData(0), theTransmitAfterTag(false),
        theTagOutstanding(false), theReservations(0), theRemoveMafEntry(false),
        theSerial(theProcessSerial++), theSendToD(false), theSendToI(false), theSendToBack(false),
        theWakeSnoops(false), theWakeEvicts(false), theWakeRegion(false), theWakeMaf(false) {
  }
  ...
  ~ProcessEntry() {
    ...
    if (theReservations != 0) {
      DBG_(Crit, (<< "Process ending with resources reserved: " << std::hex << theReservations
  ...
  void reserve(const int32_t res) {
    if ((theReservations & res) != 0) {
      DBG_(Crit, (<< "Process serial: " << serial() << " WARNING: cache resource leak of type: 0x"
                  << std::hex << (theReservations & res)));
    }
  ...
  void unreserve(const int32_t res) {
    if ((theReservations & res) != res) {
      DBG_(Crit,
           (<< "Process serial: " << serial() << " MISSING RESOURCE ON UNRESERVE: " << std::hex
            << res << " have: " << theReservations << " - > " << *theOrig[MemoryMessageTag]));
    }
    theReservations = theReservations & (~res);
  }
...
struct SnoopTransportEntry {
  ...
  SnoopTransportEntry(intrusive_ptr<ProcessEntry> process)
      : serial(process->transport()[MemoryMessageTag]->serial()), transport(process->transport()) {
  }
};
...
class CacheController {
  ...
  std::vector<std::list<std::pair<MemoryAddress, boost::intrusive_ptr<TransactionTracker>>>>
      theWakeMAFList;
...
public:
  ...
  CacheController(std::string const &aName, int32_t aCores, std::string const &anArrayConfiguration,
                  int32_t aBlockSize, uint32_t aBanks, uint32_t aPorts, uint32_t aTagLatency,
                  uint32_t aTagIssueLatency, uint32_t aDataLatency, uint32_t aDataIssueLatency,
                  int32_t nodeId, tFillLevel aCacheLevel, uint32_t aQueueSize,
                  uint32_t aPreQueueSize, uint32_t aMAFSize, uint32_t aMAFTargetsPerRequest,
                  uint32_t anEBSize, uint32_t aSnoopBufSize, bool aProbeOnIfetchMiss,
                  bool aDoCleanEvictions, bool aWritableEvictsHaveData,
                  const std::string &aCacheType, uint32_t aTraceAddress,
                  bool anAllowOffChipStreamFetch, bool anEvictOnSnoop, bool aUseReplyChannel);
  ...
  std::pair<MemoryMessage_p, TransactionTracker_p>
  getWaitingMAFEntry(const MemoryAddress &aBlockAddress);
...
private:
  ...
  void enqueueWakeMaf(MemoryAddress const &anAddress,
