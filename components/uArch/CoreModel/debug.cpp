...
void CoreImpl::dumpSRB() {
  if ( ! theSRB.empty() ) {
    DBG_( Verb, ( << theName << "*** SRB Contents ***" ) );
    rob_t::iterator iter, end;
...
void CoreImpl::dumpMemQueue() {
  if ( ! theMemQueue.empty() ) {
    DBG_( Verb, ( << theName << "*** MemQueue Contents ***" ) );
...
void CoreImpl::dumpCheckpoints() {
  if ( ! theCheckpoints.empty() ) {
    ...
    for ( iter = theCheckpoints.begin(), end = theCheckpoints.end(); iter != end; ++iter) {
      DBG_( Verb, ( << "   " << *iter->first ));
      DBG_( Verb, ( << "   Lost Permissions: " << iter->second.theLostPermissionCount ));
      DBG_( Verb, ( << "   Required Addresses" ));
      ...
      while (req != iter->second.theRequiredPermissions.end()) {
        DBG_( Verb, ( << "        " << req->first << "\t" << *req->second ));
        ++req;
      }
      DBG_( Verb, ( << "  Held Addresses " ));
...
void CoreImpl::dumpSBPermissions() {
  if ( ! theSBLines_Permission.empty() ) {
    DBG_( Verb, ( << theName << "*** SB Line Tracker ***" ) );
    ...
    for ( iter = theSBLines_Permission.begin(), end = theSBLines_Permission.end(); iter != end; ++iter) {
      DBG_( Verb, ( << "   " << iter->first << " SB: " << iter->second.first  << " onchip: " << iter->second.second ));
    }
  }
}
...
void CoreImpl::printRegMappings(std::string aRegSet) {
  if (aRegSet == "r") {
    ...
    theMapTables[rRegisters]->dumpMappings(std::cout);
  } else if (aRegSet == "f") {
    std::cout << theName << " *** f register mappings *** " << std::endl;
    theMapTables[fRegisters]->dumpMappings(std::cout);
  } else if (aRegSet == "ccr") {
...
void CoreImpl::printRegFreeList(std::string aRegSet) {
  if (aRegSet == "r") {
    ...
    theMapTables[rRegisters]->dumpFreeList(std::cout);
  } else if (aRegSet == "f") {
    std::cout << theName << " *** f register free list *** " << std::endl;
    theMapTables[fRegisters]->dumpFreeList(std::cout);
  } else if (aRegSet == "ccr") {
...
void CoreImpl::printRegReverseMappings(std::string aRegSet) {
  if (aRegSet == "r") {
    ...
    theMapTables[rRegisters]->dumpReverseMappings(std::cout);
  } else if (aRegSet == "f") {
    std::cout << theName << " *** f register reverse mappings *** " << std::endl;
    theMapTables[fRegisters]->dumpReverseMappings(std::cout);
  } else if (aRegSet == "ccr") {
...
void CoreImpl::printAssignments(std::string aRegSet) {
  if (aRegSet == "r") {
    ...
    theMapTables[rRegisters]->dumpAssignments(std::cout);
  } else if (aRegSet == "f") {
    std::cout << theName << " *** f register assignments *** " << std::endl;
    theMapTables[fRegisters]->dumpAssignments(std::cout);
  } else if (aRegSet == "ccr") {
