...
void CoreImpl::invalidate( PhysicalMemoryAddress anAddress) {
  FLEXUS_PROFILE();
  DBG_Assert ( static_cast<uint64_t>( anAddress) == ( static_cast<uint64_t>(anAddress) & ~( theCoherenceUnit - 1) ), ( << "address = " << std::hex << static_cast<uint64_t>(anAddress) ) );
  ...
  if (isSpeculating()) {
    ...
    while (iter != end ) {
      if (! violator || iter->second->sequenceNo() < (*violator)->sequenceNo() ) {
        if (iter->second->instClass() == clsAtomic) {
          ...
          memq_t::index< by_insn >::type::iterator violator_memq = theMemQueue.get<by_insn>().find( iter->second );
          DBG_Assert( violator_memq != theMemQueue.get<by_insn>().end() );
          if (! violator_memq->theSpeculatedValue) {
            //must roll back
            DBG_(Dev, ( << theName << " SLAT hit required " << anAddress << " for: " << *violator_memq << " because value is no longer speculative" ));
            violator = iter->second;
          } else {
            DBG_(Dev, ( << theName << " avoided SLAT hit on " << anAddress << " for: " << *violator_memq << " because value is still speculative" ));
    ...
    if (violator) {
      ...
      DBG_(Verb, ( << theName << " SLAT hit on " << anAddress << " earliest instruction: " << *theViolatingInstruction <<   " - speculation must abort" ));
  ...
  //(2):      the load cannot receive its value via forwarding from an unperformed store
  //(3 SC):   the load follows an incomplete load or store in program order
  //(3 TSO):  the load follows an incomplete load in program order
  //-or- the load follows a MEMBAR in the LSQ
  //-or- the load is or follows an Atomic op in the LSQ
  //-or- the system is speculating (an Atomic or MEMBAR has speculatively retired)
  ...
  while (start != end) {
    ...
    //received a value.
    if (temp->isLoad() && temp->theQueue != kSB && temp->status() == kComplete && !temp->theSpeculatedValue && ! temp->theInstruction->isAnnulled()) {
      //Count the race
      if (! race_counted) {
        ++theRaces;
        race_counted = true;
      ...
      if ( first_incomplete && temp->theSequenceNum >= *first_incomplete ) {
        ...
        DBG_( Iface, ( << "Completed load to address " << anAddress << " which does not need re-execution.  Will override Simics.") );
...
bool CoreImpl::processReply( MemOp const & anOperation ) {
  ...
  switch (anOperation.theOperation) {
    ...
    default:
      DBG_Assert( false, ( << "Memory operation type " << anOperation.theOperation << " not supported " ) );
  }
  return true;
}
...
bool CoreImpl::satisfies(eOperation aResponse, eOperation anOperation) {
  switch ( anOperation ) {
    ...
    case kRMW:
      return aResponse == kRMWReply || ( theSpeculativeOrder && ( aResponse == kAtomicPreloadReply || aResponse == kStoreReply)) ;
    case kCAS:
      return aResponse == kCASReply || ( theSpeculativeOrder && (aResponse == kAtomicPreloadReply || aResponse == kStoreReply));
    case kAtomicPreload:
  ...
  if ( match != theMSHRs.end() ) {
    ...
    if (anOperation.theTracker) {
      if ( anOperation.theTracker->fillType() && *anOperation.theTracker->fillType() == eCoherence ) {
        ++theWP_CoherenceMiss;
        DBG_( Trace, ( << theName << " WrongPath Coherence " << (anOperation.theTracker->OS() && *anOperation.theTracker->OS() ? "[S] " : "[U] ") << anOperation ) );
      } else if ( anOperation.theTracker->fillLevel() && *anOperation.theTracker->fillLevel() == ePrefetchBuffer ) {
        ++theWP_SVBHit;
        DBG_( Trace, ( << theName << " WrongPath: SVB-HIT " << (anOperation.theTracker->OS() && *anOperation.theTracker->OS() ? "[S] " : "[U] ") << anOperation ) );
      } else {
...
void CoreImpl::completeLSQ( memq_t::index<by_insn>::type::iterator lsq_entry, MemOp const & anOperation ) {
  FLEXUS_PROFILE();
  DBG_Assert
  ( lsq_entry->status() == kIssuedToMemory
  ...
  if (anOperation.theOperation == kAtomicPreloadReply ) {
    ...
    if (theSpeculateOnAtomicValue && lsq_entry->theSpeculatedValue && (lsq_entry->theQueue != kLSQ) ) {
      ...
      if ( anOperation.theValue != *lsq_entry->theExtendedValue ) {
        ...
        DBG_(Dev, ( << theName << " Value mispredict Predicted: " << *lsq_entry->theExtendedValue << " actual: " << anOperation.theValue << " LSQ entry: " << *lsq_entry ));
        ...
        DBG_(Iface, ( << theName << " Value Predict correct: " << *lsq_entry->theExtendedValue << " LSQ entry: " << *lsq_entry ));
      }
    } else {
    ...
    if ( lsq_entry->theSideEffect) {
      ...
      DBG_Assert( lsq_entry->theQueue == kSB || ( lsq_entry->theQueue == kSSB && lsq_entry->isAtomic() && theSpeculativeOrder ) );
  ...
  //We remove stores/successfully speculated atomics from the store buffer when their reply arrives
