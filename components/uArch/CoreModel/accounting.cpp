...
void CoreImpl::accountResyncReason( boost::intrusive_ptr< Instruction > anInstruction) {
  switch (anInstruction->instCode()) {
    ...
    case codeLoadFP:
    case codeLDD: {
      int32_t asi = anInstruction->getASI();
      if (resyncASI(asi)) {
...
void CoreImpl::accountRetire( boost::intrusive_ptr<Instruction> anInst) {
  ...
  int32_t retire_stall_cycles = theTimeBreakdown.retire(theLastStallCause, theROB.front()->sequenceNo(), theCycleCategory, theSpinning);
