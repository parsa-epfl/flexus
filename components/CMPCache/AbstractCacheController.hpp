...
namespace nCMPCache {
	...
	public:
		...
		{
			thePolicy =  AbstractFactory<AbstractPolicy,CMPCacheInfo>::createInstance(anInfo.thePolicyType, theCMPCacheInfo);
...
DBG_(Verb, ( << theName << " reserveEB for addr: " << process->transport()[MemoryMessageTag]->address() << ", total reservations = " << theEBReservations[process->transport()[MemoryMessageTag]->address()] ));
}
		}
...
DBG_Assert(process->snoop_out_reserved >= n, ( << theName << " process tried to unreserve " << n << " snoop entries, but only has " << process->snoop_out_reserved << " entries reserved: " << *process ));
			process->snoop_out_reserved -= n;
theSnoopReservations[process.get()] -= n;
...
if (strcmp(theName.c_str(), "13-L2") == 0)
DBG_(Verb, ( << theName << " unreserveCacheEB for addr: " << process->transport()[MemoryMessageTag]->address() << ", remaining reservations = " << theEBReservations[process->transport()[MemoryMessageTag]->address()] ));
if (theEBReservations[process->transport()[MemoryMessageTag]->address()] == 0) {
...
if (strcmp(theName.c_str(), "13-L2") == 0)
DBG_(Verb, ( << theName << " unreserveEB for addr: " << process->transport()[MemoryMessageTag]->address() << ", remaining reservations = " << theEBReservations[process->transport()[MemoryMessageTag]->address()] ));
if (theEBReservations[process->transport()[MemoryMessageTag]->address()] == 0) {
	theEBReservations.erase(process->transport()[MemoryMessageTag]->address());
}
...
for(; iter != end; iter++) {
if (strcmp(theName.c_str(), "13-L2") == 0)
	DBG_(Verb, ( << theName << " reserved " << iter->second << " EB entries for address " << std::hex << iter->first ));
}
		}
...
#define REGISTER_CMP_CACHE_CONTROLLER(type, n) const std::string type::name = n; static ConcreteFactory<AbstractCacheController,type,CMPCacheInfo> type ## _Factory
