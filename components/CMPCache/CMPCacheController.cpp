...
CMPCacheController::CMPCacheController(	const CMPCacheInfo & anInfo)
  ...
  , theMAFPipeline(anInfo.theName + "-MafPipeline", 1, 1, 0,
                   boost::intrusive_ptr<Stat::StatLog2Histogram>(new Stat::StatLog2Histogram(anInfo.theName + "-MafPipeline" + "-InterArrivalTimes")))
  , theDirTagPipeline(anInfo.theName + "-DirPipeline", 1, MAX(anInfo.theDirIssueLatency, anInfo.theTagIssueLatency), MAX(anInfo.theDirLatency, anInfo.theTagLatency),
                      boost::intrusive_ptr<Stat::StatLog2Histogram>(new Stat::StatLog2Histogram(anInfo.theName + "-DirPipeline" + "-InterArrivalTimes")))
  , theDataPipeline(anInfo.theName + "-DataPipeline", 1, anInfo.theDataIssueLatency, anInfo.theDataLatency,
                    boost::intrusive_ptr<Stat::StatLog2Histogram>(new Stat::StatLog2Histogram(anInfo.theName + "-DataPipeline" + "-InterArrivalTimes")))
...
AbstractCacheController * CMPCacheController::createInstance(std::list<std::pair<std::string, std::string> > & args, const CMPCacheInfo & params) {
...
bool CMPCacheController::isQuiesced() const {
  return  theMAFPipeline.empty()
          && theDirTagPipeline.empty()
          && theDataPipeline.empty()
          && RequestIn.empty()
          && SnoopIn.empty()
          && ReplyIn.empty()
          && RequestOut.empty()
...
void CMPCacheController::loadState(std::string const & aDirName) {
  ...
  // The functional simulator saves all the cmp cache/directory state in single files
  // but the timing simulator has seperate objects for each bank
  // This allows the timing simulator to use the same data for multiple different banking schemes
...
void CMPCacheController::scheduleNewProcesses() {
  ...
  if (!scheduled && thePolicy->MAF().hasWakingEntry()) {
    DBG_(Trace, ( << theName << " Failed to schedule waking MAF: SnoopOut " << SnoopOut.getSize() << "/" << SnoopOut.getReserve()
                  ...
                  << ", EB full " << std::boolalpha << thePolicy->EBFull() ));
  }
  ...
  // Do the Cache Evict Buffer and Directory Evict Buffer separately

  while (  theMAFPipeline.serverAvail()
	   && !theIdleWorkScheduled
  ...
  while (  theMAFPipeline.serverAvail()
	   && !theIdleWorkScheduled
           ...
           && thePolicy->DirEB().idleWorkReady()
           && thePolicy->CacheEBHasSpace()
           && SnoopOut.hasSpace(1)
        ) {
  ...
  if (!scheduled && !thePolicy->freeDirEBPending()) {
    ...
    DBG_(Iface, ( << theName << " Failed to schedule Dir Evict when DirEB FULL: Used/Reserved/Size = " << thePolicy->DirEB().used() << "/" << thePolicy->DirEB().reserved() << "/" << thePolicy->DirEB().size() << ", invalidates pending = " << thePolicy->DirEB().invalidates() << ": " << thePolicy->DirEB().listInvalidates() ));
  }
  ...
  if (!scheduled && !RequestIn.empty()) {
    DBG_(Iface, ( << theName << " Failed to schedule Request: SnoopOut " << SnoopOut.getSize() << "/" << SnoopOut.getReserve()
                  << ", ReplyOut " << ReplyOut.getSize() << "/" << ReplyOut.getReserve()
                  << ", ReqOut " << RequestOut.getSize() << "/" << RequestOut.getReserve()
                  << ", EB full " << std::boolalpha << thePolicy->EBFull() << ", MAF full " << thePolicy->MAF().full() ));
    if (thePolicy->EBFull()) {
      DBG_(Iface, ( << theName << " DirEB.full() = " << std::boolalpha << thePolicy->DirEB().full() << ", CacheEB (reserved,used,size) = (" << thePolicy->CacheEB().reservedEntries() << ", " << thePolicy->CacheEB().usedEntries() << ", " << theCMPCacheInfo.theCacheEBSize << "), ERB available = " << thePolicy->arrayEvictResourcesAvailable() << ", freeDirEBPending() = " << thePolicy->freeDirEBPending() << ", Dir.idleWorkReady() = " << thePolicy->DirEB().idleWorkReady() ));
    }
  }
...
void CMPCacheController::runReplyProcess(ProcessEntry_p process) {
  ...
  switch (process->action()) {
    ...
    case eReplyAndRemoveMAF:
      ...
      unreserveEB(process); // Make sure we get rid of any EB reservations we might have gotten from the MAF
      // Send a reply message (mimic a reply that would have been sent if the evict hadn't happened)
      break;
