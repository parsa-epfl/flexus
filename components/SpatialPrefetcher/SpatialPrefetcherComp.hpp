...
struct SpatialGroupEntry {
  ...
  SpatialGroupEntry(SpatialPattern aPattern, RepetIndex anIndex)
      : pattern(aPattern), fills(aPattern.size()), offChipFills(aPattern.size()),
        prefetched(aPattern.size()), index(anIndex), last_poke(anIndex), logical_time(0),
        finalized(false), unique_id(++theSpatialGroupCount) {
  }
  SpatialGroupEntry(SpatialPattern aPattern, RepetIndex anIndex, SpatialPattern aFills)
      : pattern(aPattern), fills(aFills), offChipFills(aPattern.size()),
        prefetched(aPattern.size()), index(anIndex), last_poke(anIndex), logical_time(0),
        finalized(false), unique_id(++theSpatialGroupCount) {
  }
  SpatialGroupEntry(SpatialPattern aPattern, SpatialPattern aFills)
      : pattern(aPattern), fills(aFills), offChipFills(aPattern.size()),
        prefetched(aPattern.size()), index(0), last_poke(0), logical_time(0), finalized(false),
        unique_id(++theSpatialGroupCount) {
  }
  SpatialGroupEntry(std::istream &ifs, uint32_t sgpBlocks)
      : pattern(sgpBlocks), fills(sgpBlocks), offChipFills(sgpBlocks), prefetched(sgpBlocks),
        logical_time(0), finalized(false), unique_id(++theSpatialGroupCount) {
    ifs >> std::hex >> index;
    last_poke = index;
...
enum RepetReturnType { kRepetRetNone, kRepetRetNewGen, kRepetRetFirstAccess };
...
public:
  SpatialHistorySingle(SpatialPattern pattern, uint64_t access)
      : theHistory(pattern), theLastAccess(access) {
  }
  SpatialHistorySingle(std::istream &line, uint32_t sgpBlocks) : theHistory(sgpBlocks) {
...
public:
  SpatialHistoryUnion(SpatialPattern pattern) : theHistory1(pattern), theHistory2(pattern.size()) {
  }
  SpatialPattern extract() const {
...
public:
  SpatialHistoryInter(SpatialPattern pattern) : theHistory1(pattern), theHistory2(pattern.size()) {
  }
  SpatialPattern extract() const {
    return (theHistory1 & theHistory2);
  }
...
public:
  SpatialHistory2Bit(SpatialPattern pattern) : theMSBs(pattern), theLSBs(~pattern) {
  }
  SpatialPattern extract() const {
    return theMSBs;
  }
...
struct GroupOrderingEntry {
  ...
  GroupOrderingEntry() : theNextSeqNo(0) {
  }
  GroupOrderingEntry(uint8_t blockNo) : theNextSeqNo(1) {
...
struct CurrentStreamEntry {
  ...
  CurrentStreamEntry(OrderingMap order, uint8_t lastBlock, GroupOffset offset)
      : theDense(false), theOrder(order), theBlockNo(lastBlock), theHits(0), theOffset(offset) {
  }
  CurrentStreamEntry(SpatialPattern pattern, uint8_t nextBlock, GroupOffset offset, bool isRight)
      : theDense(true), thePattern(pattern), theBlockNo(nextBlock), theHits(0), theOffset(offset),
        theIsRight(isRight) {
  }
  bool isDense() const {
    return theDense;
...
struct DoneStreamEntry {
  ...
  DoneStreamEntry(bool dense, int16_t possible, int16_t hits)
      : theDense(dense), thePossible(possible), theHits(hits) {
  }
  bool isDense() {
    return theDense;
  }
...
struct GroupTimeEntry {
  ...
  GroupTimeEntry(uint64_t curr)
      : live(true), lastAccess(curr), lastEvict(curr), lastBegin(curr), lastEnd(curr) {
  }
};
...
struct ActiveGroupEntry {
  ...
  ActiveGroupEntry(uint32_t curr, SpatialPattern initial)
      : start(curr), recent(curr), outstanding(initial) {
  }
};
...
enum CacheBlockState { ePrefetching, eFilling, ePrefetched, ePresent };
...
public:
  SpatialPrefetcher(std::string statName, std::string altLoadName, int32_t aNode)
      : theName(statName), theAltLoadName(altLoadName), theNodeId(aNode), theNumAccesses(0),
        theCounter(0), theCptSparseCount(0), theCurrDense(false), theStitchReady(false),
        statUsageAccess(statName + "-UsageAccess"), statUsageFill(statName + "-UsageFill"),
        statUsageOffChip(statName + "-UsageOffChip"), statRepetYes(statName + "-RepetYes"),
        statRepetCountYes(statName + "-RepetCountYes"), statRepetNo(statName + "-RepetNo"),
        statRepetCountNo(statName + "-RepetCountNo"), statRepetHit(statName + "-RepetHit"),
        statRepetTraining(statName + "-RepetTraining"),
        statRepetMispred(statName + "-RepetMispred"), statRepetMiss(statName + "-RepetMisses"),
        statRepetFillsYes(statName + "-RepetFills"), statRepetFillsNo(statName + "-RepetFillNo"),
        ...
        statParallelGroups(statName + "-ParallelGroups"), statOrderDist(statName + "-OrderDist"),
        ...
        statTimeInterEnd(statName + "-TimeInterEnd"), statPrefetchYes(statName + "-PrefetchYes"),
        statPrefetchNo(statName + "-PrefetchNo"), statPrefetchCount(statName + "-PrefetchCount"),
        statFills(statName + "-Fills"), statPrefetches(statName + "-Prefetches"),
        statCorrect(statName + "-GoodPrefetches"), statPartial(statName + "-PartialPrefetches"),
        statLate(statName + "-LatePrefetches"), statDup(statName + "-DuplicatePrefetches"),
        statMispredictEvicts(statName + "-EvictPrefetches"),
        statMispredictInvals(statName + "-InvalPrefetches"), statEvictions(statName + "-Evictions"),
        statInvalidations(statName + "-Invalidations"), statInserts(statName + "-Insertions") {
  }

  void init(bool enableUsageStats, bool enableRepetStats, bool enableBufFetch, bool enableTimeRepet,
            bool enablePrefetch, bool enableActive, bool enableOrdering, bool enableStreaming,
            int32_t blockSize, int32_t sgpBlocks, int32_t repetType, bool repetFills,
            bool sparseOpt, int32_t phtSize, int32_t phtAssoc, int32_t pcBits, int32_t cptType,
            int32_t cptSize, int32_t cptAssoc, bool cptSparse, bool fetchDist, int32_t streamWindow,
            bool streamDense, bool sendStreams, int32_t bufSize, int32_t streamDescs,
            bool delayedCommits, int32_t cptFilterSize) {
    DBG_(Dev, (<< theName << ": Initializing SGP - spatial group: " << sgpBlocks << " " << blockSize
               << "B blocks"));
    ...
    if (theEnablePrefetch) {
      DBG_Assert(theEnableBufFetch, (<< "Error: buffer prefetching must be "
    ...
    if (theEnableActive) {
      DBG_Assert(theEnableBufFetch,
                 (<< "Error: buffer prefetching must be enabled for active tracking"));
    }
    ...
    if (theCptSparse) {
      DBG_Assert(!theRepetFills, (<< "theCptSparseCount not updated correctly "
  ...
  void loadState(std::string const &aDirName) {
    ...
    if (!ifs.good()) {
      if (theEnablePrefetch) {
        DBG_(Dev, (<< " saved checkpoint states " << fname1 << ", " << fname2 << " and " << fname3
                   << " not found.  Aborting. "));
        // DBG_Assert(false);
      } else {
        DBG_(Dev, (<< " saved checkpoint states " << fname1 << ", " << fname2 << " and " << fname3
...
private:
  ...
  struct OrderedBlockEntry {
    uint64_t cycles;
    bool dense;
    OrderedBlockEntry(uint64_t curr, bool d) : cycles(curr), dense(d) {
    }
  };
...
public:
  void access(MemoryAddress addr, MemoryAddress pc, bool prefetch, bool write, bool miss,
              bool priv) {
    DBG_(Iface, (<< theName << (prefetch ? " prefetch" : " access") << " for addr " << std::hex
                 << addr << " (" << (write ? "write" : "read") << "/" << (miss ? "miss" : "hit")
                 << "/" << (priv ? "OS" : "user") << ")"));
    ...
    if (!(prefetch && miss)) {
      if (theEnableUsage)
        doGroupUsage(addr, false);
      if (theEnableRepet) {
        RepetReturnType repet = doGroupRepet(addr, pc, false, miss || prefetch);
        if (miss || prefetch) {
          if (theEnableBufFetch)
            doBufFetchMiss(block, write, priv);
        }
        if (theEnableStreaming)
          doStreamAccess(block, write, miss || prefetch, priv, repet);
        if (!theEnableStitch) {
  ...
  void evict(MemoryAddress block) {
    ...
    DBG_Assert(block == makeBlockAddress(block));
    if (theEnableUsage)
      doGroupUsage(block, true);
    if (theEnableRepet)
      doGroupRepet(block, 0, true, false);
  }
...
private:
  ...
  RepetReturnType doGroupRepet(MemoryAddress addr, MemoryAddress pc, bool evict, bool miss) {
      ...
      if (evict) {
        ...
        if ((!theRepetFills && accessedThisGen.any()) || (theRepetFills && filledThisGen.any())) {
          ...
          if (!theDelayedCommits || iter->second.hasIndex()) {
            ...
            beginend = true;
            hits = finalizeGroupRepet(iter->second.index, iter->second.pattern, iter->second.fills,
                                      iter->second.offChipFills, removedPht);
            if (theEnableOrdering) {
              OrderingBuildIter build = theOrderingBuild.find(group);
              DBG_Assert(build != theOrderingBuild.end());
            ...
            if (theCptSparse) {
              ...
              if ((theGroupRepets.size() - theCptSparseCount) >= theCptSize) {
                ...
                if (!theDelayedCommits || seq->second.hasIndex()) {
                  hits =
                      finalizeGroupRepet(seq->second.index, seq->second.pattern, seq->second.fills,
                                         seq->second.offChipFills, removedPht);
                } else {
                ...
                if (theEnableOrdering) {
                  ...
                  DBG_Assert(build != theOrderingBuild.end());
                  if (!theDelayedCommits || seq->second.hasIndex()) {
                    finalizeGroupOrdering(seq->second.index, build->second.theOrder, hits,
                                          removedPht);
                  }
                  theOrderingBuild.erase(build);
                }
                if (theEnableBufFetch)
                  doBufFetchEndGen(seq->first);
                theGroupRepets.eraseSeq(seq);
              }
            }
          }
          DBG_(Trace, (<< theName << "  group_" << std::hex << group << " set bit "
              ...
              if (theGroupRepets.size() >= theCptSize) {
                if (!theDelayedCommits || theGroupRepets.front().hasIndex()) {
                  hits = finalizeGroupRepet(theGroupRepets.front().index,
                                            ...
                                            theGroupRepets.front().offChipFills, removedPht);
                } else {
                  statRepetEvictCptNoIndex++;
                  statRepetEvictCptNoIndexAccesses
                ...
                if (theEnableOrdering) {
                  ...
                  DBG_Assert(build != theOrderingBuild.end());
                  if (!theDelayedCommits || theGroupRepets.front().hasIndex()) {
                    finalizeGroupOrdering(theGroupRepets.front().index, build->second.theOrder,
                                          hits, removedPht);
                  }
                  theOrderingBuild.erase(build);
                }
                ...
                if (theEnableBufFetch)
                  doBufFetchEndGen(theGroupRepets.front_key());
    ...
    if (theEnableTimeRepet)
      doGroupTime(addr, pc, evict, beginend);

    return ret;
  }

  std::pair<RepetReturnType, int64_t> doRepetCommit(MemoryAddress addr, MemoryAddress pc,
      ...
      if (!iter->second.hasIndex() || iter->second.last_poke != makeRepetIndex(addr, pc)) {
        ...
        if (accessedThisGen.any()) {
          ...
          if (iter->second.finalized) {
            ...
            int32_t hits = finalizeGroupRepet(index, iter->second.pattern, iter->second.fills,
                                              iter->second.offChipFills, removedPht);
            if (theEnableOrdering) {
              OrderingBuildIter build = theOrderingBuild.find(group);
              DBG_Assert(build != theOrderingBuild.end());
    ...
    if (iter != theGroupAssocRepets.end()) {
      ...
      if (!theRepetFills || miss) {
        ...
        if ((theCptAssoc > 0) && (theGroupAssocRepets.size() >= theCptAssoc)) {
          bool removedPht = false;
          finalizeGroupRepet(theGroupAssocRepets.front().index, theGroupAssocRepets.front().pattern,
                             theGroupAssocRepets.front().fills,
                             theGroupAssocRepets.front().offChipFills, removedPht);
          if (theEnableBufFetch)
            doBufFetchEndGen(theGroupAssocRepets.front_key());
  ...
  */
  ...
  int32_t finalizeGroupRepet(RepetIndex index, SpatialPattern current, SpatialPattern fills,
                             SpatialPattern offChipFills, bool &removed) {
    if (index == 0) {
      DBG_(Crit, (<< "Warning: zero index.  This is suspicious, but may be "
                     "legitimate "));
    }
    // DBG_Assert(index != 0);
    if (doRotate()) {
  ...
  void doBufFetchGen(MemoryAddress addr, MemoryAddress pc, int64_t aSeqNo) {
    ...
    if (theSendStreams) {
      if (!theCurrPrefetchCommand->addressList().empty()) {
        ...
        std::list<PhysicalMemoryAddress>::iterator iter =
            theCurrPrefetchCommand->addressList().begin();
        while (iter != theCurrPrefetchCommand->addressList().end()) {
          prefetchList << std::hex << makeGroupOffset(*iter, theSgpBits) << " ";
          ++iter;
        }
        DBG_(Trace, (<< theName << " group_" << std::hex << makeGroupAddress(addr, theSgpBits)
                     << " prefetch bits " << prefetchList.str()));
  ...
  bool doBufFetchBlock(MemoryAddress block) {
    ...
    if (iter != theOrderedBlocks.end()) {
      ...
      theOrderedBlocks.insert(
          std::make_pair(block, OrderedBlockEntry(getCycleCount(), theCurrDense)));
    }
    return ret;
  }
  ...
  void doBufFetchMiss(MemoryAddress block, bool write, bool priv) {
    ...
    if (doBufFetchErase(block, true)) {
      if (write) {
        ...
        if (priv)
          statBufFetchGoodWriteOS++;
      } else {
        statBufFetchGoodRead++;
        if (priv)
          statBufFetchGoodReadOS++;
      }
    }
  }
  ...
  void freeStreamDesc() {
    ...
    theDoneStreams.insert(
        std::make_pair(group, DoneStreamEntry(entry.isDense(), entry.possible(), entry.hits())));
    ...
    if (iter != theOrderingHistory.end()) {
      ...
      if (theStreamDense && ((int)order.size() > getDenseThreshold())) {
        ...
        if (!theSendStreams) {
          ...
          theDoneStreams.insert(
              std::make_pair(group, DoneStreamEntry(true, toPrefetch.count(), 0)));
        ...
        if (!theSendStreams) {
          ...
          // (unsigned)lastBlock ) );
          theCurrentStreams.insert(
              std::make_pair(group, CurrentStreamEntry(order, lastBlock, offset)));
          statStreamNewStream++;
        } else {
          // DBG_(Dev, ( << "  done stream" ) );
  ...
  void doStreamAccess(MemoryAddress block, bool write, bool miss, bool priv,
                      RepetReturnType repet) {
    if (miss) {
      if (write) {
        statBufFetchMissWrite++;
  ...
  RepetIndex makeIndexFromConcat(RepetIndex index) {
    ...
    case 6: // PC + rotated block offset
      ...
      break;
    case 7: // PC + rotated word offset
      index = ((index >> theWordBits) << theWordInBlockBits) |
              (index & ((1 << theWordInBlockBits) - 1));
      break;
    default:
      // do nothing
      break;
  ...
  bool loadState(std::istream &ifs) {
    ...
    if (theEnableOrdering) {
      ...
      while (token != "}") {
        ...
        std::istringstream buf2(buffer);
        if (found) {
          OrderingBuildIter iter =
              theOrderingBuild.insert(std::make_pair(addr, GroupOrderingEntry())).first;
    ...
    if (theEnableOrdering) {
      ...
      while (token != "}") {
        ...
        std::istringstream buf2(buffer);
        if (found) {
          OrderingHistoryIter iter =
              theOrderingHistory.insert(std::make_pair(index, OrderingMap())).first;
          uint8_t seqNo = 0;
          while (buf2 >> std::hex >> tempVal) {
            // DBG_(Dev, ( << "appended block no " << tempVal ) );
