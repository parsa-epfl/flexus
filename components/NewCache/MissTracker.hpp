...
#define DBG_DeclareCategories NewCacheMissTracking
#include DBG_Control()
        ...
        case kStart_Coherence:
          ...
          if (front->theTracker->address() ) {
            ...
            if ( address == theLastCoherenceAddress) {
              ...
              if (front->theTracker->OS() && *front->theTracker->OS()) {
                ...
                DBG_( Trace, AddCat(NewCacheMissTracking) ( << theName << " CONTENTION Miss to " << std::hex << address << std::dec  << "[S]" ));
              } else {
                ++theMisses_U;
                ++theMisses_U_CohContend;
                DBG_( Trace, AddCat(NewCacheMissTracking) ( << theName << " CONTENTION Miss to " << std::hex << address << std::dec  << "[U]" ));
              }
          ...
          if (front->theTracker->speculativeAtomicLoad() && *front->theTracker->speculativeAtomicLoad()) {
            if (front->theTracker->OS() && *front->theTracker->OS()) {
              ...
              DBG_( Trace, AddCat(NewCacheMissTracking) ( << theName << " SPEC-ATOMIC-COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[S]" ));
            } else {
              ++theMisses_U;
              ++theMisses_U_SpecAtomicCoherence;
              DBG_( Trace, AddCat(NewCacheMissTracking) ( << theName << " SPEC-ATOMIC-COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[U]" ));
            }
          } else {
            if (front->theTracker->OS() && *front->theTracker->OS()) {
              ++theMisses_S;
...
#endif //FLEXUS_TRACK_COHMISSES
              DBG_( Trace, AddCat(NewCacheMissTracking) ( << theName << " COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[S]" ));
            } else {
              ++theMisses_U;
...
#endif //FLEXUS_TRACK_COHMISSES
              DBG_( Trace, AddCat(NewCacheMissTracking) ( << theName << " COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[U]" ));
            }
          }
