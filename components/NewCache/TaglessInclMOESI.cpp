...
#include "TaglessInclMOESI.hpp"
#include "AbstractArray.hpp"
#include "CacheBuffers.hpp"
#include "MissTracker.hpp"

#define DBG_DeclareCategories NewCache
#define DBG_SetDefaultOps AddCat(NewCache) Set( (CompName) << theName ) Set( (CompIdx) << theNodeId )
#include DBG_Control()
...
////////////////////////////////////////////////////////////////////////////////
// TaglessInclMOESIControllerImpl class

// Register the Implementation so that it can be created automagically
REGISTER_CONTROLLER_IMPLEMENTATION(TaglessInclMOESIControllerImpl, "TaglessInclMOESI");
...
BaseCacheControllerImpl * TaglessInclMOESIControllerImpl::createInstance(std::list<std::pair<std::string, std::string> > &args, const ControllerParams & params) {
  ...
  return new TaglessInclMOESIControllerImpl(params.first, params.second, anAlwaysNAck);
}

TaglessInclMOESIControllerImpl::TaglessInclMOESIControllerImpl ( CacheController * aController,
...
boost::tuple<bool, bool, Action> TaglessInclMOESIControllerImpl::doRequest( MemoryTransport      transport,
    bool                 has_maf_entry,
...
#if 0
  // Perform lookup, select action and update cache state if necessary
  boost::tuple<bool, bool, Action> TaglessInclMOESIControllerImpl::doRequest( MemoryMessage_p        msg,
    ...
    // Check the Snoop Buffer
    // First, look for any active snoops
    SnoopBuffer::snoop_iter iter;
    iter = theSnoopBuffer.getActiveEntry(getBlockAddress(msg->address()));
    //bool snoop_inval_pending = false;
    if (iter != theSnoopBuffer.end()) {
// For now, disallow any overlap
// Need to make sure we handle the case where we have a line in Exclusive state, we get a ReadFwd, issue a Downgrade, and get a Read
...
//   }
      ...
      switch (action.theAction) {
        case kInsertMAF_WaitResponse:
          action.theRequiresData = 0;
          if (action.theBackMessage == 0) {
            action.theBackMessage = msg;
          }
          break;

        case kReplyAndRemoveMAF:
          action.theFrontMessage = msg;
          break;
    ...
    bool TaglessInclMOESIControllerImpl::evictBlock(LookupResult_p victim) {
    ...
    void TaglessInclMOESIControllerImpl::allocateBlock(LookupResult_p result, MemoryAddress aBlockAddress) {
    ...
    // Action TaglessInclMOESIControllerImpl::handleBackMessage(MemoryMessage_p msg, TransactionTracker_p tracker) {
    Action TaglessInclMOESIControllerImpl::handleBackMessage(MemoryTransport transport) {
      MemoryMessage_p msg = transport[MemoryMessageTag];
      TransactionTracker_p tracker = transport[TransactionTrackerTag];
      ...
      switch (msg->type()) {
        ...
        case MemoryMessage::Invalidate:
          if (state == State::Invalid) {
            ...
            // This is bad
            // DBG_Assert(false, ( << "received Invalidate while in " << state << " state : " << (*msg) ));
            // This can happen with Tagless Directory since we don't know if a block is exclusive or not
            DBG_Assert(!waiting_maf, ( << "Received Invalidate while in Exlcusive state and waiting for: " << (*original_miss) ));
    ...
    //Action TaglessInclMOESIControllerImpl::finalizeSnoop(MemoryMessage_p msg, TransactionTracker_p tracker, LookupResult_p result) {
    Action TaglessInclMOESIControllerImpl::finalizeSnoop(MemoryTransport transport, LookupResult_p result) {
      ...
      if (requires_tagless_msg) {
        ...
        // Now, look for any buckets that are in buckets, but not the conflicting_buckets;
        std::vector<int> conflict_free(buckets.size());
        std::vector<int>::iterator last_it = std::set_difference(buckets.begin(), buckets.end(), conflicting_buckets.begin(), conflicting_buckets.end(), conflict_free.begin());
    ...
    //Action TaglessInclMOESIControllerImpl::handleSnoopMessage(MemoryMessage_p msg, TransactionTracker_p tracker) {
    Action TaglessInclMOESIControllerImpl::handleSnoopMessage(MemoryTransport transport) {
    ...
    // at this cache level.
    //Action TaglessInclMOESIControllerImpl::handleIprobe(bool aHit, MemoryMessage_p fetchReq, TransactionTracker_p tracker) {
    Action TaglessInclMOESIControllerImpl::handleIprobe(bool aHit, MemoryTransport transport) {
    ...
    Action TaglessInclMOESIControllerImpl::doEviction() {
      Action action(kSend);
      ...
      if (evictBuffer().headEvictable(0)) {
        ...
        // If there are NO conflict free buckets, don't bother sending the evict unless it's a Dirty Evict
    ...
    bool TaglessInclMOESIControllerImpl::idleWorkAvailable() {
    ...
    MemoryMessage_p TaglessInclMOESIControllerImpl::getIdleWorkMessage(ProcessEntry_p process) {
    ...
    void TaglessInclMOESIControllerImpl::removeIdleWorkReservations(ProcessEntry_p process, Action & action) {
    ...
    //Action TaglessInclMOESIControllerImpl::handleIdleWork(MemoryMessage_p msg, TransactionTracker_p tracker) {
    Action TaglessInclMOESIControllerImpl::handleIdleWork(MemoryTransport transport) {
    ...
    //Action TaglessInclMOESIControllerImpl::handleWakeSnoop ( MemoryMessage_p      msg,
    //                TransactionTracker_p tracker ) {
    Action TaglessInclMOESIControllerImpl::handleWakeSnoop ( MemoryTransport transport ) {
    ...
    void TaglessInclMOESIControllerImpl::dumpEvictBuffer() const {
