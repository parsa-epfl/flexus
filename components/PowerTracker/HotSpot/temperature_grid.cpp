...
/* constructors	*/
blist_t * new_blist(int32_t idx, double occupancy) {
...
blist_t ** *new_b2gmap(int32_t rows, int32_t cols) {
  int32_t i;
  blist_t ** *b2gmap;
...
/* destructor	*/
void delete_b2gmap(blist_t ** *b2gmap, int32_t rows, int32_t cols) {
  int32_t i, j;
  blist_t * ptr, *temp;
...
double blist_avg(blist_t * ptr, flp_t * flp, double * v, int32_t type) {
  double  val = 0.0;
    ...
    for (i = i1; i < i2; i++)
      for (j = j1; j < j2; j++)
        ...
        if ((i > i1) && (i < i2 - 1) && (j > j1) && (j < j2 - 1)) {
          ...
          double oh = (std::min(tu, tc) - std::max(bu, bc));
          double ow = (std::min(ru, rc) - std::max(lu, lc));
          double occupancy;
...
void parse_layer_file(grid_model_t * model, FILE * fp) {
  ...
  int32_t count, i = 0, field = LCF_SNO, ival;
  double dval;
...
void xlate_vector_b2g(grid_model_t * model, double * b, grid_model_vector_t * g, int32_t type) {
  int32_t i, j, n, base = 0;
  double area;
...
void xlate_temp_g2b(grid_model_t * model, double * b, grid_model_vector_t * g) {
  int32_t i, j, n, u, base = 0, count;
  int32_t i1, j1, i2, j2, ci1, cj1, ci2, cj2;
  double min, max, avg;
...
double single_iteration_steady_pack(grid_model_t * model, grid_model_vector_t * power,
  ...
  int32_t nl = model->n_layers;
  int32_t nr = model->rows;
  int32_t nc = model->cols;
  int32_t spidx = nl - DEFAULT_PACK_LAYERS + LAYER_SP;
  int32_t hsidx = nl - DEFAULT_PACK_LAYERS + LAYER_SINK;
...
double single_iteration_steady_grid(grid_model_t * model, grid_model_vector_t * power,
  ...
  return (std::max(max, single_iteration_steady_pack(model, power, temp)));
}
...
void debug_print_grid_model_vector(grid_model_vector_t * v, int32_t nl, int32_t nr, int32_t nc) {
  int32_t n;
