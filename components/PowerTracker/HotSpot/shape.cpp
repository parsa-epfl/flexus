...
shape_t * shape_from_aspect(double area, double min,
  ...
  if (rotable && min <= 1.0 && max >= 1.0) {
    overlap = 1;
    tmin = std::min(std::min(min, max), std::min(1.0 / min, 1.0 / max));
    tmax = std::max(std::max(min, max), std::max(1.0 / min, 1.0 / max));
    min = tmin;
    max = tmax;
  }
  ...
  if (!rotable || overlap) {
    ...
    tmin = std::min(min, 1.0 / min);
    tmax = std::min(max, 1.0 / max);
    minx = sqrt(area * std::min(tmin, tmax));
    maxx = sqrt(area * std::max(tmin, tmax));
    if ( n > 1)
    ...
    /* orientations with aspect ratios > 1	*/
    tmin = std::max(min, 1.0 / min);
    tmax = std::max(max, 1.0 / max);
    minx = sqrt(area * std::min(tmin, tmax));
    maxx = sqrt(area * std::max(tmin, tmax));
    if ( n > 1)
      r = pow((maxx / minx) , 1.0 / (n - 1));
...
shape_t * shape_add(shape_t * shape1, shape_t * shape2, int32_t cut_type) {
  ...
  while (i < m && j < n) {
    ...
    if (cut_type == CUT_VERTICAL) {
      ...
      sum->y[k] = std::max(shape1->y[i], shape2->y[j]);
      sum->left_pos[k] = i;
      sum->right_pos[k] = j;
       ...
       */
      sum->x[total-1-k] = std::max(shape1->x[m-1-i], shape2->x[n-1-j]);
...
int32_t recursive_sizing (tree_node_t * node, int32_t pos,
  ...
  if (node->label.unit >= 0) {
    ...
    if (node->label.unit == CUT_VERTICAL) {
       ...
       */
      if (self->y[pos] > std::max(y1, y2)) {
        left->y[self->left_pos[pos]] += (self->y[pos] - std::max(y1, y2));
        right->y[self->right_pos[pos]] += (self->y[pos] - std::max(y1, y2));
        y1 = left->y[self->left_pos[pos]];
        y2 = right->y[self->right_pos[pos]];
      }
      if (self->x[pos] > (x1 + x2)) {
      ...
      flp->units[idx].bottomy = bottomy + std::min(y1, y2);
       ...
       */
      if (compact_dead && fabs(y2 - y1) / std::min(y1, y2) <= compact_ratio) {
#if VERBOSE > 1
      ...
      if (self->x[pos] > std::max(x1, x2)) {
        left->x[self->left_pos[pos]] += (self->x[pos] - std::max(x1, x2));
        right->x[self->right_pos[pos]] += (self->x[pos] - std::max(x1, x2));
        x1 = left->x[self->left_pos[pos]];
        x2 = right->x[self->right_pos[pos]];
      }
      if (self->y[pos] > (y1 + y2)) {
      ...
      flp->units[idx].leftx = leftx + std::min(x1, x2);
      flp->units[idx].bottomy = bottomy + ((x2 >= x1) ? 0 : y1);

      if (compact_dead && fabs(x2 - x1) / std::min(x1, x2) <= compact_ratio) {
#if VERBOSE > 1
        *compacted_area += (flp->units[idx].width * flp->units[idx].height);
#endif
        if (x2 >= x1)
