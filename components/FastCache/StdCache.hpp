...
private:
  ...
  public:
    ...
    StdLookupResult(PhysicalMemoryAddress addr, CoherenceState_t state, block_set_t *set,
                    tag_iterator block, StdCache *cache)
...
public:
  StdCache(const std::string &aName, int32_t aBlockSize, int32_t aNumSets, int32_t anAssociativity,
           evict_function_t anEvict, invalidate_function_t aSendInvalidate, int32_t anIndex,
           Flexus::SharedTypes::tFillLevel aLevel, std::string aReplPolicy, bool aTextFlexpoints) {
    theName = aName;
    theNumSets = aNumSets;
  ...
  void updateState(StdLookupResult &lookup, CoherenceState_t new_state, bool make_MRU,
                   bool make_LRU) {
  ...
  virtual LookupResult_p lookup(uint64_t tagset) {
    ...
    if (t_block == t_end) {
      ...
      return StdLookupResult_p(
          new StdLookupResult(PhysicalMemoryAddress(tag), kInvalid, set, t_block, this));
    }
    ...
    if (theTextFlexpoints) {
      ...
      for (int32_t set = 0; set < theNumSets; set++) {
        ...
        for (; block != end; block++, way++) {
          ...
          DBG_(Trace, (<< theName << " - Saving block " << std::hex << block->tag << " with state "
      ...
      for (int32_t set = 0; set < theNumSets; set++) {
        ...
        for (; block != end; block++, way++) {
          ...
          oa << bs;
          DBG_(Trace, Addr(block->tag)(<< theName << ": saving block " << std::hex << block->tag
    ...
    if (theTextFlexpoints) {
      ...
      for (int32_t set = 0; set < theNumSets; set++) {
        ...
        for (int32_t j = 0; j < theAssoc; j++) {
          ...
          DBG_(Trace, (<< theName << " - Loading block " << std::hex
                       << ((load_tag << shift) | (set << blockShift)) << " with state "
                       << state2String(state) << " in way " << j));
      ...
      for (int32_t set = 0; set < theNumSets; set++) {
        for (int32_t way = 0; way < theAssoc; way++) {
          ...
          DBG_(Trace, (<< theName << " - Loading block " << std::hex << bs.tag << " in state "
                       << (char)bs.state));
