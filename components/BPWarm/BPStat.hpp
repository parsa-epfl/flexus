...
struct BPWarm_stats {
  ...
  BPWarm_stats(std::string const &theName, int unres_branches)
      : theBP_PredMispredict(theName + "-BP_D_PredMispredict"),
        ...
        theBP_ICallInsn(theName + "-BP_ICall:Insn"), theBP_IRetInsn(theName + "-BP_IReturn:Insn"),
        ...
        offsetFreq(theName + "-offsetFreq"), targetFreq(theName + "-targetFreq") {
    char stat_name[50];
    for (int i = 0; i < 10; i++) {
  ...
  void offsetStats(VirtualMemoryAddress anAddress, eBranchType anActualType,
    ...
    //      DBG_( Tmp, ( << " target: " <<std::hex << anActualTarget << " pc: " << anAddress << "
    //      disp " << std::hex << disp << " dispDec " << disp_decoded));
    if (disp == 0) {
      //    	  DBG_( Tmp, ( << " pc: " << anAddress << " btype " << anActualType << " opcode " <<
      //    std::hex << anOpcode));
      assert(anActualType == kConditional);
    }
    ...
    //	  DBG_( Tmp, ( << "current block " << std::hex << (current_block << 6) << " prev block " <<
    //(branch_block << 6) << " diff " << std::dec << jumpDist << " btype " << anActualType));
  ...
  ICacheMissType Imiss_by_TakenBranch(eBranchType theFetchType,
                                      VirtualMemoryAddress theFetchAddress,
                                      BPredState theFetchState, bool is_mispredict) {
    ...
    if (branch_resolve.is_direction_correct && branch_resolve.is_target_correct &&
        branch_resolve.is_pred_correct)
      assert(is_mispredict == 0);
    else if (is_mispredict == 0) {
      DBG_(Tmp, (<< " Problem is " << branch_resolve.is_direction_correct << " "
                 << branch_resolve.is_target_correct << " " << branch_resolve.is_pred_correct));
      ...
      if (branch_resolve.is_pred_correct) {
        if (branch_resolve.is_direction_correct) {
          if (branch_resolve.is_target_correct) {
            theBP_CondCorrect++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Conditional Miss:Correct" ));
          } else {
            theBP_CondMispredict_Target++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Conditional Miss:TragetWrong" ));
          }
        } else {
      ...
      if (branch_resolve.is_pred_correct) {
        if (branch_resolve.is_direction_correct) {
          if (branch_resolve.is_target_correct) {
            theBP_UncondCorrect++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Unconditional Miss:Correct" ));
          } else {
            theBP_UncondMispredict++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Unconditional Miss:TargetWrong" ));
          }
        } else {
          assert(0);
        }
      ...
      if (branch_resolve.is_pred_correct) {
        if (branch_resolve.is_direction_correct) {
          if (branch_resolve.is_target_correct) {
            theBP_CallCorrect++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Call Miss:Correct" ));
          } else {
            theBP_CallMispredict++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Call Miss:TargetWrong" ));
          }
        } else {
          assert(0);
        }
      ...
      if (branch_resolve.is_pred_correct) {
        if (branch_resolve.is_direction_correct) {
          if (branch_resolve.is_target_correct) {
            theBP_IndCorrect++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Ind Miss:Correct" ));
          } else {
            theBP_IndMispredict++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Ind Miss:TargetWrong" ));
          }
        } else {
          assert(0);
        }
      ...
      if (branch_resolve.is_pred_correct) {
        if (branch_resolve.is_direction_correct) {
          if (branch_resolve.is_target_correct) {
            theBP_RetCorrect++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Ret Miss:Correct" ));
          } else {
            theBP_RetMispredict++;
            //  					DBG_( Tmp, Addr(aMessage.address()) ( << "
            //  Received on Port InsnMissIn[" << anIndex << "] Ret Miss:TargetWrong" ));
          }
        } else {
          assert(0);
        }
  ...
  void icache_sequential_run_test(uint64_t pc) {
    ...
    if (icache_block_address > last_icache_block_address ||
  ...
  BranchResolve Imiss_BP_resolution(eBranchType anActualType, uint32_t anActualTarget,
                                    BPredState &aBPState) {
    ...
    if (anActualType != aBPState.thePredictedType) {
      ...
      if (anActualType == kConditional) {
        if ((aBPState.thePrediction >= kNotTaken) && (anActualDirection >= kNotTaken)) {
          // Correct but should never enter here as the branch in not taken and are counting
