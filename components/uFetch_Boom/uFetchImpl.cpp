...
struct SimCache {
  ...
  uint64_t insert(uint64_t addr) {
    ...
    //    if (ret_val) {
    //    	DBG_ (Iface, ( << "Evicting " << (ret_val) ));
    //    } else {
    //    	DBG_ (Iface, ( << "Evicting nothing " ));
    //    }
    return ret_val;
  }
...
class FLEXUS_COMPONENT(uFetch) {
  ...
  std::vector<std::list<VirtualMemoryAddress>> thePBQC; // Rakesh: Prefetch block queue

  std::vector<std::set<PhysicalMemoryAddress>>
      theOutstandingMissQ; // Rakesh: queue for outstanding misses
  ...
  std::list<MemoryTransport> theFDIPPrefetchQueueCritical;
  ...
  std::set<PhysicalMemoryAddress> theOutstandingFDIPCriticalMisses;
  std::set<PhysicalMemoryAddress> blocks;
private:
...
public:
  FLEXUS_COMPONENT_CONSTRUCTOR(uFetch)
      : base(FLEXUS_PASS_CONSTRUCTOR_ARGS), theFetchAccesses(statName() + "-FetchAccess"),
        ...
        theMaxOutstandingEvicts(statName() + "-MaxEvicts"), theSeqMiss(statName() + "-Miss:Seq"),
        ...
        theOthers(statName() + "-Miss:Others"), theSeqMissP(statName() + "-Miss:Seq:NoPrefetch"),
        ...
        FDIPPrefetchesSquashed(statName() + "-FDIPPrefetchesSquashed"), theLastVTagSet(0),
        theLastPhysical(0), theAvailableFetchSlots(statName() + "-FetchSlotsPossible"),
  ...
  void push(interface::RASOpsIn const &, index_t anIndex,
    ...
    //	  DBG_( Iface, ( << " reconstruct RAS uFetch" ));

    for (std::list<FetchAddr>::iterator it = theFAQ[anIndex].begin(); it != theFAQ[anIndex].end();
         it++) {
      boost::intrusive_ptr<BPredState> bpState = it->theBPState;
      if (bpState->thePredictedType == kCall || bpState->thePredictedType == kReturn ||
          bpState->thePredictedType == kIndirectCall || bpState->thePredictedType == kIndirectReg) {
        //	    	  DBG_( Iface, ( << " RAS Decode " << bpState->pc << " pred type "
  ...
  void push(interface::PrefetchAddressIn const &, index_t anIndex,
            boost::intrusive_ptr<FetchCommand> &aCommand) {
    std::copy(aCommand->theFetches.begin(), aCommand->theFetches.end(),
  ...
  void push(interface::RecordedMissIn const &, index_t anIndex,
            boost::intrusive_ptr<RecordedMisses> &rMisses) {
    while (rMisses->theMisses.size() > 0) {
      VirtualMemoryAddress preFetch_addr = rMisses->theMisses.front();
      std::list<VirtualMemoryAddress>::iterator findIter = std::find(
          theRecordedMissQ[anIndex].begin(), theRecordedMissQ[anIndex].end(), preFetch_addr);
      if (findIter == theRecordedMissQ[anIndex].end()) {
        theRecordedMissQ[anIndex].push_back(preFetch_addr);
      }
      rMisses->theMisses.pop_front();
      //		  DBG_(Iface, ( << "Received recoreded miss " << preFetch_addr));
    }
  }
  ...
  int32_t pull(interface::AvailableFAQOut const &, index_t anIndex) {
    ...
    if (my_size >= cfg.FAQSize)
      return 0;
    else
      return cfg.FAQSize - my_size;
  }
  ...
  void push(interface::SquashIn const &, index_t anIndex, eSquashCause &aReason) {
    ...
    //    DBG_( Iface, Comp(*this) ( << std::endl << std::endl <<  "Squashing CPU[" <<
    //    std::setfill('0') << std::setw(2) << flexusIndex() << "." << anIndex << "] Fetch SQUASH: "
    //    << aReason << std::endl << std::endl));
    ...
    thePBQC[anIndex].clear();
    theOutstandingFDIPCriticalMisses.clear(); // Ali
  ...
  void push(interface::SquashBranchIn const &, index_t anIndex,
    ...
    //	  DBG_( Iface, Comp(*this) ( << std::endl << std::endl <<  "Squashing Mispredicted
    // branch CPU[" << std::setfill('0') << std::setw(2) << flexusIndex() << "." << anIndex << "]
    // Fetch SQUASH: " << std::endl << std::endl));
  ...
  void push(interface::ChangeCPUState const &, index_t anIndex, CPUState &aState) {
    DBG_(Iface, Comp(*this)(<< "CPU[" << std::setfill('0') << std::setw(2) << flexusIndex() << "."
                            << anIndex << "] Change CPU State. PSTATE: " << std::hex
                            << aState.thePSTATE << std::dec));
...
private:
  ...
  void prefetchNext(index_t anIndex) {
    ...
    if (cfg.PrefetchEnabled && theLastPrefetchVTagSet[anIndex]) {
      ...
      if (!xlat.thePaddr) {
        ...
        DBG_(Iface, (<< "NLP translation failed "));
        theLastPrefetchVTagSet[anIndex] = boost::none;
        return;
      } else {
        if (theOutstandingMissQ[anIndex].find(xlat.thePaddr) !=
            theOutstandingMissQ[anIndex].end()) {
          DBG_(Iface, (<< "NLP: FDIP prefetch on way " << std::hex << vprefetch));
          ...
          DBG_(Iface,
               (<< "NLP: issue prefetch " << std::hex << vprefetch << " paddr " << xlat.thePaddr
                << " outreq " << theOutstandingMissQ[anIndex].size()));
  ...
  bool icacheLookup(index_t anIndex, VirtualMemoryAddress vaddr) {
    ...
    if (tagset == theLastVTagSet) {
      ...
      if (paddr == 0) {
        ...
        return true; // Failed translations are treated as hits - they will cause an ITLB miss in
                     // the pipe.
      }
    } else {
      Flexus::SharedTypes::Translation xlat;
    ...
    if (theIcachePrefetch[anIndex] && *theIcacheMiss[anIndex] == *theIcachePrefetch[anIndex]) {
      ...
      theTransientHits++;
      DBG_(Iface,
           (<< "Miss: NLP prefetch on way " << std::hex << (tagset << theIndexShift)));
      prefetchNext(anIndex);
    } else if (theOutstandingMissQ[anIndex].find(*theIcacheMiss[anIndex]) !=
               theOutstandingMissQ[anIndex].end()) {
      // We have already sent a prefetch request out for this miss. No need
      // to request again.
      theTransientHits_FDIPLines++;
      DBG_(Iface,
           (<< "Miss: FDIP prefetch on way " << std::hex << (tagset << theIndexShift)));
    } else {
      theIcachePrefetch[anIndex] = boost::none;
      // Need to issue the miss.
      // theOutstandingMissQ[anIndex].insert(*theIcacheMiss[anIndex]);
      DBG_(Iface,
           (<< "Miss: issue fetch " << std::hex << *theIcacheMiss[anIndex] << " vaddr "
            << *theIcacheVMiss[anIndex] << " outreq " << theOutstandingMissQ[anIndex].size()));
  ...
  void sendMisses() {
    while (!theMissQueue.empty() && FLEXUS_CHANNEL(FetchMissOut).available()) {
      ...
      if (!trans[MemoryMessageTag]->isEvictType()) {
        ...
        if (theEvictSet.find(temp) != theEvictSet.end()) {
          DBG_(Trace, Comp(*this)(<< "Trying to fetch block while evict in process, stalling miss: "
      ...
      DBG_(Iface, (<< "Sending L1i miss " << *trans[MemoryMessageTag]));
    ...
    if (!theFDIPPrefetchQueueCritical.empty() && FLEXUS_CHANNEL(FetchMissOut).available()) {
      MemoryTransport trans = theFDIPPrefetchQueueCritical.front();
      bool evictInProcess = false;
      ...
      if (!evictInProcess) {
        ...
        DBG_(Iface, (<< "Sending FDIP Critical L1I Prefetch " << *trans[MemoryMessageTag]));
        trans[MemoryMessageTag]->coreIdx() = flexusIndex();
        FLEXUS_CHANNEL(FetchMissOut) << trans;
        theFDIPPrefetchQueueCritical.pop_front();
      }
    }
    ...
    if (!theFDIPPrefetchQueue.empty() && FLEXUS_CHANNEL(FetchMissOut).available()) {
      ...
      if (!evictInProcess) {
        ...
        DBG_(Iface, (<< "Sending FDIP L1I Prefetch " << *trans[MemoryMessageTag]));
        trans[MemoryMessageTag]->coreIdx() = flexusIndex();
        FLEXUS_CHANNEL(FetchMissOut) << trans;
  ...
  void fetchReply(MemoryTransport &aTransport) {
    ...
    case MemoryMessage::MissReplyWritable: {
      ...
      DBG_(Iface,
           (<< "Miss reply: for " << reply->address() << " replacing " << replacement));
      ...
      for (uint32_t i = 0; i < cfg.Threads; ++i) {
        if (theIcacheMiss[i] && *theIcacheMiss[i] == reply->address()) {
          ...
          if (aTransport[TransactionTrackerTag] && aTransport[TransactionTrackerTag]->fillLevel()) {
            ...
            DBG_(Iface, (<< "Fetchreply: for " << reply->address()));
          } else {
        ...
        if (theIcachePrefetch[i] && *theIcachePrefetch[i] == reply->address()) {
          DBG_(Iface, Comp(*this)(<< "CPU[" << std::setfill('0') << std::setw(2) << flexusIndex()
                                  << "." << i << "] L1I NLP PREFETCH-FILL " << reply->address()));
          theIcachePrefetch[i] = boost::none;
          DBG_(Iface, (<< "NLPreply: for " << reply->address()));
        }
        ...
        std::set<PhysicalMemoryAddress>::iterator itr =
            theOutstandingMissQ[anIndex].find(reply->address());
        if (itr != theOutstandingMissQ[anIndex].end()) {
          DBG_(Iface, Comp(*this)(<< "CPU[" << std::setfill('0') << std::setw(2) << flexusIndex()
                                  << "." << i << "] L1I FDIP PREFETCH-FILL " << reply->address()));
        ...
        if(theOutstandingFDIPCriticalMisses.find(reply->address()) != theOutstandingFDIPCriticalMisses.end()){
          ...
          if(theOutstandingFDIPCriticalMisses.empty()){
            bool isHit = true;
            FLEXUS_CHANNEL_ARRAY(BTBMissFetchReplyOut, anIndex) << isHit; 
          }
        }
      }
      ...
      if (cfg.SendAcks && reply->ackRequired()) {
        ...
        //   we received a reply from a peer cache (if the L2 had a copy, it would have replied
        //   instead of Fwding the request)
    ...
    case MemoryMessage::EvictAck: {
      ...
      //        DBG_(Iface, ( << "Removing from evict queue " << reply->address()));
  ...
  void issueEvict(PhysicalMemoryAddress anAddress) {
    if (cfg.CleanEvict && anAddress != 0) {
      ...
      //      DBG_(Iface, ( << "Moving to evict queue " << anAddress << " status " <<
      //      ret.second));
  ...
  void issuePreFetchCritical(PhysicalMemoryAddress anAddress, VirtualMemoryAddress vPC) {
    DBG_Assert(anAddress != 0);
    MemoryTransport transport;
    boost::intrusive_ptr<MemoryMessage> operation(MemoryMessage::newFetch(anAddress, vPC));
    operation->reqSize() = 64;
    ...
    theFDIPPrefetchQueueCritical.push_back(transport);
  }
  ...
  void missStats(index_t anIndex, FetchAddr fetch_addr) {
    ...
    DBG_(Iface,
         Comp(*this)(<< " Miss " << std::hex << fetch_addr.theAddress << " block " << std::hex
                     << (fetch_addr.theAddress & theBlockMask) << " reset distance "
                     << lastResetDistance << " prefetch on way " << !demandFetchIssued << " size "
                     << theLastFetches[anIndex].size()));
    ...
    if (fetch_addr.theAddress == theLastFetches[anIndex][1].theAddress + 4) {
      DBG_(Iface, (<< "Seq Miss " << lastResetDistance));
      theSeqMiss++;
      (*lastResetSeq[lastResetDistance])++;
      if (demandFetchIssued) {
        DBG_(Iface, (<< "Seq Miss NP " << lastResetDistance));
        (*lastResetSeqNP[lastResetDistance])++;
        theSeqMissP++;
      }
    } else if (theLastFetches[anIndex][0].theBPState &&
               theLastFetches[anIndex][0].theBPState->thePredictedType != kNonBranch &&
               theLastFetches[anIndex][0].theBPState->thePrediction <= kTaken) {
      theBranch++;
      bool is_indirect = false;
      if (theLastFetches[anIndex][0].theBPState->thePredictedType == kCall ||
          theLastFetches[anIndex][0].theBPState->thePredictedType == kIndirectReg ||
          theLastFetches[anIndex][0].theBPState->thePredictedType == kIndirectCall) {
        // MARK: in ARM64, BLR and BR are both register indirect, one is a call that sets x30, one
        // is not. See ARMv8 ISA Manual: C6.2.32 BLR and C6.2.34 BR
        is_indirect = true;
        theBranchMissIndirect++;
      }
      (*lastResetBranch[lastResetDistance])++;
      if (demandFetchIssued) {
        DBG_(Iface, (<< "Branch Miss "));
    ...
    if (lastResetDistance < 30) {
      ...
      DBG_(Iface, (<< "Miss squash reason " << lastSquashCause));
      ...
      if (lastSquashCause == kBranchMispredict) {
        ...
        DBG_(Iface, (<< "Sending miss pair pc " << lastSquashedBPState->pc
                              << " missed block " << missPair.second << " predicted type "
                              << lastSquashedBPState->thePredictedType << " "
                              << Flexus::Qemu::Processor::getProcessor(flexusIndex())
                                     ->disassemble(lastSquashedBPState->pc)));
        FLEXUS_CHANNEL(MissPairOut) << missPair;

        assert(lastSquashedBPState->thePredictedType < 10);
        DBG_(Iface, (<< "Miss squash Serial " << lastSquashedBPState->theSerial << " pc "
                              << lastSquashedBPState->pc << " miss prediected "
                              << lastSquashedBPState->thePredictedType << " actual type "
                              << lastSquashedBPState->theActualType << " prefetch on way "
                              << !demandFetchIssued << " "
                              << Flexus::Qemu::Processor::getProcessor(flexusIndex())
                                     ->disassemble(lastSquashedBPState->pc)));
        ...
        if (lastSquashedBPState->thePredictedType == kConditional) {
          if (lastSquashedBPState->bimodalPrediction) {
            assert(lastSquashedBPState->saturationCounter >= 0 &&
                   lastSquashedBPState->saturationCounter < 4);
            (*bimodalConfindence[lastSquashedBPState->saturationCounter])++;
          } else {
            assert(lastSquashedBPState->saturationCounter >= 0 &&
                   lastSquashedBPState->saturationCounter < 8);
  ...
  void doFetch(index_t anIndex) {
    ...
    //	  DBG_(Iface, ( << std::endl<< "Entering uFetch " << theFlexus->cycleCount() <<
    // std::endl) );
    if (theIcacheMiss[anIndex]) {
      //    	DBG_(Iface, ( << "Handling I-cache Miss"));
      ++theMissCycles;
      return;
    }
    ...
    //    DBG_(Iface, ( << "AvailableFIQ " << available_fiq << " FAQ Size " <<
    //    theFAQ[anIndex].size()));    
    if (waitingForOpcodeQueue->theOpcodes.size() < theMissQueueSize && available_fiq > 0 &&
      ...
      DBG_(Iface, (<< "AvailableFIQ " << available_fiq << " max addr " << remaining_fetch
                            << " limit " << cfg.MaxFetchInstructions));
      while (remaining_fetch > 0 && (theFAQ[anIndex].size() > 1 || theFlexus->quiescing())) {
        ...
        DBG_(Iface, (<< "Fetching " << fetch_addr.theAddress << " block " << block_addr));
        if (available_lines.count(block_addr) == 0) {
          ...
          if (available_lines.size() >= cfg.MaxFetchLines) {
            ...
            DBG_(Iface, (<< "Reached I-cache reads per cycle limit "));
            break;
          }
        ...
        DBG_(Iface,
             (<< "Fetched " << fetch_addr.theAddress << " " << std::hex << op_code << " "
              << Flexus::Qemu::Processor::getProcessor(flexusIndex())
                     ->disassemble(fetch_addr.theAddress)));
        DBG_(Verb, (<< "Fetched " << fetch_addr.theAddress));

        theFAQ[anIndex].pop_front();
        waitingForOpcodeQueue->theOpcodes.emplace_back(FetchedOpcode(
            ...
            fetch_addr.theBPState, theFetchReplyTransactionTracker[anIndex], aFetchSerial,
            nullptr));

        /* TODO: determine how the "op_code" was being used. Cannot be resolved here in a real
        design. bundle->theOpcodes.push_back(FetchedOpcode( fetch_addr.theAddress,
            theFAQ[anIndex].empty() ? VirtualMemoryAddress(0) : theFAQ[anIndex].front().theAddress,
            op_code, fetch_addr.theBPState, theFetchReplyTransactionTracker[anIndex], aFetchSerial,
            nullptr // no InfoMissStats
            ));
          */
        if (from_icache && theLastMiss[anIndex] && theLastPhysical == theLastMiss[anIndex]->first) {
          waitingForOpcodeQueue->theFillLevels.push_back(theLastMiss[anIndex]->second);
          theLastMiss[anIndex] = boost::none;
        } else {
          waitingForOpcodeQueue->theFillLevels.push_back(eL1I);
        }
    ...
    processBundle();
    //    DBG_(Iface, ( << std::endl<< "Leaving uFetch " << std::endl) );
  }
  ...
  uint32_t fetchFromQEMU(index_t anIndex, VirtualMemoryAddress const &anAddress) {
    ...
    DBG_(Verb, Comp(*this)(<< "Fetch " << anAddress << " op: " << std::hex << std::setw(8)
                           << op_code << std::dec));
    return op_code;
  }
  ...
  void doPrefetch(index_t anIndex) {
    ...
    //	  DBG_(Iface, ( << std::endl<< "Entering uPrefetch " << std::endl) );
    ...
    while (thePAQ[anIndex].size() > 0) {
      ...
      if (available_lines.find(block_addr) == available_lines.end()) {
        available_lines.insert(block_addr);
        DBG_(Iface, (<< "FDIP: potential prefetch " << block_addr));

        std::list<VirtualMemoryAddress>::iterator findIter =
            std::find(thePBQ[anIndex].begin(), thePBQ[anIndex].end(), block_addr);
        //	    	std::list<VirtualMemoryAddress>::iterator itr =
        // std::find(theOutstandingMissVQ[anIndex].begin(), theOutstandingMissVQ[anIndex].end(),
        // block_addr);
        /*if (itr != theOutstandingMissVQ[anIndex].end()) {
                //Outstanding request, no need to prefetch
                DBG_(Iface, ( << "FDIP: already outstanding " << std::hex << block_addr));
        } else*/
        if (findIter != thePBQ[anIndex].end()) {
          DBG_(Iface, (<< "FDIP: already in PBQ " << std::hex << block_addr));
        } else {
          DBG_(Iface, (<< "FDIP: push in PBQ " << std::hex << block_addr));
    ...
    enqueuePrefetchCritical(anIndex);
  ...
  void enqueuePrefetch(index_t anIndex) {
    if (thePBQ[anIndex].size()) {
      ...
      DBG_(Iface, (<< "FDIP: check block " << std::hex << prefetch_addr));
      ...
      if (!paddr) {
        DBG_(Iface, (<< "NLP translation failed"));
        // Unable to translate for prefetch. Fixme: should we just move to the next request or keep
        // trying translation
        return;
      } else {
        if (theOutstandingMissQ[anIndex].find(paddr) != theOutstandingMissQ[anIndex].end()) {
          // Fetch already sent
          DBG_(Iface, (<< "FDIP: Outstanding req " << std::hex << prefetch_addr));
          thePBQ[anIndex].pop_front();
          //	    		assert(0);
        } else if (theIcachePrefetch[anIndex] && paddr == *theIcachePrefetch[anIndex]) {
          thePBQ[anIndex].pop_front();
          DBG_(Iface, (<< "FDIP: NLP request already sent " << std::hex << prefetch_addr));
          // Prefetch already sent
        } else if (theIcacheMiss[anIndex] && paddr == *theIcacheMiss[anIndex]) {
          thePBQ[anIndex].pop_front();
          DBG_(Iface,
               (<< "FDIP: demand fetch already sent " << std::hex << prefetch_addr));
          // Fetch already sent
        } else if (lookup(paddr)) {
          thePBQ[anIndex].pop_front();
          DBG_(Iface,
               (<< "FDIP: already in cache " << std::hex << prefetch_addr << " paddr " << paddr));
          // No need to prefetch, already in cache
        } else {
          if (theOutstandingMissQ[anIndex].size() < theMaxOutstandingFDIPMisses) {
            ...
            DBG_(Iface,
                 (<< "FDIP: issue prefech " << std::hex << paddr << " vaddr " << prefetch_addr
                  << " outreq " << theOutstandingMissQ[anIndex].size()));
            thePBQ[anIndex].pop_front();
          } else {
            DBG_(Iface, (<< "FDIP: max outstanding req " << prefetch_addr));
          }
        }
      }
    }
  ...
  void enqueuePrefetchCritical(index_t anIndex) {
    bool missed = true;
    ...
    if (thePBQC[anIndex].size()) {
      ...
      DBG_(Iface, (<< "FDIP Critical: check block " << std::hex << prefetch_addr));
      ...
      if (!paddr) {
        ...
        if (theOutstandingMissQ[anIndex].find(paddr) != theOutstandingMissQ[anIndex].end()) {
          // Fetch already sent
          DBG_(Iface, (<< "FDIP Critical: Outstanding req " << std::hex << prefetch_addr));
          thePBQC[anIndex].pop_front();
          theOutstandingFDIPCriticalMisses.insert(paddr);
          //	    		assert(0);
        } else if (theIcachePrefetch[anIndex] && paddr == *theIcachePrefetch[anIndex]) {
          thePBQC[anIndex].pop_front();
          theOutstandingFDIPCriticalMisses.insert(paddr);
          DBG_(Iface, (<< "FDIP Critical: NLP request already sent " << std::hex << prefetch_addr));
          // Prefetch already sent
        } else if (theIcacheMiss[anIndex] && paddr == *theIcacheMiss[anIndex]) {
          thePBQC[anIndex].pop_front();
          theOutstandingFDIPCriticalMisses.insert(paddr);
          DBG_(Iface,
               (<< "FDIP Critical: demand fetch already sent " << std::hex << prefetch_addr));
          // Fetch already sent
        } else if (lookup(paddr)) {
          thePBQC[anIndex].pop_front();
          DBG_(Iface,
               (<< "FDIP Critical: already in cache " << std::hex << prefetch_addr << " paddr " << paddr));
          missed = false;
          // No need to prefetch, already in cache
        } else {
          if (theOutstandingMissQ[anIndex].size() < theMaxOutstandingFDIPMisses) {
            theOutstandingMissQ[anIndex].insert(paddr);
            issuePreFetchCritical(paddr, prefetch_addr);
            theFDIPPrefetches++;

            DBG_(Iface,
                 (<< "FDIP Critical: issue prefech " << std::hex << paddr << " vaddr " << prefetch_addr
                  << " outreq " << theOutstandingMissQ[anIndex].size()));
            thePBQC[anIndex].pop_front();
            theOutstandingFDIPCriticalMisses.insert(paddr);
          } else {
            DBG_(Iface, (<< "FDIP Critical: max outstanding req " << prefetch_addr));
          }
        }
      }
  ...
  void enqueueRecMissPrefetch(index_t anIndex) {
    if (theRecordedMissQ[anIndex].size()) {
      ...
      //		DBG_(Iface, ( << "Rec miss: check block " << std::hex <<
      // prefetch_addr));
      ...
      if (!paddr) {
        ...
        if (theOutstandingMissQ[anIndex].find(paddr) != theOutstandingMissQ[anIndex].end()) {
          // Fetch already sent
          //	    		DBG_(Iface, ( << "recmiss: Outstanding req " << std::hex <<
          // prefetch_addr));
          theRecordedMissQ[anIndex].pop_front();
          //	    		assert(0);
        } else if (theIcachePrefetch[anIndex] && paddr == *theIcachePrefetch[anIndex]) {
          theRecordedMissQ[anIndex].pop_front();
          //				DBG_(Iface, ( << "FDIP: NLP request already sent "
          //<< std::hex
          //<< prefetch_addr)); Prefetch already sent
        } else if (theIcacheMiss[anIndex] && paddr == *theIcacheMiss[anIndex]) {
          theRecordedMissQ[anIndex].pop_front();
          //				DBG_(Iface, ( << "FDIP: demand fetch already sent "
          //<< std::hex
          //<< prefetch_addr)); Fetch already sent
        } else if (lookup(paddr)) {
          theRecordedMissQ[anIndex].pop_front();
          //				DBG_(Iface, ( << "recmiss: already in cache " <<
          // std::hex
          ...
          if (theRecMissPrefetchQueue.size() < theMaxOutstandingRecMisses) {
            ...
            //				  DBG_(Iface, ( << "recmiss: issue prefech " <<
            // std::hex
            //<< paddr
            //<< " vaddr " << prefetch_addr << " outreq " << theOutstandingMissQ[anIndex].size()));
            theRecordedMissQ[anIndex].pop_front();
          } else {
            //					DBG_(Iface, ( << "recmiss: max outstanding
            // req
            //"
            //<< prefetch_addr
            //));
...
FLEXUS_PORT_ARRAY_WIDTH(uFetch, BTBRequestIn) {
  return (cfg.Threads);
}
FLEXUS_PORT_ARRAY_WIDTH(uFetch, BTBReplyOut) {
  return (cfg.Threads);
}
FLEXUS_PORT_ARRAY_WIDTH(uFetch, BTBMissFetchReplyOut) {
  return (cfg.Threads);
}
