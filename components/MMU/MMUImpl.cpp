...
void TLB::loadState(json checkpoint)
{
    ...
    for (size_t i = 0; i < set; ++i) {
        ...
        for (size_t j = 0; j < TLBSize; j++) {
            VirtualMemoryAddress aVaddr  = VirtualMemoryAddress((uint64_t)checkpoint["entries"][i].at(j)["vpn"] << 12);
            PhysicalMemoryAddress aPaddr = PhysicalMemoryAddress((uint64_t)checkpoint["entries"][i].at(j)["ppn"] << 12);
            uint16_t anASID               = (uint16_t)checkpoint["entries"][i].at(j)["asid"];
            bool aNG                     = (bool)checkpoint["entries"][i].at(j)["ng"];
            uint64_t index               = (uint64_t)(TLBSize - j - 1);
...
json TLB::saveState()
{
    ...
    for (size_t set_idx = 0; set_idx < theSets; ++set_idx) {
        ...
        for (const auto& entry : entries) {
            checkpoint["entries"][set_idx][i++] = { { "vpn", (uint64_t)entry.theVaddr >> 12 },
