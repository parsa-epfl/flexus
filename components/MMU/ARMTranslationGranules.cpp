...
TranslationGranule::TranslationGranule()
    : KBSize(4096), logKBSize(12), granuleShift(9), granuleEntries(512), PAddrWidth(48),
      IAddrOffset(48) {
} // 4KB default
...
TranslationGranule::TranslationGranule(unsigned ksize, unsigned PASize, unsigned IASize)
    : KBSize(ksize), logKBSize(log2(ksize)), granuleShift(logKBSize - 3),
      granuleEntries(exp2(granuleShift)), PAddrWidth(PASize), IAddrOffset(IASize) {
}

unsigned TranslationGranule::getKBSize() const {
  return KBSize;
}
unsigned TranslationGranule::getlogKBSize() const {
  return logKBSize;
}
unsigned TranslationGranule::getGranuleShift() const {
  return granuleShift;
}
unsigned TranslationGranule::getGranuleEntries() const {
  return granuleEntries;
}
uint8_t TranslationGranule::getPARange_Raw() const {
  return PARange_RawValue;
}
void TranslationGranule::setPARange_Raw(uint8_t aRawValue) {
  PARange_RawValue = aRawValue;
}
uint8_t TranslationGranule::getPAddrWidth() const {
  return PAddrWidth;
}
void TranslationGranule::setPAddrWidth(uint8_t aSize) {
  PAddrWidth = aSize;
}
uint8_t TranslationGranule::getIAddrOffset() const {
  return IAddrOffset;
}
void TranslationGranule::setIAddrOffset(uint8_t aSize) {
  IAddrOffset = aSize;
}
...
uint8_t TranslationGranule::getIAddrSize() const {
 ...
 */
uint8_t TranslationGranule::getInitialLookupLevel() const {
...
uint64_t TranslationGranule::GetLowerAddressRangeLimit() const {
  DBG_Assert(false);
  return 0;
}
uint64_t TranslationGranule::GetUpperAddressRangeLimit() const {
  DBG_Assert(false);
  return 0;
}
 ...
 */
uint64_t TG0_Granule::GetLowerAddressRangeLimit() const {
  return 0;
}
 ...
 */
uint64_t TG0_Granule::GetUpperAddressRangeLimit() const {
  uint8_t TnSz = getIAddrOffset();
 ...
 * Armv8 Ref Manual, page D4-2046
 */
uint64_t TG1_Granule::GetLowerAddressRangeLimit() const {
  uint8_t TnSz = getIAddrOffset();
