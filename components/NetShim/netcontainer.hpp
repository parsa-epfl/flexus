...
class NetContainer
{
  ...
  public:
    ...
    static bool attachSwitchChannels(NetContainer* nc, const int32_t sw, const int32_t port, const bool destination);

    static bool attachNodeChannels(NetContainer* nc, const int32_t node);

    bool networkEmpty(void) const { return (mslHead == nullptr); }

    int32_t getActiveMessages(void) const { return activeMessages; }
    ...
    bool isNodeAvailable(const int32_t node, const int32_t vc) { return isNodeAvailablePtr(node, vc); }

    bool isNodeOutputAvailable(const int32_t node, const int32_t vc) { return nodes[node]->isOutputAvailable(vc); }
    ...
    int32_t getNumNodes(void) const { return numNodes; }
