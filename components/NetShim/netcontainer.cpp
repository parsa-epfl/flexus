...
NetContainer::buildNetwork(const char* filename)
{
    ...
    if (!infile.good()) {
        std::cerr << "NetShim: error opening configuration/topology file: \"" << filename << "\".  Bailing out!"
                  << endl;
        return true;
    }
...
bool
NetContainer::validateParameters(void)
{
...
bool
NetContainer::readStringToken(istream& infile, char* str)
{
    // Skip whitespace
    ws(infile);
...
bool
NetContainer::readIntToken(istream& infile, int32_t& i)
{
    // Skip whitespace
    ws(infile);
...
bool
NetContainer::readTopologyDoubleToken(istream& infile, int32_t& tok1, int32_t& tok2, NetContainer* nc)
{
    char c;
...
bool
NetContainer::readTopologyTripleToken(istream& infile, int32_t& sw, int32_t& port, int32_t& vc, NetContainer* nc)
{
    char c;
    ...
    for (i = 0; i < (int)TOP_TOKEN_COUNT; i++) {
        if (strcasecmp(topTokens[i].token, firstTok) == 0) {
            ...
            if (topTokens[i].order == VALIDATE_BEFORE && numChannels != 0) {
                std::cerr << "ERROR: token " << firstTok << " must appear before topology and routing tokens!" << endl;
                return true;
...
NetContainer::drive(void)
{
    ...
    for (i = 0; i < numSwitches; i++) {
        if (switches[i]->driveInputPorts()) return true;
    }

    for (i = 0; i < numNodes; i++) {
        if (nodes[i]->driveInputPorts()) return true;
    }
...
bool
NetContainer::handleInclude(istream& infile, NetContainer* nc)
{
    char filename[100];

    if (readStringToken(infile, filename)) return true;
...
bool
NetContainer::handleChannelLatency(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->channelLatency);
}

bool
NetContainer::handleChannelLatencyData(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->channelLatencyData);
}

bool
NetContainer::handleChannelLatencyControl(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->channelLatencyControl);
}

bool
NetContainer::handleChannelBandwidth(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->channelBandwidth);
}

bool
NetContainer::handleLocalChannelLatencyDivider(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->localChannelLatencyDivider);
}

bool
NetContainer::handleNumNodes(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->numNodes);
}

bool
NetContainer::handleNumSwitches(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->numSwitches);
}

bool
NetContainer::handleSwitchInputBuffers(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->switchInputBuffers);
}

bool
NetContainer::handleSwitchOutputBuffers(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->switchOutputBuffers);
}

bool
NetContainer::handleSwitchInternalBuffersPerVC(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->switchInternalBuffersPerVC);
}

bool
NetContainer::handleSwitchBandwidth(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->switchBandwidth);
}

bool
NetContainer::handleSwitchPorts(istream& infile, NetContainer* nc)
{
    return readIntToken(infile, nc->switchPorts);
}

bool
NetContainer::readTopSrcDest(istream& infile,
                             bool& isSwitch,
                             int32_t& node,
                             int32_t& sw,
                             int32_t& port,
                             NetContainer* nc)
{
    ...
    if (strcasecmp(STR_NODE, str) == 0) {
        if (readIntToken(infile, node)) return true;
...
bool
NetContainer::handleTopology(istream& infile, NetContainer* nc)
{
    int
...
bool
NetContainer::attachSwitchChannels(NetContainer* nc, const int32_t sw, const int32_t port, const bool destination)
{
    const char* msg = "";
...
bool
NetContainer::attachNodeChannels(NetContainer* nc, const int32_t node)
{
    if (nc->nodes[node]->addFromChannel(nc->channels[nc->maxChannelIndex])) goto error;

    if (nc->nodes[node]->addToChannel(nc->channels[nc->maxChannelIndex + 1])) goto error;
...
bool
NetContainer::handleRoute(istream& infile, NetContainer* nc)
{
    char str[100];
    ...
    while (infile.peek() != '}') {
        ...
        if (vc >= MAX_NET_VC || vc < 0) {
            std::cerr << "ERROR: virtual channel number " << vc << " in route specification is invalid" << endl;
            return true;
        }

        if (nc->switches[sw]->addRoutingEntry(node, port, vc)) { return true; }
...
bool
NetContainer::allocateNetworkStructures(void)
{
    int i;
...
bool
NetContainer::validateNetwork(void)
{
    int i;
    ...
    for (i = 0; i < numNodes; i++) {
        if (nodes[i]->checkTopology()) foundErrors = true;
    }

    for (i = 0; i < numSwitches; i++) {
        if (switches[i]->checkTopology()) foundErrors = true;

        if (switches[i]->checkRoutingTable()) foundErrors = true;
    }
...
bool
NetContainer::insertMessage(MessageState* msg)
{
    ...
    if (nodes[msg->srcNode]->insertMessage(msg)) return true;

    return false;
}

bool
NetContainer::deliverMessage(MessageState* msg)
{
    assert(msg->myList != nullptr);

    // Deliver the message to the flexus node
    if (deliverMessagePtr(msg)) { return true; }
    ...
    if (activeMessages == 0) { assert(mslHead == nullptr && mslTail == nullptr); }
...
bool
NetContainer::dumpState(ostream& out, const int32_t flags)
{
    int i;
    ...
    if (flags & NS_DUMP_NODES) {
        for (i = 0; i < numNodes; i++) {
            if (nodes[i]->dumpState(out)) return true;
        }
    }

    if (flags & NS_DUMP_CHANNELS) {
        for (i = 0; i < maxChannelIndex; i++) {
            if (channels[i]->dumpState(out)) return true;
        }
    }

    if (flags & NS_DUMP_SWITCHES) {
        for (i = 0; i < numSwitches; i++) {
            if (switches[i]->dumpState(out, flags & NS_DUMP_SWITCHES)) return true;
        }
    }
