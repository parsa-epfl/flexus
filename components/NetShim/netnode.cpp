...
NetNode::drive(void)
{
    ...
    if (!messagesWaiting) return false;
    ...
    for (i = 0; i < MAX_VC; i++) {
        if (toNodePort->hasMessage(i) && nc->isNodeAvailable(nodeId, NETVC_TO_PROTVC(i))) {
            ...
            if (nc->deliverMessage(msg)) { return true; }
...
bool
NetNode::checkTopology(void) const
{
    int i;
    ...
    for (i = 0; i < MAX_NET_VC; i++) {
        if (!fromNodePort->isConnected() || !toNodePort->isConnected()) {
            ...
            std::cerr << "ERROR: node " << nodeId << " is missing channel connections (node unreachable)" << endl;
...
bool
NetNode::dumpState(ostream& out)
{
    out << "Node: " << nodeId << endl;
    if (fromNodePort->dumpState(out) || toNodePort->dumpState(out)) return true;
