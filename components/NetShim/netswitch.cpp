...
NetSwitch::NetSwitch(const int32_t name_,     // Name/id of the switch
    ...
    : name(name_), numNodes(numNodes_), numPorts(numPorts_), inputBufferDepth(inputBufferDepth_),
      outputBufferDepth(outputBufferDepth_), vcBufferDepth(vcBufferDepth_),
      crossbarBandwidth(crossbarBandwidth_), nextStartingPort(0) {
  int i, j;
...
bool NetSwitch::attachChannel(Channel *channel, const int32_t port, const bool isInput) {
  assert(channel);
  ...
  for (i = 0; i < numPorts * MAX_NET_VC; i++) {
    ...
    if (routingTable[msg->destNode][i] >= 0) {
      ...
      if (outputPorts[routingPort]->hasBufferSpace(BUILD_VC(msg->priority, routingVC))) {
        ...
        TRACE(msg, " SW" << name << " routing destination " << msg->destNode << " to port "
                         << routingPort << ":" << routingVC << " (priority = " << msg->priority);
        return false;
      }
...
bool NetSwitch::checkTopology(void) const {
  ...
  for (i = 0; i < numPorts; i++) {
    if (!inputPorts[i]->isConnected() || !outputPorts[i]->isConnected()) {
      ...
      std::cerr << "WARNING: switch " << name << " port " << i << " left unused (may be safe)"
                << endl;
      //      foundErrors = true;
    }
  }
  ...
  for (i = 0; i < numNodes; i++) {
    ...
    for (j = 0; j < numPorts * MAX_NET_VC; j++) {
      if (routingTable[i][j] != -1 && !outputPorts[routingTable[i][j]]->isConnected()) {
        ...
        std::cerr << "ERROR: routing table specifies route to node " << i << " exiting switch "
                  << name << " on unconnected port " << routingTable[i][j] << ":" << vcTable[i][j]
                  << endl;
        foundErrors = true;
      }
    }
...
bool NetSwitch::addRoutingEntry(const int32_t node, const int32_t port, const int32_t vc) {
  int i;
  ...
  std::cerr << "ERROR: too many routing table entries in switch " << name
            << " for destination node " << node << " (port " << port << ", vc " << vc << ")"
            << endl;
...
NetSwitchInternalBuffer::NetSwitchInternalBuffer(const int32_t bufferCount_, NetSwitch *netSwitch_)
    : currMessage(nullptr), currPriority(0), netSwitch(netSwitch_) {
  int i;
