...
class MRCMessage : public boost::counted_base
{
  ...
  public:
    const MRCMessageType type() const { return theType; }

    uint32_t node() const { return theNode; }

    PhysicalMemoryAddress address() const { return theAddress; }

    int64_t location() const { return theLocation; }

    int64_t tag() const { return theTag; }

    MRCMessage(MRCMessageType aType,
               uint32_t aNode,
