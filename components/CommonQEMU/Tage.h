...
// used for storing the history lengths
  ...
  PREDICTOR ()
  {
	...
	ch_t_runahead[1][i].init (ch_i_runahead[i].OLENGTH,
			 TBITS - ((i + (NHIST & 1)) / 2) - 1);
      }
...
//F serves to mix path history
  ...
  int gindex (address_t pc, int bank, bool is_runahead)
  {
	  ...
	  if (is_runahead) {
		if (m[bank] >= 16)
		  index =
		pc ^ (pc >> ((LOGG - (NHIST - bank - 1)))) ^ ch_i_runahead[bank].
		comp ^ F (phist_runahead, 16, bank);

		else
		  index =
		pc ^ (pc >> (LOGG - NHIST + bank + 1)) ^
		ch_i_runahead[bank].comp ^ F (phist_runahead, m[bank], bank);
...
//	    	  DBG_( Tmp, ( << " Base predict: " << (int)btable[BI].pred << " " << (int)btable[BI].hyst));
...
//	  DBG_(Tmp, ( << "Tage history prediciton"));
	  ...
	  aBPState.saturationCounter = gtable[bank][GI[bank]].ctr + 4/*To make the value positive (0 and 7) */;
	  return (gtable[bank][GI[bank]].ctr >= 0);
...
//	if (printLog)
//		std::cout << "Predicting pc " << std::hex << instruction_addr << " pred " << pred_taken << " bank " << bank << " alttaken " << alttaken << std::endl;
//	DBG_( Tmp, ( << " TAGE prdict: " << std::hex << instruction_addr << " " << pred_taken));
      }
...
//	std::cout << std::endl<< std::endl<< std::endl<< std::endl << "Tage predict " << pred_taken << " cond? " << aBPState.is_conditional << std::endl<< std::endl<< std::endl<< std::endl<< std::endl;
...
//  int Seed;
  ...
  void confirm_state(BPredState & aBPState)
  {
	  if (ghist_retired != aBPState.ghist) {
	  	  DBG_( Tmp, ( << " Ghist is different retired: " << ghist_retired << " carried " << aBPState.ghist));
	  	  assert(0);
	  } else if (phist_retired != aBPState.phist)
	  {
		  DBG_( Tmp, ( << " Phist is different retired: " << phist_retired << " carried " << aBPState.phist));
		  assert(0);
	  }
  }
  ...
  // PREDICTOR UPDATE
  void update_predictor (uint64_t instruction_addr, BPredState & aBPState,
			 bool taken)
  {
...
//    	if (printLog) {
//			std::cout << "UPdate pc " << std::hex << instruction_addr << " pred " << aBPState.pred_taken << " outcome " << taken << std::endl;
//			std::cout << "phist " << std::hex << aBPState.phist << " ghist " << aBPState.ghist << " BI " << aBPState.BI << " bank "<< aBPState.bank << " altpred "<< aBPState.alttaken << std::endl;
//    	}
//
	 ...
	 * We only check if the direction was correct and not the target as Tage only prediction the direction.
	 * If the direction was correct but not the target, Tage did its job and we do not was to allocate another entry.
	 */
...
// update the ubit counter
	 ...
	 * Therefore, we use "is_mispredict" variable instead of comparing the predicted and actual direction.
	 * Moved to "feedback" function in BranchPredictor.cpp
	 */
//	if(is_mispredict) {
...
//	  s << "\n";
	  ...
	  for (int i = 0; i < NHIST; i++){
		s << (uint32_t)ch_i[i].comp << " " << ch_i[i].CLENGTH << " " << ch_i[i].OLENGTH << " " << ch_i[i].OUTPOINT << "\n";
	  }
	  s << "\n";

	  for (int i = 0; i < NHIST; i++){
		s << (uint32_t)ch_t[0][i].comp << " " << ch_t[0][i].CLENGTH << " " << ch_t[0][i].OLENGTH << " " << ch_t[0][i].OUTPOINT << "\n";
	  }
	  s << "\n";

	  for (int i = 0; i < NHIST; i++){
		s << (uint32_t)ch_t[1][i].comp << " " << ch_t[1][i].CLENGTH << " " << ch_t[1][i].OLENGTH << " " << ch_t[1][i].OUTPOINT << "\n";
	  }
	  s << "\n";
...
//			s >> gtable[i][j].ctr >>  gtable[i][j].tag >> gtable[i][j].ubit;
  ...
  void print_state (std::ostream & s)
  const {
	  s << PWIN << "\n";
	  s << TICK << "\n";
	  s << Seed << "\n";
	  ...
	  for (int i = 0; i < NHIST; i++){
		for(int j = 0; j < (1<<LOGG); j++){
		  s << (int)gtable[i][j].ctr << " " << (uint32_t)gtable[i][j].tag << " " << (int)gtable[i][j].ubit << "\n";
