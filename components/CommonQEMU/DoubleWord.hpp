...
struct DoubleWord
{
    ...
    void set(uint64_t aValue, int32_t aSize, uint32_t anOffset)
    {
        switch (aSize) {
            ...
            case 8:
                ...
                theValidBits            = 0xFF;
                break;
            default: DBG_Assert(false, (<< "Unsupported size for DoubleWord::set"));
        }
    }
    ...
    bool isValid(int32_t aSize, uint32_t anOffset) const
    {
        uint8_t bits = 0;
        switch (aSize) {
            case 1: bits = 0x1 << (7 - anOffset); break;
            case 2: bits = 0x3 << (6 - anOffset); break;
            case 4: bits = 0xF << (4 - anOffset); break;
            case 8: bits = 0xFF; break;
    ...
    {
        if (!isValid(aSize, anOffset)) { return false; }
        switch (aSize) {
            case 1: return getByte(anOffset) == static_cast<uint8_t>(aValue);
            case 2: return getHalfWord(anOffset) == static_cast<uint16_t>(aValue);
            case 4: return getWord(anOffset) == static_cast<uint32_t>(aValue);
            case 8: return getDoubleWord(anOffset) == aValue;
            default: DBG_Assert(false, (<< "Unsupported size for DoubleWord::set")); return false;
        }
    }

    uint8_t& getByte(uint32_t anOffset) { return *(reinterpret_cast<uint8_t*>(&theDoubleWord) + (7 - anOffset)); }
...
bool
operator==(uint64_t compare, DoubleWord const& aDoubleWord);
bool
operator==(DoubleWord const& entry, uint64_t compare);
std::ostream&
operator<<(std::ostream& anOstream, DoubleWord const& aDoubleWord);
