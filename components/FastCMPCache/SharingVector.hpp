...
class SharingVector
{
  protected:
    ...
    SharingVector(const std::bitset<MAX_NUM_SHARERS>& s)
      : sharers(s)
    {
    }
  ...
  public:
    ...
    virtual int32_t countSharers() const { return sharers.count(); }
    ...
    virtual std::list<int> toList() const
    {
        std::list<int> l;
        for (int32_t i = 0; i < MAX_NUM_SHARERS; i++) {
            if (sharers[i]) { l.push_back(i); }
        }
        return l;
    }

    virtual const std::bitset<MAX_NUM_SHARERS>& getSharers() const { return sharers; }

    virtual const bool operator==(const SharingVector& a) const { return (sharers == a.sharers); }

    virtual const bool operator!=(const SharingVector& a) const { return (sharers != a.sharers); }

    virtual const SharingVector operator&(const SharingVector& a) const { return SharingVector(sharers & a.sharers); }
    ...
    virtual void clear() { sharers.reset(); }

    virtual void flip() { sharers.flip(); }

    virtual const bool any() const { return sharers.any(); }
