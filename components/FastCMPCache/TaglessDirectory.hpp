...
class TaglessDirectory : public AbstractDirectory<_State, _EState>
{
  public:
    class TaglessLookupResult : public AbstractLookupResult<_State>
    {
      private:
        ...
        TaglessLookupResult(MemoryAddress addr,
                            const std::set<int>& buckets,
                            std::vector<_State>& set,
                            int32_t num_sharers)
          : theAddress(addr)
          , theBucketList(buckets)
          , theSet(set)
          , theAggregateState(num_sharers)
        {
      ...
      public:
        ...
        virtual const _State& state() const { return theAggregateState; }
        ...
        {
            DBG_Assert(false,
                       (<< "Interface not supported by TaglessDirectory. Use "
                           "remove(sharer,msg) instead."));
        }
        void removeSharer(int32_t sharer, TaglessDirMsg_p msg)
        {
            if (msg->getConflictFreeBuckets().size() == 0) { return; }
        ...
        {
            DBG_Assert(false,
                       (<< "Interface not supported by TaglessDirectory. No "
  ...
  private:
    ...
    int32_t getBank(uint64_t addr) { return (addr >> theBankShift) & theBankMask; }
    int32_t getGroup(uint64_t addr) { return (addr >> theGroupShift) & theGroupMask; }
  ...
  public:
      ...
      , thePartitioned(false)
    {
        ...
        if (theNumLocalSets > 0) {
            ...
            DBG_Assert(
              (theNumLocalSets * theTotalBanks) == theTotalNumSets,
              (<< "Invalid number of sets. Total Sets = " << theTotalNumSets << ", Num Banks = " << theTotalBanks));
        } else {
        ...
        if (thePartitioned) { theTotalNumBuckets *= theHashFns.size(); }
        ...
        DBG_(Dev,
             (<< "Creating directory with " << theNumLocalSets << " sets and " << theTotalNumBuckets
              << " total buckets."));
    ...
    virtual bool sameSet(MemoryAddress a, MemoryAddress b) { return (get_set(a) == get_set(b)); }
    ...
    virtual DirEvictBuffer<_EState>* getEvictBuffer() { return nullptr; }
