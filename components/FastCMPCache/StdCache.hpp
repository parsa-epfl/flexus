    ...
    virtual void saveState(std::ostream& s)
    {
        ...
        for (size_t set = 0; set < (size_t)theNumSets; set++) {
            ...
            for (; block != end; block++) {
                ...
                switch (block->state) {
                    case kModified:
                        checkpoint["tags"][set][i++] = { { "tag", tag }, { "writable", true }, { "dirty", true } };
                        break;
                    case kOwned:
                        checkpoint["tags"][set][i++] = { { "tag", tag }, { "writable", false }, { "dirty", true } };
                        break;
                    case kExclusive:
                        checkpoint["tags"][set][i++] = { { "tag", tag }, { "writable", true }, { "dirty", false } };
                        break;
                    case kShared:
                        checkpoint["tags"][set][i++] = { { "tag", tag }, { "writable", false }, { "dirty", false } };
                        break;
                    case kInvalid: break;
                ...
                DBG_(Trace,
                     (<< theName << " L3 - Saving block " << std::hex << block->tag << " with state "
    ...
    virtual bool loadState(std::istream& s)
    {
        ...
        for (size_t set = 0; set < (size_t)theNumSets; set++) {
            ...
            for (size_t block = 0; block < blockSize; block++) {
                ...
                DBG_(Trace,
                     (<< theName << " L3 - Loading block " << std::hex << ((tag << shift) | (set << blockShift))
                      << " with state " << state2String(state) << " in way " << block));
