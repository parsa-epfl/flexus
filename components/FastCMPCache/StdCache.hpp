...
public:
  StdCache(std::string aName, int32_t aBlockSize, int32_t aNumSets, int32_t anAssociativity,
           evict_function_t anEvict, invalidate_function_t aSendInvalidate, int32_t anIndex,
           Flexus::SharedTypes::tFillLevel aLevel, std::string aReplPolicy)
      : theName(aName) {
    theNumSets = aNumSets;
  ...
  virtual bool loadState(std::istream &s) {
    ...
    for (int32_t set = 0; set < theNumSets; set++) {
      for (int32_t way = 0; way < theAssoc; way++) {
        ...
        DBG_(Trace, (<< theIndex << "-L2: Loading block " << std::hex << bs.tag << " in state "
                     << (char)bs.state));
