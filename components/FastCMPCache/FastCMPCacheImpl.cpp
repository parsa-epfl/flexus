...
class FLEXUS_COMPONENT(FastCMPCache) {
  ...
  combineSnoopResponse(MemoryMessage::MemoryMessageType &a,
...
public:
  ...
  void invalidateBlock(PhysicalMemoryAddress address, SharingVector sharers) {
    ...
    while (!slist.empty()) {
      DBG_(Iface, Addr(address) Comp(*this)(<< "Sending Invalidate to core " << slist.front()
                                            << " for block " << std::hex << address));
  ...
  bool sendInvalidate(uint64_t addr, bool icache, bool dcache) {
    ...
    if (icache) {
      ...
      DBG_(Iface, Addr(addr) Comp(*this)(<< "Sending ICache Invalidate: " << std::hex << addr
                                         << std::dec));
    ...
    if (dcache) {
      theInvalidateMessage.type() = MemoryMessage::Invalidate;
      DBG_(Iface, Addr(addr) Comp(*this)(<< "Sending DCache Invalidate: " << std::hex << addr
                                         << std::dec));
  ...
  void initialize(void) {
    ...
    theDirectory->setInvalidateAction([this](PhysicalMemoryAddress address, SharingVector sharers) {
      this->invalidateBlock(address, sharers);
    });
    ...
    if (cfg.StdArray) {
      ...
      theCache = new RTCache(
          ...
          theIndex, cfg.CacheLevel, cfg.RegionSize, cfg.RTAssoc, cfg.RTSize, cfg.ERBSize, false,
          cfg.ReplPolicy);
    }

    Flexus::Stat::getStatManager()->addFinalizer(
        [this]() { return this->theCacheStats->update(); });
  }
  ...
  void push(interface::RequestIn const &, index_t anIndex, MemoryMessage &aMessage) {
    DBG_(Iface, Comp(*this) Addr(aMessage.address())(<< "Received on Port RequestIn[" << anIndex
                                                     << "] Request: " << aMessage));
  ...
  void push(interface::SnoopIn const &, MemoryMessage &aMessage) {
    ...
    std::tie(sharers, state, dir_entry, dir_valid) =
        theDirectory->snoopLookup(0, addr, aMessage.type());

    // Perform cache lookup
    lookup = theCache->lookup(addr);

    DBG_(Trace, Comp(*this) Addr(aMessage.address())(
                    << "Received SNOOP " << aMessage.type() << " for 0x" << std::hex << addr
                    << std::dec << ", state = " << SharingStatePrinter(state) << ", sharers = "
                    << sharers << ", cache state = " << state2String(lookup->getState())));
        ...
        if ((*index_iter) & 1) {
          ...
          DBG_(Iface,
               Comp(*this) Addr(aMessage.address())(
                   << "Sending " << aMessage.type() << " to " << snoop_core << "-I after recieving "
                   << aMessage.type() << " from DMA reponse is " << snoop_msg.type()));
        } else {
          FLEXUS_CHANNEL_ARRAY(SnoopOutD, snoop_core) << snoop_msg;
          DBG_(Iface,
               Comp(*this) Addr(aMessage.address())(
                   << "Sending " << aMessage.type() << " to " << snoop_core << "-D after recieving "
                   << aMessage.type() << " from DMA reponse is " << snoop_msg.type()));
        }
      } else {
        snoop_msg.type() = aMessage.type();
        FLEXUS_CHANNEL_ARRAY(SnoopOutD, *index_iter) << snoop_msg;
        DBG_(Iface, Comp(*this) Addr(aMessage.address())(
                        << "Sending " << aMessage.type() << " to " << (*index_iter)
                        << "-D after recieving " << aMessage.type() << " from DMA reponse is "
                        << snoop_msg.type()));
      }
  ...
  void push(interface::RegionNotify const &, index_t anIndex, RegionScoutMessage &aMessage) {
    DBG_(Iface, Addr(aMessage.region())(<< "Received on Port RegionNotify[" << anIndex
                                        << "] Request: " << aMessage));
...
private:
  ...
  void processRequest(const index_t anIndex, MemoryMessage &aMessage) {
    ...
    // Perform directory lookup
    std::tie(sharers, state, dir_entry) =
        theDirectory->lookup(anIndex, addr, aMessage.type(), extra_actions);
    ...
    DBG_(Trace, Comp(*this) Addr(aMessage.address())(
                    << "Received " << aMessage.type() << " for 0x" << std::hex << addr << std::dec
                    << " from " << anIndex << ", state = " << SharingStatePrinter(state)
                    << ", sharers = " << sharers
    ...
    DBG_(Iface, Comp(*this) Addr(aMessage.address())(<< "Snoop is " << action.snoop_type
    ...
    if (action.snoop_type != MemoryMessage::NoRequest) {
      ...
      for (; index_iter != snoop_list.end(); index_iter++) {
        DBG_(Iface,
             Comp(*this) Addr(aMessage.address())(<< "Snoop list contains " << (*index_iter)));
      ...
      for (; index_iter != snoop_list.end(); index_iter++) {
        ...
        if ((uint32_t)*index_iter == anIndex) {
          DBG_(Iface,
               Comp(*this) Addr(aMessage.address())(<< "Skipping snoop to " << (*index_iter)));
          // potential_sharers--;
          continue;
        }
        ...
        if (cfg.SeparateID) {
          ...
          if ((*index_iter) & 1) {
            FLEXUS_CHANNEL_ARRAY(SnoopOutI, snoop_core) << snoop_msg;
            DBG_(Iface, Comp(*this) Addr(aMessage.address())(
                            << "Sending " << action.snoop_type << " to " << snoop_core
                            << "-I after recieving " << aMessage.type() << " from " << anIndex
                            << " reponse is " << snoop_msg.type()));
          } else {
            FLEXUS_CHANNEL_ARRAY(SnoopOutD, snoop_core) << snoop_msg;
            DBG_(Iface, Comp(*this) Addr(aMessage.address())(
                            << "Sending " << action.snoop_type << " to " << snoop_core
                            << "-D after recieving " << aMessage.type() << " from " << anIndex
                            << " reponse is " << snoop_msg.type()));
          }
          ...
          FLEXUS_CHANNEL_ARRAY(SnoopOutD, *index_iter) << snoop_msg;
          DBG_(Iface, Comp(*this) Addr(aMessage.address())(
                          << "Sending " << action.snoop_type << " to " << (*index_iter)
                          << "-D after recieving " << aMessage.type() << " from " << anIndex
                          << " reponse is " << snoop_msg.type()));
          response = combineSnoopResponse(response, snoop_msg.type());
        }
        ...
        if (!action.multicast && !(theAlwaysMulticast && multiple_snoops)) {
          if (response == action.terminal_response[0] || response == action.terminal_response[1]) {
            DBG_(Iface, Comp(*this) Addr(aMessage.address())(
                            << "Got terminal response, ending snoop process"));
            break;
          } else {
    ...
    theDirectory->processRequestResponse(anIndex, orig_msg_type, aMessage.type(), dir_entry, addr,
                                         accessed_memory);
    ...
    if (action.allocate) {
      ...
      if (nstate != lookup->getState()) {
        DBG_Assert((lookup->getState() != kInvalid),
                   (<< statName() << "- trying the change state from Invalid to "
                    << state2String(nstate) << " without allocating it."));
...
FLEXUS_PORT_ARRAY_WIDTH(FastCMPCache, RequestIn) {
  return (cfg.CMPWidth ? cfg.CMPWidth
                       : Flexus::Core::ComponentManager::getComponentManager().systemWidth());
}
