...
struct LocalValue {
  ...
  LocalValue(bits aValue) : theValue(aValue), theOutstandingStores(1) {
  }
};

typedef enum { kSimicsReflectsG = -1, kPoisonedByDMA = -2 } eValueTrackSpecialValues;
...
public:
  DMATracerImpl(API::conf_object_t *anUnderlyingObjec) : theUnderlyingObject(anUnderlyingObjec) {
  }
...
struct ValueTracker {
  ...
  void access(uint32_t aCPU, PhysicalMemoryAddress anAddress) {
    ...
    if (iter == theTracker.end()) {
      ...
      if (iter->second.theSimicsReflectsCPU == kPoisonedByDMA) {
        ...
        DBG_(Iface, AddCategory(Special)(<< "CPU[" << aCPU << "] Access.UpdatedGlobalWithDMA "
                                         << anAddress << " Now: " << iter->second));
      }
      ...
      if (local != iter->second.theLocallyVisibleValues.end()) {
        if (iter->second.theSimicsReflectsCPU == static_cast<int>(aCPU)) {
          ...
          DBG_(Iface, (<< "CPU[" << aCPU << "] Access.SimicsReflectsCorrectCPU " << anAddress
                       << " Now: " << iter->second));
          ...
          DBG_(Trace,
               AddCategory(Special)(<< "CPU[" << aCPU << "] Access.SimicsSwitchedToCorrectCPU "
                                    << anAddress << " Now: " << iter->second));
        }
        ...
        if (iter->second.theSimicsReflectsCPU == kSimicsReflectsG) {
          DBG_(Iface, (<< "CPU[" << aCPU << "] Access.SimicsReflectsCorrectGlobal " << anAddress
                       << " Now: " << iter->second));
        } else {
          Flexus::Qemu::Processor cpu = Flexus::Qemu::Processor::getProcessor(aCPU);
          ...
          DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] Access.SwitchedToGlobalValue "
                                           << anAddress << " Now: " << iter->second));
        }
      }
    }
  ...
  void store(uint32_t aCPU, PhysicalMemoryAddress anAddress, eSize aSize, bits aStoreValue) {
    ...
    if (iter == theTracker.end()) {
      ...
      if (iter->second.theSimicsReflectsCPU == kPoisonedByDMA) {
        ...
        iter->second.theGloballyVisibleValue = cpu->readPhysicalAddress(aligned, 8);
        iter->second.theSimicsReflectsCPU = kSimicsReflectsG;
        DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] Store.UpdatedGlobalWithDMA "
                                         << anAddress << " Now: " << iter->second));
      }
      ...
      if (local != iter->second.theLocallyVisibleValues.end()) {
        if (iter->second.theSimicsReflectsCPU == static_cast<int>(aCPU)) {
          ...
          if (simics_value != local->second.theValue) {
            DBG_(Trace, (<< "Simics value more recent than ValueTracker: " << std::hex
                         << simics_value << " Expected: " << local->second.theValue << std::dec));
            // DBG_Assert(false);
          }
          bits updated_value = overlay(simics_value, anAddress, aSize, aStoreValue);
          ...
          DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] Store.UpdateByCurrent "
                                           << anAddress << " Now: " << iter->second));
          ...
          DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] Store.UpdateByExisting "
                                           << anAddress << " Now: " << iter->second));
        }
      } else {
        // No previous value for this CPU.  Start with the globally visible
        ...
        iter->second.theLocallyVisibleValues.insert(
            std::make_pair(aCPU, LocalValue(updated_value)));

        DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] Store.UpdateByNew " << anAddress
                                         << " Now: " << iter->second));
      }
    }
  }
  ...
  void commitStore(uint32_t aCPU, PhysicalMemoryAddress anAddress, eSize aSize, bits aStoreValue) {
    ...
    DBG_Assert(anAddress != 0);
    DBG_Assert(anAddress < 0x40000000000LL);
    DBG_(Iface, (<< "CPU[" << aCPU << "] CommitStore " << anAddress << "[" << aSize
                 << "] = " << std::hex << aStoreValue << std::dec));
    ...
    if (iter->second.theSimicsReflectsCPU == kPoisonedByDMA) {
      ...
      iter->second.theGloballyVisibleValue = cpu->readPhysicalAddress(aligned, 8);
      iter->second.theSimicsReflectsCPU = kSimicsReflectsG;
      DBG_(Iface, AddCategory(Special)(<< "CPU[" << aCPU << "] CommitStore.UpdatedGlobalWithDMA "
                                       << anAddress << " Now: " << iter->second));
    }
    ...
    */
    ...
    if (local->second.theOutstandingStores == 0) {
      DBG_(Trace,
           AddCategory(Special)(<< "CPU[" << aCPU << "] CommitStore.LastOutstandingStoreForCPU "
                                << anAddress));
      iter->second.theLocallyVisibleValues.erase(local);
    }
    if (iter->second.theLocallyVisibleValues.empty()) {
      DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] CommitStore.LastOutstandingStore "
                                       << anAddress));
      theTracker.erase(iter);
    } else {
      DBG_(Trace, AddCategory(Special)(<< "CPU[" << aCPU << "] CommitStore.ResultingState "
                                       << anAddress << " Now: " << iter->second));
    }
  }
  ...
  bits load(uint32_t aCPU, PhysicalMemoryAddress anAddress, eSize aSize) {
    ...
    if (iter->second.theSimicsReflectsCPU == kPoisonedByDMA) {
      ...
      iter->second.theGloballyVisibleValue = cpu->readPhysicalAddress(aligned, 8);
      iter->second.theSimicsReflectsCPU = kSimicsReflectsG;
      DBG_(Iface, AddCategory(Special)(<< "CPU[" << aCPU << "] Load.UpdatedGlobalWithDMA "
                                       << anAddress << " Now: " << iter->second));
    }
    ...
    if (local == iter->second.theLocallyVisibleValues.end()) {
      if (iter->second.theSimicsReflectsCPU != kSimicsReflectsG) {
        ...
        bits val = cpu->readPhysicalAddress(anAddress, aSize);
        DBG_(Iface, (<< "CPU[" << aCPU << "] Load.SimicsAlreadySetToGlobal " << anAddress << "["
                     << aSize << "] = " << std::hex << val << std::dec));
        return bits(val);
      }
    }
    ...
    if (iter->second.theSimicsReflectsCPU != static_cast<int>(aCPU)) {
      ...
      bits val = cpu->readPhysicalAddress(anAddress, aSize);
      DBG_(Trace,
           AddCategory(Special)(<< "CPU[" << aCPU << "] Load.SwitchToLocalValue " << anAddress
                                << "[" << aSize << "] = " << std::hex << val << std::dec));
      return bits(val);
    } else {

      bits val = cpu->readPhysicalAddress(anAddress, aSize);
      DBG_(Iface, (<< "CPU[" << aCPU << "] Load.SimicsAlreadySetToLocal " << anAddress << "["
                   << aSize << "] = " << std::hex << val << std::dec));
      return bits(val);
    }
  }
  ...
  void invalidate(PhysicalMemoryAddress anAddress, eSize aSize) {
    ...
    for (int32_t i = 0; i < aSize; i++) {
      ...
      if (iter != theTracker.end()) {
        ...
        DBG_(Iface, (<< "VT invalidating entry due to DMA on " << aligned << " entry: "
                     << iter->second << " simics_value=" << cpu->readPhysicalAddress(aligned, 8)));
