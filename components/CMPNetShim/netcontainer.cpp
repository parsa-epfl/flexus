...
bool NetContainer::buildNetwork ( const char * filename ) {
  ...
  if ( !infile.good() ) {
    std::cerr << "NetShim: error opening configuration/topology file: \""
              << filename << "\".  Bailing out!" << endl;
    return true;
  }
...
bool NetContainer::readTopologyDoubleToken ( istream & infile,
    int32_t   &   tok1,
    int32_t   &   tok2,
    NetContainer * nc ) {
  char
...
bool NetContainer::readTopologyTripleToken ( istream & infile,
    int32_t & sw,
    int32_t & port,
    int32_t & vc,
    NetContainer * nc ) {
    ...
    if ( strcasecmp ( topTokens[i].token, firstTok ) == 0 ) {
      ...
      if ( topTokens[i].order == VALIDATE_BEFORE && numChannels != 0 ) {
        ...
        if ( validateParameters() ) {
          std::cerr << "ERROR: bad or missing parameters before topology/routing section ";
          return true;
        }
...
bool NetContainer::readTopSrcDest ( istream & infile,
                                    bool   &  isSwitch,
                                    int32_t   &   node,
                                    int32_t   &   sw,
                                    int32_t   &   port,
  ...
  if ( strcasecmp ( STR_NODE, str ) == 0 ) {
    ...
    if ( readTopologyDoubleToken ( infile,
                                   sw,
                                   port,
                                   nc ) )
      return true;
...
bool NetContainer::handleTopology ( istream & infile, NetContainer * nc ) {
  ...
  if ( !fromSwitch && !toSwitch ) {
    std::cerr << "ERROR: topology declaration must have at least one switch endpoint"
              << endl;
    return true;
  }
  ...
  // and the switch the destination (order doesn't really matter in the topology,
  // but this makes the file format more flexible).
  //
  // Similarly for Node->SWITCH connections, we explicitly set the latency on the
  // switch side to be the local delay.
  if ( ! ( fromSwitch && toSwitch ) ) {
...
bool NetContainer::attachSwitchChannels ( NetContainer * nc,
    const int32_t      sw,
    const int32_t      port,
  ...
  * msg = "";
  ...
  if ( nc->switches[sw]->attachChannel ( nc->channels[nc->maxChannelIndex],
                                         port,
                                         destination ) ) {
    msg = "from";
    goto error;
  ...
  if ( nc->switches[sw]->attachChannel ( nc->channels[nc->maxChannelIndex+1],
                                         port,
                                         !destination ) ) {
    msg = "to";
    goto error;
...
bool NetContainer::attachNodeChannels ( NetContainer * nc,
                                        const int32_t node ) {
...
bool NetContainer::handleRoute ( istream & infile, NetContainer * nc ) {
  ...
  // where one or more port/outgoing vc pairs can be specified for adaptive routing
  // ( in order of priority for now )
  ...
  if ( readIntToken ( infile, sw ) || ( sw >= nc->numSwitches || sw < 0 ) ) {
    std::cerr << "ERROR: switch number " << sw
              << " in route specification is invalid" << endl;
    return true;
  }
  ...
  if ( readIntToken ( infile, node ) || ( node >= nc->numNodes || node < 0 ) ) {
    std::cerr << "ERROR: node number " << node
              << " in route specification is invalid" << endl;
    return true;
  }

  if ( readStringToken ( infile, str ) || strcasecmp ( STR_OPENCURLY, str ) ) {
    std::cerr << "ERROR: expected \'" << STR_OPENCURLY
              << "\' in route specification" << endl;
    return true;
  }
  ...
  while ( infile.peek() != '}' ) {
    ...
    if ( readTopologyDoubleToken ( infile,
                                   ...
                                   vc,
                                   nc ) ) {
      std::cerr << "ERROR: unable to parse port:vc specification for route from switch "
                << sw << " to node " << node << endl;
      return true;
    }

    if ( port >= nc->switchPorts || port < 0 ) {
      std::cerr << "ERROR: port number " << port
                << " in route specification is invalid" << endl;
      return true;
    }

    if ( vc >= MAX_NET_VC || vc < 0 ) {
      std::cerr << "ERROR: virtual channel number " << vc
                << " in route specification is invalid" << endl;
      return true;
    }
    ...
    if ( nc->switches[sw]->addRoutingEntry ( node,
         port,
         vc ) ) {
      return true;
    }
  ...
  if ( readStringToken ( infile, str ) || strcasecmp ( STR_CLOSECURLY, str ) ) {
    std::cerr << "ERROR: expected \'" << STR_CLOSECURLY
              << "\' in route specification" << endl;
    return true;
  }
...
bool NetContainer::insertMessage ( MessageState * msg ) {
  ...
  * newNode;
  ...
  if ( msg->srcNode < 0 ||
       msg->srcNode >= numNodes ) {
    std::cerr << "Message has invalid source node: "
              << msg->srcNode << endl;
    return true;
  }

  if ( msg->destNode < 0 ||
       msg->destNode >= numNodes ) {
    std::cerr << "Message has invalid destination node: "
              << msg->destNode << endl;
    return true;
  }

  if ( msg->priority < 0 ||
       msg->priority >= NUM_PRIORITIES ) {
    std::cerr << "Message has an invalid priority: "
              << msg->priority << endl;
    return true;
  }
