...
int SIM_arbiter_record(SIM_arbiter_t *arb, LIB_Type_max_uint new_req, LIB_Type_max_uint old_req, u_int new_grant, u_int old_grant)
{
	...
	switch (arb->model) {
		case MATRIX_ARBITER:
			arb->n_chg_req += int (new_req/2);//SIM_Hamming(new_req, old_req, arb->mask);
		...
		case RR_ARBITER:
			arb->n_chg_req += int (new_grant/2);//SIM_Hamming(new_req, old_req, arb->mask);
			arb->n_chg_grant += new_grant != old_grant;
			/* FIXME: use average instead */
	...
	switch (arb->model) {
		case MATRIX_ARBITER:
			return (arb->n_chg_req * arb->e_chg_req + arb->n_chg_grant * arb->e_chg_grant +
					...
					arb->pri_ff.n_keep_0 * arb->pri_ff.e_keep_0);// +arb->pri_ff.n_clock * arb->pri_ff.e_clock);
