...
#define N_ITEM	(PARM(flit_width) / 8 + (PARM(flit_width) % 8 ? 1:0))
	...
	u_char* data_lineFlexus= new u_char[PARM(flit_width)];
	u_char* old_dataFlexus = new u_char[PARM(flit_width)];
	u_char* new_dataFlexus = new u_char[PARM(flit_width)];

	for(int mg=0;mg< (PARM(flit_width)) ; mg++)
	{
		data_lineFlexus[mg]=0;	
		old_dataFlexus[mg]=0;
		new_dataFlexus[mg]=0;
	}
	for(int mg=0;mg< (PARM(flit_width))/2 ; mg++)
	{
		data_lineFlexus[mg-1+ (PARM(flit_width))/2 ]=0xff;	
		old_dataFlexus[mg]=0xff;
	}
	...
	delete [] new_dataFlexus; // = new u_char[PARM(flit_width)]; 
...
void Orion_link_record(double * d,  double LinkEnergy, long new_data,  long old_data, int data_width)
{
	int bit_flip= int(data_width/2);//SIM_Hamming(new_data, old_data, data_width);
	
	double newpower= LinkEnergy * (double)bit_flip;
	//printf("\n%g %g %d", LinkEnergy, newpower, bit_flip);
	(*d)+=newpower;
	//printf("\n-- d: %g ", (*d));
