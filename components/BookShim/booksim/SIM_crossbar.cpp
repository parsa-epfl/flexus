...
{
	double Ctotal = 0, Ctrans=0, psize=0, nsize=0;
	...
	/* segmented crossbar */
	if (n_seg > 1) {
		Ctotal /*transmission gate*/*= (n_seg + 1)/(n_seg * 2);
		...
		switch (model) {
			case MATRIX_CROSSBAR:
				...
				if (connect_type == TRANS_GATE) { /*transmission gate*/
    				...
    				nsize = Wmemcellr;  
    				I_static += (nsize * NMOS_TAB[0]) * n_in * n_out * data_width;

				    if (trans_type == NP_GATE) {
					      psize = nsize * Wdecinvp / Wdecinvn;
					      I_static += (psize * PMOS_TAB[0]) * n_in * n_out * data_width; 
				    }           
  				}             
...
{
	int BitFlip= int(xb->data_width/2) ;//SIM_Hamming(new_data, old_data, xb->mask);
	//printf("\n### %d ", BitFlip);
	switch (xb->model) {
		case MATRIX_CROSSBAR:
			if (io)	/* input port */
...
{
	//printf("\n in: %d  out: %d   ctrl: %d \n",int(crsbar->n_chg_in), int(crsbar->n_chg_out), int(crsbar->n_chg_ctr));
