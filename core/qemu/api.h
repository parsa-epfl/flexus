...
typedef enum instruction_error {
  ...
  QEMU_IE_Speculative,
  QEMU_IE_Stalling,
  QEMU_IE_Not_Inserted, /* trying to execute or squash an instruction that is
                           inserted. */
  QEMU_IE_Exception,    /* (SPARC-V9 only) */
  QEMU_IE_Fault = QEMU_IE_Exception,
  QEMU_IE_Trap,             /* (X86 only) Returned if a trap is encountered */
  QEMU_IE_Interrupt,        /* (X86 only) Returned if an interrupt is waiting and
                               interrupts are enabled */
  QEMU_IE_Sync_Instruction, /* Returned if sync instruction is not allowd to
                               execute */
...
typedef struct memory_transaction {
  ...
  int data_is_set_and_way : 1;     // wether or not the operation provides set&way
                                   // or address (range)
  union {
...
typedef struct armInterface {
  // This is the interface for a Sparc CPU in QEMU. The interface should provide
  // the following functions: uint64_t read_fp_register_x(conf_object_t *cpu,
  // int reg) void write_fp_register_x(conf_object_t *cpu, int reg, uint64
  // value); uint64_t read_global_register(conf_object_t *cpu, int globals, int
  // reg); uint64_t read_window_register(conf_object_t *cpu, int window, int
  // reg); exception_type_t access_asi_handler(conf_object_t *cpu,
...
typedef void (*QEMU_WRITE_REGISTER_PROC)(conf_object_t *cpu, arm_register_t reg_type, int reg_index,
                                         uint64_t value);
// typedef void                (*CPU_WRITE_REGISTER_PROC) (conf_object_t *cpu,
// int reg_index, unsigned *reg_size, uint64_t value);

// read
typedef uint64_t (*QEMU_READ_REGISTER_PROC)(conf_object_t *cpu, arm_register_t reg_type,
                                            int reg_index);
// typedef uint64_t            (*CPU_READ_REGISTER_PROC)           (void *cs_,
// int reg_idx, int reg_type);
...
// typedef physical_address_t  (*MMU_LOGICAL_TO_PHYSICAL_PROC)     (void *cs,
// logical_address_t va);
...
typedef uint64_t (*QEMU_GET_SIMULATION_TIME_PROC)(void);
...
// void cpu_set_program_counter                            (void* cs, uint64_t
// aVal);
