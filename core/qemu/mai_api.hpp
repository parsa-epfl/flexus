...
protected:
  ...
  void checkException() const {
    //DBG_( Tmp, ( << "MMU: !!!PROVISIONAL!!! Exception Checked! - Implement this" ) );
  //dummy
    //assert(false);
...
public:
  ...
  uint32_t getInstruction(uint64_t addr) const {
    return  API::QEMU_get_instruction(*this, &addr);
  }
...
#else
  uint64_t readRegister(int aRegister, int reg_type = -1) const {
   return API::QEMU_read_register_by_type(*this, aRegister, reg_type);
     }
#endif
...
//#if FLEXUS_TARGET_IS(v9) || FLEXUS_TARGET_IS(ARM)
struct Translation {
...
//  unsigned long long mmuRead(VirtualMemoryAddress anAddress);
//  void mmuWrite(VirtualMemoryAddress anAddress, unsigned long long aValue);
