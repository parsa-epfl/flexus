...
public:
  ...
  bits readVirtualAddress(VirtualMemoryAddress anAddress, size_t size) {
    return readPhysicalAddress(PhysicalMemoryAddress(API::QEMU_logical_to_physical(*this, API::QEMU_DI_Instruction, API::logical_address_t(anAddress))), size);
  }

  PhysicalMemoryAddress translateVirtualAddress(VirtualMemoryAddress anAddress) {
    return PhysicalMemoryAddress(API::QEMU_logical_to_physical(*this, API::QEMU_DI_Instruction, API::logical_address_t(anAddress)));
  }
  ...
  int QEMUId() const { return theQEMUProcessorNumber;  }
  bool mai_mode() const { return true; }
