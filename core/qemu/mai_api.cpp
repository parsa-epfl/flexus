...
struct InterruptManager {
  ...
  void registerHAP() {
    theHandle = API::QEMU_hap_add_callback( "Core_Asynchronous_Trap", reinterpret_cast< API::obj_hap_func_t>(&onInterrupt), 0);
  }
...
void v9ProcessorImpl::initialize() {
  DBG_( Dev, ( << "CPU[" << Qemu::APIFwd::QEMU_get_processor_number(*this) << "] Registering for interrupts "));
...
void v9ProcessorImpl::handleInterrupt( long long aVector) {
  DBG_( Iface, ( << "CPU[" << Qemu::APIFwd::QEMU_get_processor_number(*this) << "] Interrupt " << std::hex << aVector << std::dec ));
...
int v9ProcessorImpl::advance(bool anAcceptInterrupt) {
  ...
  API::instruction_id_t inst = API::QEMU_instruction_begin(*this);
  API::QEMU_instruction_insert(0, inst);

  bool retry = false;
  do {
    retry = false;
    API::instruction_error_t err = API::QEMU_instruction_commit(inst);
    if (err == API::QEMU_IE_Exception ) {
      err = API::QEMU_instruction_handle_exception(inst);
      exception = getPendingException();
      DBG_( Verb, ( << "Exception raised by CPU[" << Qemu::APIFwd::QEMU_get_processor_number(*this) << "]: " << err << " Exception #:" << exception) );
      API::QEMU_instruction_end(inst);
      if (err != API::QEMU_IE_OK) {
        DBG_( Crit, ( << "Got an error trying to handle exception on CPU[" << Qemu::APIFwd::QEMU_get_processor_number(*this) << ": " << err << " exception was: " << exception) );
      }
      return exception; //Instruction is already ended
    } else if (err == API::QEMU_IE_Code_Breakpoint || err == API::QEMU_IE_Step_Breakpoint || err == API::QEMU_IE_Hap_Breakpoint ) {
      DBG_( Verb, ( << "Triggerred Breakpoint on CPU[" << Qemu::APIFwd::QEMU_get_processor_number(*this) << "]" ) );
      API::QEMU_break_cycle(*this, 0);
      retry = true;
    } else if (err != API::QEMU_IE_OK) {
      DBG_( Crit, ( << "Got an error trying to advance CPU[" << Qemu::APIFwd::QEMU_get_processor_number(*this) << "]: " << err) );
...
ProcessorMapper::ProcessorMapper() {
  ...
  DBG_Assert( proc_list.kind == API::QEMU_Val_List, ( << "QEMU_get_all_processors() did NOT return a LIST" ));
  ...
  for (int i = 0; i < proc_count; i++) {
    ...
    int simics_id = APIFwd::QEMU_get_processor_number(cpu);
    ...
    if (boost::regex_match(cpu->name, what, vm_expression)) {
      ...
      DBG_Assert(max_vm >= 0 || (num_flexus_cpus == 0 && num_client_cpus == 0), 
				  ( << "Combined VM/Non-VM workload detected: max_vm = " 
					<< max_vm 
					<< ", flexus = " 
					<< num_flexus_cpus 
					<< ", clients = " 
