...
struct InterruptManager {
  int theHandle;
  typedef std::map< CPUState *, std::function< void( long long) > > dispatch_map;
  dispatch_map theInterruptDispatch;

  void registerHAP() {
    //NOOSHIN: Added the function in QEMU
  ...
  void registerCPU(API::conf_object_t * aCPU, std::function< void( long long) > aDispatchFn ) {
    ...
    DBG_( Tmp, ( << "Flexus: before make_pair with the cpu: " << aCPU ));
    theInterruptDispatch.insert( std::make_pair( (CPUState*)(aCPU->object), aDispatchFn) );
  }

  void interrupt(CPUState* anObj, long long aVector ) {
    DBG_( Tmp, ( << "Flexus: in interrupt with the object: " << anObj ));
    dispatch_map::iterator iter = theInterruptDispatch.find(anObj);

    if (iter != theInterruptDispatch.end()) {
       DBG_( Tmp, ( << "Flexus: in interrupt found the object in theInterruptDispatch" ));
      iter->second( aVector );
    }
    DBG_( Tmp, ( << "Flexus: in interrupt cannot find the object: " << anObj ));
  }

} theInterruptManager;

void onInterrupt (void * aPtr, void* anObj, long long aVector) {
  DBG_( Tmp, ( << "Flexus: onInterrupt called with the object: " << anObj << "and the vector: "<< aVector));
  theInterruptManager.interrupt( (CPUState*) anObj, aVector );
}
...
unsigned long long armProcessorImpl::interruptRead(VirtualMemoryAddress anAddress, int anASI) const {
        ...
        case 0:
            aTranslation.TTAddressResolver = ( aTranslation.isBR0 ?
                    std::make_shared<MMU::L0Resolver>(aTranslation.isBR0,
                        theMMU->Gran0, theMMU->mmu_regs.TTBR0[getQEMUExceptionLevel()],PAWidth) :
                    std::make_shared<MMU::L0Resolver>(aTranslation.isBR0,
                        theMMU->Gran1,theMMU->mmu_regs.TTBR1[getQEMUExceptionLevel()],PAWidth) );
            break;
        case 1:
            aTranslation.TTAddressResolver = ( aTranslation.isBR0 ?
...
armProcessorImpl::InitialTranslationSetup( Translation& aTranslation )
{
    ...
    if(aTranslation.isBR0) initialTTBR = theMMU->mmu_regs.TTBR0[getQEMUExceptionLevel()];
    else initialTTBR = theMMU->mmu_regs.TTBR1[getQEMUExceptionLevel()];
...
armProcessorImpl::doTTEAccess( Translation& aTranslation )
{
    ...
    /* some magic checks */
    unsigned long long lower2Bits = extractBitsWithBounds(rawTTEValue,0,2);
    DBG_Assert( lower2Bits == 0x3, ( << "Encountered INVALID table entry in doTTEAccess: " << std::hex << rawTTEValue << std::dec ));
    /* TODO: distinguish between block and table entries, could cut pwalk early!!! */

    /* Remake the next level resolver */
    aTranslation.currentLookupLevel += 1;
