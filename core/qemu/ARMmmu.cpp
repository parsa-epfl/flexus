...
mmu_access(mmu_t * mmu, mmu_access_t * access) {
  ...
  switch (access->asi) {
    ...
    case 0x5D:  /* DTLB_DATA_ACCESS_REG */
      DBG_( Tmp, ( << "DTLB_DATA_ACCESS_REG tag: " << std::hex << mmu->d_regs.tag_access << " address: " << access->va) );
      i = (access->va >> 3) & 0x1FF;
      tlb = (access->va >> 16) & 0x3;
      size = (access->val >> 61ULL) & 3;
      /*
	For the fTLB, the lower 4 bits are the index number and the
	upper 7 bits are ignored. The relationship between the value
	of the lower 4 bits and the TLB index is as follows:
	0-15: fTLB index number
      */
