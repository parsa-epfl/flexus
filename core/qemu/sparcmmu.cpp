...
void v9ProcessorImpl::initializeMMUs() {
  if (!theMMUs_initialized) {
    ...
    for (unsigned int i = 0; i < theMMUs.size(); ++i) {
      API::conf_object_t * cpu = Qemu::APIFwd::SIM_get_processor(ProcessorMapper::mapFlexusIndex2ProcNum(i));
...
long v9ProcessorImpl::fetchInstruction_QemuImpl(VirtualMemoryAddress const & anAddress) {
  ...
  long op_code = Qemu::API::SIM_read_phys_memory( *this, phy_addr, 4);
  checkException();
...
unsigned long long v9ProcessorImpl::readVAddr_QemuImpl(VirtualMemoryAddress anAddress, int anASI, int aSize) const {
  try {
    API::v9_memory_transaction_t xact;
    translate_QemuImpl( xact, anAddress, anASI );
    unsigned long long value = Qemu::API::SIM_read_phys_memory( *this, xact.s.physical_address, aSize);
    checkException();
  ...
  try {
    ...
    if (anASI == 0x80) {
      ...
      try {
        ...
        checkException();
        unsigned long long value = Qemu::API::SIM_read_phys_memory( *this, phy_addr, aSize);
        checkException();
        return value;
      } catch (MemoryException & anError ) {}
    }
...
void v9ProcessorImpl::translate_QemuImpl(  API::v9_memory_transaction_t & xact, VirtualMemoryAddress anAddress, int anASI ) const {
  ...
  if (anASI == 0x80) {
    ...
    if (phy_addr != xact.s.physical_address ) {
      DBG_(Verb, ( << "CPU[" << Qemu::APIFwd::SIM_get_processor_number(*this) << "] Translation difference between CPU and MMU for " << anAddress << ".  Using CPU translation of " << PhysicalMemoryAddress(phy_addr) ));
...
void v9ProcessorImpl::ckptMMU() {
  DBG_(Verb, ( << "CPU[" << Qemu::APIFwd::SIM_get_processor_number(*this) << "] checkpointing MMU. size=" << theMMUckpts[id()].size()));
  theMMUckpts[id()].push_back(theMMUs[id()]);
}

void v9ProcessorImpl::releaseMMUCkpt() {
  DBG_(Verb, ( << "CPU[" << Qemu::APIFwd::SIM_get_processor_number(*this) << "] releasing oldest MMU checkpoint. size=" << theMMUckpts[id()].size()));
  DBG_Assert(!theMMUckpts[id()].empty(), ( << "CPU[" << Qemu::APIFwd::SIM_get_processor_number(*this) << "] has no checkpoint to release"));
  theMMUckpts[id()].pop_front();
}

void v9ProcessorImpl::rollbackMMUCkpts(int n) {
  DBG_(Verb, ( << "CPU[" << Qemu::APIFwd::SIM_get_processor_number(*this) << "] rolling back " << n << " MMU checkpoints"));
  DBG_Assert(theMMUckpts[id()].size() > (unsigned)n, ( << "CPU[" << Qemu::APIFwd::SIM_get_processor_number(*this) << "] has " << theMMUckpts[id()].size() << " but needs > " << n));
...
long v9ProcessorImpl::fetchInstruction_MMUImpl(Translation & aTranslation, bool aTakeTrap) {
  ...
  if (aTranslation.thePaddr != 0) {
    try {
      op_code = Qemu::API::SIM_read_phys_memory( *this, aTranslation.thePaddr, 4);
      checkException();
    } catch (...) {
      op_code = 0;
    }
...
unsigned long long v9ProcessorImpl::readVAddr_MMUImpl(Translation & aTranslation, int aSize) const {
  try {
    ...
    unsigned long long value = Qemu::API::SIM_read_phys_memory( *this, aTranslation.thePaddr, aSize);
    checkException();
...
/*
 * fm_init_mmu_from_simics: initializes an MMU to Qemu MMU state
 */
void
