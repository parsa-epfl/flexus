...
class StatValue_Counter : public StatValueBase
{
  ...
  public:
    ...
    void reduceSum(StatValue_Counter const& aCounter) { theValue += aCounter.theValue; }

    boost::intrusive_ptr<StatValueBase> sumAccumulator() { return new StatValue_Counter(*this); }
    boost::intrusive_ptr<StatValueBase> avgAccumulator();
    boost::intrusive_ptr<StatValueBase> stdevAccumulator();

    void update(update_type anUpdate) { theValue += anUpdate; }
    void reset(value_type aValue) { theValue = aValue; }
    void print(std::ostream& anOstream, std::string const& options = std::string("")) const { anOstream << theValue; }
    int64_t asLongLong() const { return theValue; };
};
...
class StatValue_PredictionCounter : public StatValueBase
{
  ...
  public:
    ...
    void dismiss() { thePending--; }
    void reset(value_type aValue) { theValue = aValue; }
    void guess(value_type anUpdate) { theValue += anUpdate; }
    void goodGuess() { thePending--; }
    void reject(value_type anUpdate) { theValue -= anUpdate; }
    void print(std::ostream& anOstream, std::string const& options = std::string("")) const
    {
        if (options == "pending") { anOstream << thePending; }
        anOstream << theValue;
    }
    int64_t asLongLong() const { return theValue; };
};
...
class StatValue_Annotation : public StatValueBase
{
  ...
  public:
    ...
    {
        if (other.theValue.length() > theValue.length()) { theValue = other.theValue; }
    }

    boost::intrusive_ptr<StatValueBase> sumAccumulator() { return new StatValue_Annotation(*this); }
    boost::intrusive_ptr<StatValueBase> avgAccumulator() { return new StatValue_Annotation(*this); }

    void update(update_type anUpdate) { theValue = anUpdate; }
    void reset(value_type aValue) { theValue = aValue; }
    void print(std::ostream& anOstream, std::string const& options = std::string("")) const { anOstream << theValue; }
    int64_t asLongLong() const { return 0; };
};
...
class StatValue_Max : public StatValueBase
{
  ...
  public:
    ...
    {
        if (!aMax.theIsValid) { return; }
        if (!theIsValid) {
            theIsValid = true;
            theValue   = aMax.theValue;
            return;
        }
        if (aMax.theValue > theValue) { theValue = aMax.theValue; }
    }

    boost::intrusive_ptr<StatValueBase> sumAccumulator() { return new StatValue_Max(*this); }
    ...
    void update(update_type anUpdate)
    {
        ...
        if (anUpdate > theValue) { theValue = anUpdate; }
    }
    void reset(value_type /*ignored*/) { theIsValid = false; }
    void print(std::ostream& anOstream, std::string const& options = std::string("")) const
    {
        if (theIsValid) {
            anOstream << theValue;
...
class StatValue_Average : public StatValueBase
{
  ...
  public:
    ...
    boost::intrusive_ptr<StatValueBase> sumAccumulator() { return new StatValue_Average(*this); }
    boost::intrusive_ptr<StatValueBase> avgAccumulator();
    boost::intrusive_ptr<StatValueBase> stdevAccumulator();
...
class StatValue_StdDev : public StatValueBase
{
  ...
  public:
    ...
    boost::intrusive_ptr<StatValueBase> sumAccumulator() { return new StatValue_StdDev(*this); }
