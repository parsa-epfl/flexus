...
public:
  Measurement(std::string const &aName, std::string const &aStatExpression)
      : theName(aName), theStatExpressionStr(aStatExpression), theStatExpression(aStatExpression) {
  }
  virtual ~Measurement() {
  }
...
public:
  ...
  void format(std::ostream &anOstream, std::string const &aStat,
              std::string const &options = std::string(""));
...
public:
  PeriodicMeasurement(std::string const &aName, std::string const &aStatExpression, int64_t aPeriod,
...
private:
  ...
  LoggedPeriodicMeasurement() : theOstream(std::cout) {
  }

public:
  LoggedPeriodicMeasurement(std::string const &aName, std::string const &aStatExpression,
                            int64_t aPeriod, accumulation_type anAccumulationType,
