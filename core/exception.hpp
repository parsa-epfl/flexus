...
public:
  ...
  explicit FlexusException(string anExplanation) : theFile(nullptr), theLine(0) {
    theExplanation = new char[anExplanation.size() + 1];
    memcpy(theExplanation, anExplanation.c_str(), anExplanation.size() + 1);
  }
  explicit FlexusException(char const *anExplanation) : theFile(nullptr), theLine(0) {
  ...
  FlexusException(FlexusException const &anException)
      : theExplanation(anException.theExplanation), theFile(anException.theFile),
...
public:
  ...
  explicit SimicsException(string anExplanation) : base(anExplanation) {
  }
  explicit SimicsException(char const *anExplanation) : base(anExplanation) {
  }
...
public:
  ...
  explicit QemuException(string anExplanation) : base(anExplanation) {
  }
  explicit QemuException(char const *anExplanation) : base(anExplanation) {
  }
