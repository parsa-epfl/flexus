...
//{__DBG__internal_IS_UNNAMED__} DBG__internal_IS_UNNAMED( Named(1) (2) (3) )
//=== 0

#define DBG__internal_make_cmd(cmd, input, state)                                                  \
  BOOST_PP_CAT(DBG__internal_make_cmd_, DBG__internal_IS_UNNAMED(input))                           \
  (cmd, input, state)

// Front output command for the unnamed command
#define DBG__internal_make_cmd_1(cmd, input, state)                                                \
  DBG__internal_make_cmd_named(cmd, state,                                                         \
                               BOOST_PP_CAT(DBG__internal_REMAP_COMMAND_, Unnamed input))

// Front output command for any named command
#define DBG__internal_make_cmd_0(cmd, input, state)                                                \
  DBG__internal_make_cmd_named(cmd, state, BOOST_PP_CAT(DBG__internal_REMAP_COMMAND_, input))

#define DBG__internal_make_cmd_named(cmd, state, expanding)                                        \
  DBG__internal_make_cmd_named_d(cmd, state, expanding)
#define DBG__internal_make_cmd_named_d(cmd, state, name, args, junk)                               \
  BOOST_PP_CAT(cmd, name)(args, state)

//{__DBG__internal_make_cmd__} DBG__internal_make_cmd( Output_, (1) (2) (3),st )
//=== Output_Unnamed(1,st)
//{__DBG__internal_make_cmd__} DBG__internal_make_cmd( DBG__internal_NextState_,
// Named(1) (2) (3), st ) === DBG__internal_NextState_Named(1,st)

#define DBG__internal_remove_front(input)                                                          \
  BOOST_PP_CAT(DBG__internal_remove_front_, DBG__internal_IS_UNNAMED(input))                       \
  (input)

// Unnnamed
#define DBG__internal_remove_front_1(input)                                                        \
  DBG__internal_remove_front_named(BOOST_PP_CAT(DBG__internal_REMAP_COMMAND_, Unnamed input))

#define DBG__internal_remove_front_0(input)                                                        \
  DBG__internal_remove_front_named(BOOST_PP_CAT(DBG__internal_REMAP_COMMAND_, input))
...
//{__DBG__internal_remove_front__} DBG__internal_remove_front( (1) (2) (3) ) ===
//(2) (3)
//{__DBG__internal_remove_front__} DBG__internal_remove_front( Named(1) (2) (3)
//) === (2) (3)

#define DBG__internal_recognize(input)                                                             \
  BOOST_PP_CAT(DBG__internal_recognize_, DBG__internal_IS_UNNAMED(input))(input)
#define DBG__internal_recognize_1(input) 1
#define DBG__internal_recognize_0(input)                                                           \
  DBG__internal_recognize_0_D(                                                                     \
      DBG__internal_recognize_test BOOST_PP_CAT(DBG__internal_VALID_COMMAND_, input))
#define DBG__internal_recognize_0_D(test) DBG__internal_recognize_named(test)
#define DBG__internal_recognize_named(recognize)                                                   \
  DBG__internal_recognize_named_D(BOOST_PP_CAT(DBG__internal_recognize_test_, recognize))
...
#define DBG__internal_INPUT_OK_input(input)                                                        \
  BOOST_PP_CAT(DBG__internal_INPUT_OK_, BOOST_PP_NOT(BOOST_PP_VA_IS_EMPTY(input)))                 \
  (input) /**/
...
#define DBG__internal_INPUT_OK_check_token(input)                                                  \
  BOOST_PP_CAT(DBG__internal_INPUT_OK_recognize_, DBG__internal_recognize(input))
#define DBG__internal_INPUT_OK_recognize_0                                                         \
  0(, ; DBG__ERROR_Unrecognized_Operation_In_DBG_Statement;)DBG__internal_INPUT_OK_EAT_2
#define DBG__internal_INPUT_OK_recognize_1 1

#define DBG__internal_PROCESS_INITIAL_STATE(Sev, input)                                            \
  (, DBG__internal_OPERATION_INITIAL_STATE(Sev), input)

#define DBG__internal_PROCESS_STEP(d, state) DBG__internal_PROCESS_STEP_D(state) /**/

#define DBG__internal_PROCESS_STEP_D(state)                                                        \
  (                                 /* previous output*/                                           \
   BOOST_PP_TUPLE_ELEM(3, 0, state) /* output command for the next token */                        \
   DBG__internal_make_cmd(DBG__internal_MakeOutput_, BOOST_PP_TUPLE_ELEM(3, 2, state),             \
                          BOOST_PP_TUPLE_ELEM(3, 1, state)), /* next state command given next      \
                                                                token and present state */         \
   DBG__internal_make_cmd(DBG__internal_NextState_, BOOST_PP_TUPLE_ELEM(3, 2, state),              \
                          BOOST_PP_TUPLE_ELEM(3, 1, state)),     /*remove first token from input*/ \
   DBG__internal_remove_front(BOOST_PP_TUPLE_ELEM(3, 2, state))) /**/

#define DBG__internal_PROCESS(Sev, input)                                                          \
  BOOST_PP_WHILE(DBG__internal_INPUT_OK, DBG__internal_PROCESS_STEP,                               \
...
//)
//{__DBG__internal_PROCESS__} DBG__internal_PROCESS( xx, (Bar) BadName(Foo)
//(Foo) )

// Not currently supported
#define DBG__internal_PROCESS_DEFAULT_OPS(output, State, empty)                                    \
  BOOST_PP_CAT(DBG__internal_PROCESS_DEFAULT_OPS_,                                                 \
               DBG__internal_State_GetSuppressDefaultOps(State))                                   \
  (output, State, empty) /**/
...
#define DBG__internal_FINALIZE(output, state, empty)                                               \
  if (Flexus::Dbg::Debugger::theDebugger->theMinimumSeverity <=                                    \
          DBG__internal_State_GetSev(state) &&                                                     \
    ...
    if (DBG__internal_BUILTIN_CONDITIONS(state) && DBG__internal_State_GetCondition(state)) {      \
      ...
      BOOST_PP_CAT(DBG__internal_FINALIZE_Categories_,                                             \
                   DBG__internal_State_GetHasCategories(state))                                    \
      (state) Flexus::Dbg::Debugger::theDebugger->process(entry__);                                \
    }                                                                                              \
  }                                                                                                \
  do {                                                                                             \
  } while (0) /*require a semicolon after DBG_ statements */ /**/
...
#define DBG__internal_PROCESS_DBG_0(delay)                                                         \
  DBG__internal_PROCESS_DBG_1(DBG__internal_PROCESS_DEFAULT_OPS delay)
