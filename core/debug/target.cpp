...
void
Target::process(Entry const& anEntry)
{
    if (theFilter->match(anEntry) == Filter::Include) { theAction->process(anEntry); }
}
...
void
Target::setFilter(std::unique_ptr<Filter> aFilter)
{
...
void
Target::setAction(std::unique_ptr<Action> anAction)
{
    theAction = std::move(anAction);
}

void
Target::printConfiguration(std::ostream& anOstream, std::string const& anIndent)
{
