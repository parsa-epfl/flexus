...
StaticFormat::StaticFormat(std::string const &aValue) : theValue(aValue) {
...
FieldFormat::FieldFormat(std::string const &aField) : theField(aField) {
}
...
void CompoundFormat::printConfiguration(std::ostream &anOstream,
                                        std::string const &anIndent) const {
  for (auto *aFormat : theFormats) {
