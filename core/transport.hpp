...
struct transport_copier
{
    ...
    {
        void operator()(Transport& theDest, OtherTransport& theSrc) { theDest.set(Slice::tag(), theSrc[Slice::tag()]); }
    };
...
template<class Transport, class OtherTransport>
void
transport_copy(Transport& aDest, OtherTransport& aSrc)
{
...
class TransportSlice : public TransportSlice<N - 1, typename mpl::next<SliceIter>::type>
{
  ...
  public:
    using base::operator[];
    using base::set;
    boost::intrusive_ptr<data> operator[](const tag&) { return theObject; }
    boost::intrusive_ptr<const data> operator[](const tag&) const { return theObject; }
    void set(const tag&, boost::intrusive_ptr<data> anObject) { theObject = anObject; }
};
...
struct Transport : public aux_::TransportSlice<mpl::size<SliceVector>::value, typename mpl::begin<SliceVector>::type>
{
