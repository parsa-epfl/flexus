...
#define BOOST_PP_VA_ELEM(i, ...) BOOST_PP_VA_HEAD(BOOST_PP_VA_SKIP_N(i, __VA_ARGS__))

#define BOOST_PP_VA_FILTER(pred, ...) BOOST_PP_VA_FOLD_LEFT_I(BOOST_PP_VA_FILTER_OP, (pred), , __VA_ARGS__)

#define BOOST_PP_VA_FILTER_OP(elem, pred, ...)                                                                         \
    pred BOOST_PP_VA_LEFT_COMMA(__VA_ARGS__) BOOST_PP_VA_LEFT_COMMA(BOOST_PP_EXPR_IF(pred(elem), elem))

#define BOOST_PP_VA_FIRST_N(n, ...)                                                                                    \
    BOOST_PP_VA_WHILE_HEAD_REM(BOOST_PP_VA_FIRST_N_PRED, BOOST_PP_VA_FIRST_N_OP, (), n, __VA_ARGS__)

#define BOOST_PP_VA_FIRST_N_PRED(result, n, ...) n

#define BOOST_PP_VA_FIRST_N_OP(result, n, elem, ...) (BOOST_PP_VA_RIGHT_COMMA result elem), BOOST_PP_DEC(n), __VA_ARGS__
...
#define BOOST_PP_VA_FOLD_LEFT_I(op, state, first, ...)                                                                 \
    BOOST_PP_VA_SKIP_N(BOOST_PP_VA_SIZE state,                                                                         \
                       BOOST_PP_VA_WHILE_HEAD_REM(BOOST_PP_VA_FOLD_LEFT_I_PRED,                                        \
                                                  BOOST_PP_VA_FOLD_LEFT_I_OP,                                          \
                                                  (BOOST_PP_VA_RIGHT_COMMA state first),                               \
                                                  op,                                                                  \
                                                  __VA_ARGS__))

#define BOOST_PP_VA_FOLD_LEFT_I_PRED(state, op, ...) BOOST_PP_VA_IS_NOT_EMPTYM(__VA_ARGS__)

#define BOOST_PP_VA_FOLD_LEFT_I_OP(state, op, elem, ...)                                                               \
    (BOOST_PP_VA_APPLY(op, elem, BOOST_PP_VA_REM(state))), op, __VA_ARGS__

#define BOOST_PP_VA_FOLD_RIGHT(op, state, ...) BOOST_PP_VA_FOLD_LEFT(op, state, BOOST_PP_VA_REVERSE(__VA_ARGS__))

#define BOOST_PP_VA_FOR(pred, op, macro, ...)                                                                          \
    BOOST_PP_VA_WHILE_HEAD_REM(BOOST_PP_VA_FOR_PRED, BOOST_PP_VA_FOR_OP, (), pred, op, macro, (__VA_ARGS__))
...
#define BOOST_PP_VA_FOR_EACH(macro, ...) BOOST_PP_VA_FOLD_LEFT_I(BOOST_PP_VA_FOR_EACH_OP, (macro), , __VA_ARGS__)

#define BOOST_PP_VA_FOR_EACH_OP(elem, macro, ...) macro, __VA_ARGS__ macro(elem)

#define BOOST_PP_VA_FOR_EACH_I(macro, ...) BOOST_PP_VA_FOLD_LEFT_I(BOOST_PP_VA_FOR_EACH_I_OP, (macro, 0), , __VA_ARGS__)

#define BOOST_PP_VA_FOR_EACH_I_OP(elem, macro, i, ...) macro, BOOST_PP_INC(i), __VA_ARGS__ macro(i, elem)
...
#define BOOST_PP_VA_INSERT(i, elem, ...)                                                                               \
    BOOST_PP_VA_FIRST_N(i, __VA_ARGS__), elem BOOST_PP_VA_LEFT_COMMA(BOOST_PP_VA_SKIP_N(i, __VA_ARGS__))

#define BOOST_PP_VA_IS_EMPTY(...)                                                                                      \
    BOOST_PP_VA_HEAD(                                                                                                  \
      BOOST_PP_VA_CAT1(BOOST_PP_VA_IS_EMPTY_,                                                                          \
                       BOOST_PP_VA_CAT1(BOOST_PP_VA_IS_EMPTY_, BOOST_PP_VA_IS_EMPTY_EAT __VA_ARGS__)()))

#define BOOST_PP_VA_IS_EMPTY_EAT(...)                                      BOOST_PP_VA_IS_EMPTY_EATEN
#define BOOST_PP_VA_IS_EMPTY_BOOST_PP_VA_IS_EMPTY_EAT()                    BOOST_PP_VA_IS_EMPTY_EMPTY
#define BOOST_PP_VA_IS_EMPTY_BOOST_PP_VA_IS_EMPTY_EATEN                    BOOST_PP_VA_IS_EMPTY_NOT_EMPTY
...
#define BOOST_PP_VA_IS_EMPTYM(...) BOOST_PP_VA_HEAD(BOOST_PP_VA_IS_EMPTYM_I(BOOST_PP_VA_IS_EMPTYM_ __VA_ARGS__))

#define BOOST_PP_VA_IS_EMPTYM_I(...) BOOST_PP_VA_IS_EMPTYM_##__VA_ARGS__##BOOST_PP_VA_IS_EMPTYM_
...
#define BOOST_PP_VA_IS_NOT_EMPTYM(...) BOOST_PP_NOT(BOOST_PP_VA_IS_EMPTYM(__VA_ARGS__))

#define BOOST_PP_VA_IS_TUPLE(...) BOOST_PP_VA_HEAD(BOOST_PP_VA_IS_TUPLE_I(BOOST_PP_VA_IS_TUPLE_TEST __VA_ARGS__))

#define BOOST_PP_VA_IS_TUPLE_I(...) BOOST_PP_VA_CAT1(BOOST_PP_VA_IS_TUPLE_, __VA_ARGS__)
...
#define BOOST_PP_VA_LAST_N(n, ...) BOOST_PP_VA_SKIP_N(BOOST_PP_SUB(BOOST_PP_VA_SIZE(__VA_ARGS__), n), __VA_ARGS__)

#define BOOST_PP_VA_LEFT_COMMA(...) BOOST_PP_COMMA_IF(BOOST_PP_VA_IS_NOT_EMPTYM(__VA_ARGS__)) __VA_ARGS__

#define BOOST_PP_VA_LIST(...) BOOST_PP_VA_FOLD_RIGHT(BOOST_PP_VA_TUPLE, BOOST_PP_NIL, __VA_ARGS__)

#define BOOST_PP_VA_POP_BACK(...) BOOST_PP_VA_FIRST_N(BOOST_PP_DEC(BOOST_PP_VA_SIZE(__VA_ARGS__)), __VA_ARGS__)

#define BOOST_PP_VA_PUSH(elem, ...) elem BOOST_PP_VA_LEFT_COMMA(__VA_ARGS__)

#define BOOST_PP_VA_PUSH_BACK(elem, ...) BOOST_PP_VA_RIGHT_COMMA(__VA_ARGS__) elem
...
#define BOOST_PP_VA_REPEAT(count, macro, ...) BOOST_PP_REPEAT(count, BOOST_PP_VA_REPEAT_OP, (macro, __VA_ARGS__))

#define BOOST_PP_VA_REPEAT_OP(z, n, data) BOOST_PP_VA_APPLY(BOOST_PP_VA_INSERT(1, n, BOOST_PP_VA_REM(data)))

#define BOOST_PP_VA_REPLACE(i, elem, ...) BOOST_PP_VA_INSERT(i, elem, BOOST_PP_VA_REMOVE(i, __VA_ARGS__))

#define BOOST_PP_VA_REVERSE(...) BOOST_PP_VA_FOLD_LEFT_I(BOOST_PP_VA_ARGS, (), __VA_ARGS__)

#define BOOST_PP_VA_RIGHT_COMMA(...) __VA_ARGS__ BOOST_PP_COMMA_IF(BOOST_PP_VA_IS_NOT_EMPTYM(__VA_ARGS__))

#define BOOST_PP_VA_SEQ(...) BOOST_PP_VA_FOR_EACH(BOOST_PP_VA_TUPLE, __VA_ARGS__)

#define BOOST_PP_VA_SIZE(...)                                                                                          \
    BOOST_PP_VA_WHILE(BOOST_PP_VA_SIZE_PRED, BOOST_PP_VA_SIZE_OP, 0 BOOST_PP_VA_LEFT_COMMA(__VA_ARGS__))

#define BOOST_PP_VA_SIZE_PRED(n, ...) BOOST_PP_VA_IS_NOT_EMPTYM(__VA_ARGS__)

#define BOOST_PP_VA_SIZE_OP(n, elem, ...) BOOST_PP_INC(n) BOOST_PP_VA_LEFT_COMMA(__VA_ARGS__)

#define BOOST_PP_VA_SLICE(start, len, ...) BOOST_PP_VA_FIRST_N(len, BOOST_PP_VA_SKIP_N(start, __VA_ARGS__))

#define BOOST_PP_VA_SKIP_N(n, ...)                                                                                     \
    BOOST_PP_VA_TAIL(BOOST_PP_VA_WHILE(BOOST_PP_VA_HEAD, BOOST_PP_VA_SKIP_N_OP, n, __VA_ARGS__))

#define BOOST_PP_VA_SKIP_N_OP(n, ...) BOOST_PP_DEC(n), BOOST_PP_VA_TAIL(__VA_ARGS__)

#define BOOST_PP_VA_TAIL(...)         BOOST_PP_VA_TAIL_I(__VA_ARGS__)
#define BOOST_PP_VA_TAIL_I(head, ...) __VA_ARGS__

#define BOOST_PP_VA_TRANSFORM(op, ...) BOOST_PP_VA_FOLD_LEFT_I(BOOST_PP_VA_TRANSFORM_OP, (op), , __VA_ARGS__)

#define BOOST_PP_VA_TRANSFORM_OP(elem, macro, ...) macro, BOOST_PP_VA_RIGHT_COMMA(__VA_ARGS__) macro(elem)

#define BOOST_PP_VA_TUPLE(...) (__VA_ARGS__)

#define BOOST_PP_VA_WHILE(pred, op, ...)                                                                               \
    BOOST_PP_VA_REM(BOOST_PP_EXPAND(                                                                                   \
      BOOST_PP_VA_HEAD BOOST_PP_WHILE(BOOST_PP_VA_WHILE_PRED, BOOST_PP_VA_WHILE_OP, ((__VA_ARGS__), pred, op))))
...
#define BOOST_PP_VA_WHILE_HEAD_REM(...) BOOST_PP_VA_REM(BOOST_PP_VA_HEAD(BOOST_PP_VA_WHILE(__VA_ARGS__)))
