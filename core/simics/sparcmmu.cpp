...
void v9ProcessorImpl::initializeMMUs() {
  if (!theMMUs_initialized) {
    ...
    for (unsigned int i = 0; i < theMMUs.size(); ++i) {
      API::conf_object_t * cpu = Simics::APIFwd::SIM_get_processor(ProcessorMapper::mapFlexusIndex2ProcNum(i));
...
unsigned long long v9ProcessorImpl::readVAddr(VirtualMemoryAddress anAddress, int anASI, int aSize) const {
  return readVAddr_SimicsImpl(anAddress, anASI, aSize);
}
...
PhysicalMemoryAddress v9ProcessorImpl::translateInstruction_SimicsImpl( VirtualMemoryAddress anAddress) const {
  try {
...
long v9ProcessorImpl::fetchInstruction_SimicsImpl(VirtualMemoryAddress const & anAddress) {
  API::logical_address_t addr(anAddress);
  API::physical_address_t phy_addr = API::SIM_logical_to_physical(*this, API::Sim_DI_Instruction, addr);
  checkException();

  long op_code = Simics::API::SIM_read_phys_memory( *this, phy_addr, 4);
  checkException();
...
std::tuple<PhysicalMemoryAddress, bool, bool> v9ProcessorImpl::translateTSB_SimicsImpl(VirtualMemoryAddress anAddress, int anASI) const {
  ...
  try {
    ...
    translate_SimicsImpl( xact, anAddress, anASI);
...
unsigned long long v9ProcessorImpl::readVAddr_SimicsImpl(VirtualMemoryAddress anAddress, int anASI, int aSize) const {
  try {
    API::v9_memory_transaction_t xact;
    translate_SimicsImpl( xact, anAddress, anASI );
    unsigned long long value = Simics::API::SIM_read_phys_memory( *this, xact.s.physical_address, aSize);
    checkException();
...
unsigned long long v9ProcessorImpl::readVAddrXendian_SimicsImpl(VirtualMemoryAddress anAddress, int anASI, int aSize) const {
...
void v9ProcessorImpl::translate_SimicsImpl(  API::v9_memory_transaction_t & xact, VirtualMemoryAddress anAddress, int anASI ) const {
  ...
  DBG_Assert(mmu());
  except = mmu()->logical_to_physical( theMMU, &xact ) ;
  ...
  if (anASI == 0x80) {
    ...
    if (phy_addr != xact.s.physical_address ) {
      DBG_(Verb, ( << "CPU[" << Simics::APIFwd::SIM_get_processor_number(*this) << "] Translation difference between CPU and MMU for " << anAddress << ".  Using CPU translation of " << PhysicalMemoryAddress(phy_addr) ));
...
void v9ProcessorImpl::ckptMMU() {
  DBG_(Verb, ( << "CPU[" << Simics::APIFwd::SIM_get_processor_number(*this) << "] checkpointing MMU. size=" << theMMUckpts[id()].size()));
  theMMUckpts[id()].push_back(theMMUs[id()]);
}

void v9ProcessorImpl::releaseMMUCkpt() {
  DBG_(Verb, ( << "CPU[" << Simics::APIFwd::SIM_get_processor_number(*this) << "] releasing oldest MMU checkpoint. size=" << theMMUckpts[id()].size()));
  DBG_Assert(!theMMUckpts[id()].empty(), ( << "CPU[" << Simics::APIFwd::SIM_get_processor_number(*this) << "] has no checkpoint to release"));
  theMMUckpts[id()].pop_front();
}

void v9ProcessorImpl::rollbackMMUCkpts(int n) {
  DBG_(Verb, ( << "CPU[" << Simics::APIFwd::SIM_get_processor_number(*this) << "] rolling back " << n << " MMU checkpoints"));
  DBG_Assert(theMMUckpts[id()].size() > (unsigned)n, ( << "CPU[" << Simics::APIFwd::SIM_get_processor_number(*this) << "] has " << theMMUckpts[id()].size() << " but needs > " << n));
...
void v9ProcessorImpl::resyncMMU() {
  MMU::fm_init_mmu_from_simics(&theMMUs[id()], SIM_get_attribute(*this, "mmu").u.object );
}
...
void v9ProcessorImpl::translate_MMUImpl(Translation & aTranslation, bool aTakeException) const {
  ...
  if (aTranslation.theType == Translation::eFetch) {
    ...
    if (asi_info & Sim_ASI_Translating) {
...
long v9ProcessorImpl::fetchInstruction_MMUImpl(Translation & aTranslation, bool aTakeTrap) {
  ...
  if (aTranslation.thePaddr != 0) {
    try {
      op_code = Simics::API::SIM_read_phys_memory( *this, aTranslation.thePaddr, 4);
      checkException();
    } catch (...) {
      op_code = 0;
    }
...
unsigned long long v9ProcessorImpl::readVAddr_MMUImpl(Translation & aTranslation, int aSize) const {
  try {
    ...
    unsigned long long value = Simics::API::SIM_read_phys_memory( *this, aTranslation.thePaddr, aSize);
    checkException();
