...
void
StatValueHandle_Base::releaseUpdater()
{
    theUpdater = 0;
}

void
StatValueHandle_Base::reset()
{
    if (theUpdater) { theUpdater->reset(); }
}

std::string
StatValueHandle_Base::name()
{
    if (theStatName) {
        return *theStatName;
    } else {
...
void
StatValueHandle_Base::rename(std::string aName)
{
    if (theStatName) { delete theStatName; }
    theStatName = new std::string(aName);
}

std::ostream&
operator<<(std::ostream& anOstream, StatValueHandle const& aValueHandle)
{
...
std::ostream&
operator<<(std::ostream& anOstream, StatValueArrayHandle const& aValueHandle)
{
    if (aValueHandle.theStatName && aValueHandle.theValue) {
        anOstream << std::setw(40) << std::left << *aValueHandle.theStatName << " " << *aValueHandle.theValue;
    }
...
StatValueHandle::StatValueHandle(Stat* aStat,
...
boost::intrusive_ptr<StatValueBase>
StatValueHandle::sumAccumulator()
{
    return theValue->sumAccumulator();
}
boost::intrusive_ptr<StatValueBase>
StatValueHandle::avgAccumulator()
{
    return theValue->avgAccumulator();
}
boost::intrusive_ptr<StatValueBase>
StatValueHandle::stdevAccumulator()
{
    return theValue->stdevAccumulator();
}
boost::intrusive_ptr<StatValueBase>
StatValueHandle::countAccumulator()
{
    return theValue->countAccumulator();
}

void
StatValueHandle::print(std::ostream& anOstream, std::string const& options)
{
    if (theValue) {
        if (options == "name") {
            anOstream << name();
...
int64_t
StatValueHandle::asLongLong(std::string const& options)
{
    if (theValue) {
...
double
StatValueHandle::asDouble(std::string const& options)
{
    if (theValue) {
...
StatValueArrayHandle::StatValueArrayHandle(Stat* aStat,
...
void
StatValueArrayHandle::newValue(accumulation_type aType)
{
    theValue->newValue(aType);
}
void
StatValueArrayHandle::print(std::ostream& anOstream, std::string const& options)
{
    if (theValue) {
        if (options == "name") {
            anOstream << name();
...
int64_t
StatValueArrayHandle::asLongLong()
{
    if (theValue) {
        return theValue->asLongLong();
    } else {
...
int64_t
StatValueArrayHandle::asLongLong(std::string const& options)
{
    if (theValue) {
        return theValue->asLongLong(options);
    } else {
...
void
Prediction::connectCounter(boost::intrusive_ptr<aux_::StatValue_PredictionCounter> aCounter)
{
...
Prediction::confirm()
{
    ...
    // std::vector< boost::intrusive_ptr< aux_::StatValue_PredictionCounter >
    // >::iterator iter = theCounters.begin(); std::vector< boost::intrusive_ptr<
    // aux_::StatValue_PredictionCounter > >::iterator end = theCounters.end();
    // while (iter != end) {
...
void
Prediction::confirm(int64_t anOverride)
{
    for (auto& aCounter : theCounters)
...
void
Prediction::guess(int64_t anOverride)
{
    for (auto& aCounter : theCounters)
...
void
Prediction::reject(int64_t anOverride)
{
    for (auto& aCounter : theCounters)
...
class LineFormatter
{
  ...
  public:
    ...
    {
        if (aMatch[1].matched) { theOut << aMatch.str(1); }
        if (aMatch[2].matched) {
    ...
    void format(BiDirIter aFirst, BiDirIter aSecond)
    {
        ...
        if (aFirst != aSecond && iter != end) {
            // if (aFirst != aSecond && boost::regex_grep(*this, aFirst, aSecond,
            // theFieldRegex)  > 0) {
            theOut << std::endl;
        }
    }

    void setDefaultMeasurement(std::string const& aMeasurement) { theDefaultMeasurement = aMeasurement; }

    measurement_map& measurements() { return theMeasurements; }
    std::ostream& out() { return theOut; }
};
...
class RootDirective
{
  ...
  public:
    ...
    bool ready()
    {
        return theSubDirective ? theSubDirective->ready() : true;
        // if (theSubDirective) {
    ...
    {
        if (theSubDirective) {
            if (theSubDirective->process()) { theSubDirective = 0; }
        } else {
...
class ForEachMeasurementDirective : public Directive<BiDirIter>
{
  ...
  public:
    ...
    bool ready()
    {
        if (theSubDirective) { return false; }
        return isReady;
    }
    void addLine(BiDirIter aStart, BiDirIter anEnd)
    {
        if (theSubDirective) {
            theSubDirective->addLine(aStart, anEnd);
            if (theSubDirective->ready()) { theSubDirective = 0; }
        } else {
    ...
    bool process()
    {
        ...
        try {
            ...
            // typename measurement_map::iterator iter =
            // theFormatter.measurements().begin(); typename measurement_map::iterator
            // end = theFormatter.measurements().end();
            ...
            //         aLine.theDirective->process();
            //       else
            //         theFormatter.format(aLine.theLine->first,
            //         aLine.theLine->second);
            //     }
            ...
            //     //   }
            //     //   if (l_iter->theLine) {
            //     //     theFormatter.format(l_iter->theLine->first,
            //     l_iter->theLine->second);
            //     //   }
            //     //   ++l_iter;
            //     // }
            //   }
    ...
    bool operator()(const boost::match_results<BiDirIter>& aMatch)
    {
        ...
        if (boost::regex_match(aMatch[0].first, aMatch[0].second, theCommentRegex)) {
            ...
            if (directive_match.str(1) == "FOR-MEASUREMENTS") {
                ...
                if (directive_match[2].matched) { measurements = directive_match.str(2); }
                theDirectiveStack.addDirective(
                  new ForEachMeasurementDirective<BiDirIter>(theLineFormatter, measurements));
        ...
        if (theDirectiveStack.ready()) { theDirectiveStack.process(); }
        return true;
    }
};
...
class StatFormatter
{
  ...
  public:
    ...
    void format(BiDirIter aBegin, BiDirIter anEnd)
    {
        ...
        // boost::regex_grep(theLineProcessor, aBegin, anEnd, theLineRegex,
        // std::match_not_dot_newline);
    }
};
  ...
  public:
    ...
    void registerStat(Stat* aStat)
    {
        ...
        for (auto& aMeasurement : theMeasurements)
            if (aMeasurement.second.get() != nullptr) aMeasurement.second->addToMeasurement(aStat);
    ...
    boost::intrusive_ptr<Measurement> openMeasurement(std::string const& aName,
                                                      std::string const& aStatSpec = std::string(".*"))
    {
    ...
    void openPeriodicMeasurement(std::string const& aName,
                                 int64_t aPeriod,
                                 accumulation_type anAccumulation,
                                 std::string const& aStatSpec = std::string(".*"))
    {
        if (theMeasurements.find(aName) == theMeasurements.end()) {
            boost::intrusive_ptr<PeriodicMeasurement> measurement(
              new PeriodicMeasurement(aName, aStatSpec, aPeriod, anAccumulation));
            for (auto* aStat : theStats)
                measurement->addToMeasurement(aStat);
            // stat_collection::iterator iter = theStats.begin();
            // stat_collection::iterator end = theStats.end();
    ...
    void openLoggedPeriodicMeasurement(std::string const& aName,
                                       int64_t aPeriod,
                                       accumulation_type anAccumulation,
    ...
    {
        if (theMeasurements.find(aName) == theMeasurements.end()) {
            boost::intrusive_ptr<LoggedPeriodicMeasurement> measurement(
              new LoggedPeriodicMeasurement(aName, aStatSpec, aPeriod, anAccumulation, anOstream));
            for (auto* aStat : theStats)
                measurement->addToMeasurement(aStat);
            // stat_collection::iterator iter = theStats.begin();
            // stat_collection::iterator end = theStats.end();
    ...
    void reduceNodes(std::string const& aMeasurementSpec)
    {
        ...
        for (auto& aMeasurement : theMeasurements)
            if (boost::regex_match(aMeasurement.first, spec)) selected_measurements.insert(aMeasurement);
        // measurement_collection::iterator iter = theMeasurements.begin();
        // measurement_collection::iterator end = theMeasurements.end();
        // while (iter != end) {
        ...
        // measurement_collection::iterator sel_iter =
        // selected_measurements.begin(); measurement_collection::iterator sel_end =
        // selected_measurements.end(); while (sel_iter != sel_end) {
        //   SimpleMeasurement * msmt = dynamic_cast<SimpleMeasurement
        //   *>(sel_iter->second.get()); if (msmt) {
        //     msmt->reduceNodes();
        //   }
        //   ++sel_iter;
        // }
    ...
    boost::intrusive_ptr<Measurement> doReduce(eReduction aReduction,
                                               std::string aName,
                                               std::map<std::string, Measurement*>& aMatches)
    {
        ...
        // std::map< std::string, Measurement * >::iterator matchIter =
        // aMatches.begin(); while (matchIter != aMatches.end()) {
    ...
    void printMeasurement(std::string const& aMeasurementSpec, std::ostream& anOstream)
    {
        ...
        for (auto& pair : theMeasurements)
            if (boost::regex_match(pair.first, spec)) matches[pair.first] = pair.second.get();
        // measurement_collection::iterator iter = theMeasurements.begin();
        // measurement_collection::iterator end = theMeasurements.end();
        // while (iter != end) {
        //   if (std::regex_match(iter->first, spec)) {
    ...
    void collapse(std::string const& aMeasurementSpec, std::string const& aFormat, std::ostream& anOstream)
    {
        ...
        if (matches.size() == 0) {
            ...
            boost::intrusive_ptr<Measurement> collapsedMeasurement(
              doReduce(eReduction::eSum, collapsedMeasurementName, matches));
    ...
    void reduce(eReduction aReduction,
                std::string const& aMeasurementSpec,
    ...
    void saveMeasurements(std::string const& aMeasurementSpec, std::string const& aFile) const
    {
        ...
        measurement_collection selected_measurements;
        for (auto& pair : theMeasurements)
            if (boost::regex_match(pair.first, spec)) selected_measurements.insert(pair);
    ...
    void addFinalizer(std::function<void()> aFinalizer) { theFinalizers.push_back(aFinalizer); }
...
getStatManager()
{
    if (theStatManagerImpl == nullptr) { theStatManagerImpl = new aux_::StatManagerImpl(); }
...
void
HistogramPrint::doPrint(std::ostream& anOstream, std::string const& options) const
{
    if (buckets() > 0) {
        if (options.size() > 0) {
            if (options == "buckets") {
...
void
InstanceCounterPrint::doPrint(std::ostream& anOstream, std::string const& options) const
{
    ...
    if (countSort) { std::sort(elements.begin(), elements.end()); }
    ...
    anOstream << std::endl;
    if (exclude) { anOstream << "Excluding bucket " << std::dec << excludeBucket << std::endl; }
    if (printCDF) {
        anOstream << "     " << std::setw(10) << "Count" << std::setw(8) << "Cum"
                  << " Value" << std::endl;
    } else {
    ...
    for (auto& element : elements) {
        ...
        if (valuePrintWidth >= 0) { anOstream << std::setw(valuePrintWidth); }
    ...
    //   anOstream << "      ";
    //   anOstream << std::left << std::setw(10) << std::setfill(' ') <<
    //   elements[i].count; if (printCDF) {
    //     anOstream << std::right << std::setprecision(2) << std::fixed <<
    //     std::setw(6) << cdf_pct << "% ";
    //   } else {
    //     anOstream << std::right << std::setprecision(2) << std::fixed <<
    //     std::setw(6) << pct << "% ";
    ...
    //   }
    //   if (valuePrintHex || ((elements[i].value > 100000) && ((elements[i].value
    //   & 63) == 0))) {
